...
execute as @a[gamemode=!spectator,x=637,y=8,z=-62,dx=12,dy=9,dz=33] unless entity @s[scores={Room=38}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=637,y=8,z=-62,dx=12,dy=9,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 38
scoreboard players set #temp Room 38

execute as @a[scores={Room=38}] run function luigis_beta_mansion:room/original/missingno_22/tick_per_player
