...
execute as @e[x=695,y=8,z=-70,dx=17,dy=9,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 29
scoreboard players set #temp Room 29

execute as @a[scores={Room=29}] run function luigis_beta_mansion:room/original/missingno_13/tick_per_player
