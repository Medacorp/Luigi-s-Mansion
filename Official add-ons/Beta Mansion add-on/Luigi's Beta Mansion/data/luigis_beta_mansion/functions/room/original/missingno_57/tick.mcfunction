...
execute as @a[gamemode=!spectator,x=634,y=91,z=-27,dx=20,dy=8,dz=18] unless entity @s[x=645,y=90,z=-9,dx=6,dy=7,dz=10] unless entity @s[scores={Room=73}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=634,y=91,z=-27,dx=20,dy=8,dz=18] unless entity @s[x=645,y=90,z=-9,dx=6,dy=7,dz=10] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 73
scoreboard players set #temp Room 73

execute as @a[scores={Room=73}] run function luigis_beta_mansion:room/original/missingno_57/tick_per_player
