...
execute as @e[x=672,y=118,z=38,dx=10,dy=9,dz=7] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 54
scoreboard players set #temp Room 54

execute as @a[scores={Room=54}] run function luigis_beta_mansion:room/original/missingno_38/tick_per_player
