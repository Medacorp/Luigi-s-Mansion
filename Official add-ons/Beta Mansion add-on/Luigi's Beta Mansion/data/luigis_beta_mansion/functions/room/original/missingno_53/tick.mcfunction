...
execute as @e[x=721,y=-1,z=-6,dx=21,dy=9,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 69
scoreboard players set #temp Room 69

execute as @a[scores={Room=69}] run function luigis_beta_mansion:room/original/missingno_53/tick_per_player
