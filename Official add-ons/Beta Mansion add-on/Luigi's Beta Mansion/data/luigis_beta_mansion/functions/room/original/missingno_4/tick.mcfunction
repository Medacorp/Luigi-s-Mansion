...
execute as @a[gamemode=!spectator,x=678,y=8,z=-55,dx=8,dy=19,dz=25] unless entity @s[scores={Room=20}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=678,y=8,z=-55,dx=8,dy=19,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 20
