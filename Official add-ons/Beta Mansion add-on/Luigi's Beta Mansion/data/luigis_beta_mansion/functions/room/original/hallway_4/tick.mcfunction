...
execute as @a[gamemode=!spectator,x=686,y=8,z=-23,dx=26,dy=9,dz=8] unless entity @s[scores={Room=9}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=686,y=8,z=-23,dx=26,dy=9,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 9
scoreboard players set #temp Room 9
tag @e[tag=ghost,scores={Room=9}] add no_hidden_move

execute as @a[scores={Room=9}] run function luigis_beta_mansion:room/original/hallway_4/tick_per_player
