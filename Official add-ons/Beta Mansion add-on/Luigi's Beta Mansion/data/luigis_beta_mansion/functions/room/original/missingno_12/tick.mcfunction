...
execute as @e[x=712,y=-1,z=-63,dx=8,dy=18,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 28
scoreboard players set #temp Room 28

execute as @a[scores={Room=28}] run function luigis_beta_mansion:room/original/missingno_12/tick_per_player
