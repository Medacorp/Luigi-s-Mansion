...
execute as @e[x=740,y=26,z=40,dx=19,dy=9,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 56
scoreboard players set #temp Room 56

execute as @a[scores={Room=56}] run function luigis_beta_mansion:room/original/missingno_40/tick_per_player
