...
execute as @e[x=734,y=17,z=-39,dx=14,dy=9,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 70
scoreboard players set #temp Room 70

execute as @a[scores={Room=70}] run function luigis_beta_mansion:room/original/missingno_54/tick_per_player
