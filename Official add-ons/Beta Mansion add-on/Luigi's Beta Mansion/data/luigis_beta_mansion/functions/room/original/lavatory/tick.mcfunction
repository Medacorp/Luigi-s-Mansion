...
execute as @a[gamemode=!spectator,x=660,y=8,z=34,dx=8,dy=9,dz=14] unless entity @s[scores={Room=13}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=660,y=8,z=34,dx=8,dy=9,dz=14] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 13
...
execute as @a[scores={Room=13}] run function luigis_beta_mansion:room/original/lavatory/tick_per_player
