...
execute if data storage luigis_mansion:data current_state.current_data.rooms.back_yard{seen:0b} unless entity @s[gamemode=spectator] run function luigis_beta_mansion:room/original/back_yard/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function luigis_beta_mansion:room/original/back_yard/spectator_tick
