...
execute as @a[gamemode=!spectator,x=696,y=26,z=56,dx=17,dy=10,dz=25] unless entity @s[scores={Room=66}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=696,y=26,z=56,dx=17,dy=10,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 66
