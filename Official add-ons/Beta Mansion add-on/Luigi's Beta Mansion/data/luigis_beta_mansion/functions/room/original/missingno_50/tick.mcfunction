...
execute as @a[gamemode=!spectator,x=696,y=26,z=56,dx=17,dy=9,dz=25] unless entity @s[scores={Room=66}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=696,y=26,z=56,dx=17,dy=9,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 66
scoreboard players set #temp Room 66

execute as @a[scores={Room=66}] run function luigis_beta_mansion:room/original/missingno_50/tick_per_player
