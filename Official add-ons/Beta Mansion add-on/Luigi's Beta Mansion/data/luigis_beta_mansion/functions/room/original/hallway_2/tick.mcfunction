...
execute as @a[gamemode=!spectator,x=722,y=17,z=42,dx=8,dy=9,dz=38] unless entity @s[scores={Room=4}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=722,y=17,z=42,dx=8,dy=9,dz=38] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 4
scoreboard players set #temp Room 4
tag @e[tag=ghost,scores={Room=4}] add no_hidden_move

execute as @a[scores={Room=4}] run function luigis_beta_mansion:room/original/hallway_2/tick_per_player
