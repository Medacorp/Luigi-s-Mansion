...
execute as @e[x=652,y=8,z=-29,dx=12,dy=9,dz=28] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 36
scoreboard players set #temp Room 36

execute as @a[scores={Room=36}] run function luigis_beta_mansion:room/original/missingno_20/tick_per_player
