...
execute as @e[x=737,y=8,z=53,dx=14,dy=9,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 34
scoreboard players set #temp Room 34

execute as @a[scores={Room=34}] run function luigis_beta_mansion:room/original/missingno_18/tick_per_player
