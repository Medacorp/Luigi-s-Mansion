...
execute as @e[x=682,y=128,z=-31,dx=31,dy=16,dz=77] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 64
scoreboard players set #temp Room 64

execute as @a[scores={Room=64}] run function luigis_beta_mansion:room/original/missingno_48/tick_per_player
