...
execute as @e[x=640,y=14,z=-83,dx=10,dy=12,dz=10] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 46
scoreboard players set #temp Room 46

execute as @a[scores={Room=46}] run function luigis_beta_mansion:room/original/missingno_30/tick_per_player
