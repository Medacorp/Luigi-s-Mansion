...
execute if data storage luigis_mansion:data current_state.current_data.rooms.childs_room{seen:0b} unless entity @s[gamemode=spectator] run function luigis_beta_mansion:room/original/childs_room/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function luigis_beta_mansion:room/original/childs_room/spectator_tick
