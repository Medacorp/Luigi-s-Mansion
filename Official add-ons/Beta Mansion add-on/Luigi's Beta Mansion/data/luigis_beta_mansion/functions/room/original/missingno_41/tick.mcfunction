...
execute as @e[x=736,y=26,z=0,dx=20,dy=9,dz=35] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 57
scoreboard players set #temp Room 57

execute as @a[scores={Room=57}] run function luigis_beta_mansion:room/original/missingno_41/tick_per_player
