...
execute as @e[x=697,y=8,z=28,dx=14,dy=9,dz=30] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 14
scoreboard players set #temp Room 14

execute as @a[scores={Room=14}] run function luigis_beta_mansion:room/original/dining_room/tick_per_player
