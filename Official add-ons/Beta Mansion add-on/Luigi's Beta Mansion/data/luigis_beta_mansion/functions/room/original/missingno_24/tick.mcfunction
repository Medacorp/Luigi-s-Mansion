...
execute as @a[gamemode=!spectator,x=645,y=90,z=-9,dx=6,dy=8,dz=10] unless entity @s[scores={Room=40}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=645,y=90,z=-9,dx=6,dy=8,dz=10] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 40
