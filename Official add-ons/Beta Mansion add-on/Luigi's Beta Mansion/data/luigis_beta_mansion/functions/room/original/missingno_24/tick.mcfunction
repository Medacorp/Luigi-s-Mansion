...
execute as @e[x=645,y=90,z=-9,dx=6,dy=7,dz=10] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 40
scoreboard players set #temp Room 40

execute as @a[scores={Room=40}] run function luigis_beta_mansion:room/original/missingno_24/tick_per_player
