...
execute as @e[x=671,y=99,z=37,dx=14,dy=9,dz=19] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 16
scoreboard players set #temp Room 16

execute as @a[scores={Room=16}] run function luigis_beta_mansion:room/original/back_yard/tick_per_player
