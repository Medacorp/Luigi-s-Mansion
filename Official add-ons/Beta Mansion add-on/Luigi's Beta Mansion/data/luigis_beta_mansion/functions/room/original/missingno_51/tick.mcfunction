...
execute as @e[x=693,y=17,z=-88,dx=17,dy=9,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 67
scoreboard players set #temp Room 67

execute as @a[scores={Room=67}] run function luigis_beta_mansion:room/original/missingno_51/tick_per_player
