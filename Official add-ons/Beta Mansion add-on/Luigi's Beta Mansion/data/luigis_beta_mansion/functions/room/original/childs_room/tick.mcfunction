...
execute as @a[gamemode=!spectator,x=737,y=17,z=53,dx=14,dy=9,dz=20] unless entity @s[scores={Room=7}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=737,y=17,z=53,dx=14,dy=9,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 7
scoreboard players set #temp Room 7

execute as @a[scores={Room=7}] run function luigis_beta_mansion:room/original/childs_room/tick_per_player
