...
execute as @e[x=679,y=8,z=-88,dx=12,dy=9,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 30
scoreboard players set #temp Room 30

execute as @a[scores={Room=30}] run function luigis_beta_mansion:room/original/missingno_14/tick_per_player
