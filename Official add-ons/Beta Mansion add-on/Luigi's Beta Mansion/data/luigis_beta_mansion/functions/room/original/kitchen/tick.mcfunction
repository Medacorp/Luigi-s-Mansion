...
execute as @e[x=697,y=8,z=61,dx=14,dy=9,dz=31] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 15
scoreboard players set #temp Room 15

execute as @a[scores={Room=15}] run function luigis_beta_mansion:room/original/kitchen/tick_per_player
