...
execute as @e[x=653,y=-1,z=-22,dx=33,dy=9,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 61
scoreboard players set #temp Room 61

execute as @a[scores={Room=61}] run function luigis_beta_mansion:room/original/missingno_45/tick_per_player
