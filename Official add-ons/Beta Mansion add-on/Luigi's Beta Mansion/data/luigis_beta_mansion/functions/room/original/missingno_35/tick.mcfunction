...
execute as @a[gamemode=!spectator,x=741,y=26,z=-17,dx=19,dy=10,dz=8] unless entity @s[scores={Room=51}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=741,y=26,z=-17,dx=19,dy=10,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 51
