...
execute as @e[x=741,y=26,z=-17,dx=19,dy=9,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 51
scoreboard players set #temp Room 51

execute as @a[scores={Room=51}] run function luigis_beta_mansion:room/original/missingno_35/tick_per_player
