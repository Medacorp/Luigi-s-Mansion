...
execute as @e[x=682,y=108,z=37,dx=17,dy=9,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 6
scoreboard players set #temp Room 6

execute as @a[scores={Room=6}] run function luigis_beta_mansion:room/original/bed_room_1/tick_per_player
