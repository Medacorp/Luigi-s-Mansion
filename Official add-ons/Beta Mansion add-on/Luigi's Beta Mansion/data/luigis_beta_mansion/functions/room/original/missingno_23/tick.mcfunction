...
execute as @e[x=644,y=99,z=-41,dx=10,dy=9,dz=78] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 39
scoreboard players set #temp Room 39

execute as @a[scores={Room=39}] run function luigis_beta_mansion:room/original/missingno_23/tick_per_player
