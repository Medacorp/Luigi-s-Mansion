...
execute as @e[x=644,y=99,z=38,dx=27,dy=9,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 37
scoreboard players set #temp Room 37

execute as @a[scores={Room=37}] run function luigis_beta_mansion:room/original/missingno_21/tick_per_player
