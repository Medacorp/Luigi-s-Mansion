...
execute as @e[x=744,y=26,z=54,dx=22,dy=9,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 65
scoreboard players set #temp Room 65

execute as @a[scores={Room=65}] run function luigis_beta_mansion:room/original/missingno_49/tick_per_player
