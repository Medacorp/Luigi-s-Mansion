...
execute if entity @s[scores={Room=1..73,OpenMapFocus=1..}] run function luigis_beta_mansion:items/game_boy_horror/map/display/original/focus_point
execute unless entity @a[tag=looking_at_map,scores={Room=1..73}] if entity @s[tag=!cancel] run function luigis_beta_mansion:items/game_boy_horror/map/display/original/generate