...
execute if entity @e[tag=same_room,tag=!spectator,tag=player,distance=..0.7,limit=1] if entity @s[tag=!vanish,tag=!appear,tag=!dying,tag=!dead,tag=!captured,tag=!removed_from_existence,tag=!basher,tag=!hurt,tag=!fleeing,tag=!element_hurt] unless entity @s[tag=attack,scores={AttackType=0,ActionTime=41..60}] run function luigis_beta_mansion:entities/beta_puncher/collide
execute if entity @s[scores={AttackType=0}] run function luigis_beta_mansion:entities/beta_puncher/select_attack/punch
execute if entity @s[scores={AttackType=1}] run function luigis_beta_mansion:entities/beta_puncher/select_attack/basher
