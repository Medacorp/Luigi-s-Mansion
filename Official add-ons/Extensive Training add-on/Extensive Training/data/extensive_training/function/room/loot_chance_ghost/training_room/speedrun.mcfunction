...
execute if entity @e[tag=killer,limit=1,scores={GhostCount=2..}] if entity @s[tag=!had_heart,tag=!portrait_ghost,nbt=!{data:{entity:{namespace:"luigis_mansion",id:"boo"}}}] run data modify entity @s data.loot set value {contents:[{name:{namespace:"luigis_mansion",id:"item/small_heart"},variables:{type:"small"}}]}
