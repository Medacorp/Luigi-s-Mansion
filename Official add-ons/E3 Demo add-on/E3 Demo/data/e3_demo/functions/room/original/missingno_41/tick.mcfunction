...
execute as @a[gamemode=!spectator,x=736,y=26,z=0,dx=20,dy=9,dz=35] unless entity @s[scores={Room=57}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=736,y=26,z=0,dx=20,dy=9,dz=35] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 57
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=57},limit=1] run function e3_demo:room/original/missingno_41/ghosts
