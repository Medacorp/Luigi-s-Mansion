...
execute as @a[gamemode=!spectator,x=640,y=14,z=-83,dx=10,dy=12,dz=10] unless entity @s[scores={Room=46}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=640,y=14,z=-83,dx=10,dy=12,dz=10] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 46
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=46},limit=1] run function e3_demo:room/original/missingno_30/ghosts
