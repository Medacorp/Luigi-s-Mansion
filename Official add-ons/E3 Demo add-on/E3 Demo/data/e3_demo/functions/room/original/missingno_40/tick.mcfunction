...
execute as @a[gamemode=!spectator,x=740,y=26,z=40,dx=19,dy=9,dz=8] unless entity @s[scores={Room=56}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=740,y=26,z=40,dx=19,dy=9,dz=8] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 56
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=56},limit=1] run function e3_demo:room/original/missingno_40/ghosts
