...
execute as @a[gamemode=!spectator,x=693,y=17,z=-88,dx=17,dy=9,dz=33] unless entity @s[scores={Room=67}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=693,y=17,z=-88,dx=17,dy=9,dz=33] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 67
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=67},limit=1] run function e3_demo:room/original/missingno_51/ghosts
