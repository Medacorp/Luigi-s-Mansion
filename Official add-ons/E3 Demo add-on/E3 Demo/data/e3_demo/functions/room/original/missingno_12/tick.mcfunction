...
execute as @a[gamemode=!spectator,x=712,y=-1,z=-63,dx=8,dy=18,dz=21] unless entity @s[scores={Room=28}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=712,y=-1,z=-63,dx=8,dy=18,dz=21] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 28
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=28},limit=1] run function e3_demo:room/original/missingno_12/ghosts
