...
execute as @a[gamemode=!spectator,x=744,y=26,z=54,dx=22,dy=9,dz=25] unless entity @s[scores={Room=65}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=744,y=26,z=54,dx=22,dy=9,dz=25] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 65
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=65},limit=1] run function e3_demo:room/original/missingno_49/ghosts
