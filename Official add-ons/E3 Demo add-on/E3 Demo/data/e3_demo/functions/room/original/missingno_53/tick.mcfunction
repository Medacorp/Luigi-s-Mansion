...
execute as @a[gamemode=!spectator,x=721,y=-1,z=-6,dx=21,dy=9,dz=18] unless entity @s[scores={Room=69}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=721,y=-1,z=-6,dx=21,dy=9,dz=18] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 69
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=69},limit=1] run function e3_demo:room/original/missingno_53/ghosts
