...
execute as @a[x=713,y=8,z=-35,dx=7,dy=9,dz=60] unless entity @s[scores={Room=8}] run scoreboard players operation @s LastRoom = @s Room
...
function #e3_demo:room/original/hallway_3/interactions/room

scoreboard players set #temp Room 8
execute as @a[tag=!pause_dialog] run function luigis_mansion:main/get_same_room
scoreboard players reset #temp Room
execute if entity @a[tag=same_room,limit=1] run function e3_demo:room/original/hallway_3/ghosts
