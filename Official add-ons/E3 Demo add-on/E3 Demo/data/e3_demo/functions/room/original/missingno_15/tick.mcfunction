...
execute as @a[gamemode=!spectator,x=734,y=8,z=-39,dx=14,dy=9,dz=20] unless entity @s[scores={Room=31}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=734,y=8,z=-39,dx=14,dy=9,dz=20] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 31

execute as @a[scores={Room=31}] run function e3_demo:room/original/missingno_15/tick_per_player

function #e3_demo:room/original/missingno_15/interactions/room

execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=31},limit=1] run function e3_demo:room/original/missingno_15/ghosts
