...
execute as @a[gamemode=!spectator,x=741,y=26,z=-17,dx=19,dy=9,dz=8] unless entity @s[scores={Room=51}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=741,y=26,z=-17,dx=19,dy=9,dz=8] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 51
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=51},limit=1] run function e3_demo:room/original/missingno_35/ghosts
