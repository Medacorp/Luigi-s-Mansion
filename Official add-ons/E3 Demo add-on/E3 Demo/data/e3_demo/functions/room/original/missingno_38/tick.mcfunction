...
execute as @a[gamemode=!spectator,x=672,y=118,z=38,dx=10,dy=9,dz=7] unless entity @s[scores={Room=54}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=672,y=118,z=38,dx=10,dy=9,dz=7] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 54
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=54},limit=1] run function e3_demo:room/original/missingno_38/ghosts
