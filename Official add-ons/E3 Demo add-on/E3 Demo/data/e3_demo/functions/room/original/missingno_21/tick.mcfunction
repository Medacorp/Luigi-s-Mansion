...
execute as @a[gamemode=!spectator,x=644,y=99,z=38,dx=27,dy=9,dz=18] unless entity @s[scores={Room=37}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=644,y=99,z=38,dx=27,dy=9,dz=18] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 37
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=37},limit=1] run function e3_demo:room/original/missingno_21/ghosts
