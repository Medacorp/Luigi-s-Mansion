...
execute as @a[gamemode=!spectator,x=682,y=108,z=37,dx=17,dy=9,dz=20] unless entity @s[scores={Room=6}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=682,y=108,z=37,dx=17,dy=9,dz=20] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 6
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=6},limit=1] run function e3_demo:room/original/bed_room_1/ghosts
