...
execute as @a[gamemode=!spectator,x=678,y=8,z=-55,dx=8,dy=18,dz=25] unless entity @s[scores={Room=20}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=678,y=8,z=-55,dx=8,dy=18,dz=25] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 20

execute as @a[scores={Room=20}] run function e3_demo:room/original/missingno_4/tick_per_player

function #e3_demo:room/original/missingno_4/interactions/room

execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=20},limit=1] run function e3_demo:room/original/missingno_4/ghosts
