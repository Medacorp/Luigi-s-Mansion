...
execute as @a[gamemode=!spectator,x=697,y=17,z=-23,dx=15,dy=9,dz=8] unless entity @s[scores={Room=24}] run scoreboard players operation @s LastRoom = @s Room
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=24},limit=1] run function e3_demo:room/original/missingno_8/ghosts
