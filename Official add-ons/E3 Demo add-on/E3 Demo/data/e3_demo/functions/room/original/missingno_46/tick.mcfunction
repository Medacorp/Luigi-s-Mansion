...
execute as @a[gamemode=!spectator,x=686,y=-1,z=-63,dx=14,dy=9,dz=21] unless entity @s[scores={Room=62}] run scoreboard players operation @s LastRoom = @s Room
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=62},limit=1] run function e3_demo:room/original/missingno_46/ghosts
