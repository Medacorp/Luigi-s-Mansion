...
execute as @e[x=732,y=26,z=-59,dx=14,dy=9,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 50
...
function #e3_demo:room/original/missingno_34/interactions/room

scoreboard players set #temp Room 50
execute as @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door] run function #luigis_mansion:get_same_room
scoreboard players reset #temp Room
execute if entity @a[tag=exact_same_room,limit=1] run function e3_demo:room/original/missingno_34/ghosts
