...
execute as @a[gamemode=!spectator,x=679,y=8,z=-88,dx=12,dy=9,dz=33] unless entity @s[scores={Room=30}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=679,y=8,z=-88,dx=12,dy=9,dz=33] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 30
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=30},limit=1] run function e3_demo:room/original/missingno_14/ghosts
