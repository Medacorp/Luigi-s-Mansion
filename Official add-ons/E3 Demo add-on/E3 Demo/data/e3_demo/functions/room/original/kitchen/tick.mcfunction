...
execute as @a[gamemode=!spectator,x=697,y=8,z=61,dx=14,dy=9,dz=31] unless entity @s[scores={Room=15}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=697,y=8,z=61,dx=14,dy=9,dz=31] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 15
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=15},limit=1] run function e3_demo:room/original/kitchen/ghosts
