...
execute as @a[gamemode=!spectator,x=644,y=99,z=-41,dx=10,dy=9,dz=78] unless entity @s[scores={Room=39}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=644,y=99,z=-41,dx=10,dy=9,dz=78] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 39
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=39},limit=1] run function e3_demo:room/original/missingno_23/ghosts
