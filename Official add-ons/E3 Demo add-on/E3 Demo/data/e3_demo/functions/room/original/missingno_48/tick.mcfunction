...
execute as @a[gamemode=!spectator,x=682,y=128,z=-31,dx=31,dy=16,dz=77] unless entity @s[scores={Room=64}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=682,y=128,z=-31,dx=31,dy=16,dz=77] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 64
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=64},limit=1] run function e3_demo:room/original/missingno_48/ghosts
