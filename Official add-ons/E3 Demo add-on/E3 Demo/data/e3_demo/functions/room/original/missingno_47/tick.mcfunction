...
execute as @a[gamemode=!spectator,x=682,y=117,z=-10,dx=17,dy=8,dz=35] unless entity @s[scores={Room=63}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=682,y=117,z=-10,dx=17,dy=8,dz=35] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 63

execute as @a[scores={Room=63}] run function e3_demo:room/original/missingno_47/tick_per_player

function #e3_demo:room/original/missingno_47/interactions/room

execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=63},limit=1] run function e3_demo:room/original/missingno_47/ghosts
