...
execute unless data storage luigis_mansion:data dialogs[{room:1}] unless data storage luigis_mansion:data current_state.current_data{obtained_keys:["living_room"]} unless entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{item:{namespace:"luigis_mansion",id:"key"},variant:"living_room"}}}]},limit=1] if entity @e[x=736,y=18,z=8,dx=0,dy=3,dz=1,tag=door,scores={AnimationProgress=39}] run playsound luigis_mansion:entity.ghost.mumble hostile @a 751.0 13 9.0 3
...
execute if entity @a[tag=same_room,limit=1] run function e3_demo:room/original/hallway_1/ghosts
