...
execute as @a[gamemode=!spectator,x=648,y=8,z=15,dx=12,dy=9,dz=14] unless entity @s[scores={Room=12}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=648,y=8,z=15,dx=12,dy=9,dz=14] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 12
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=12},limit=1] run function e3_demo:room/original/bath_room/ghosts
