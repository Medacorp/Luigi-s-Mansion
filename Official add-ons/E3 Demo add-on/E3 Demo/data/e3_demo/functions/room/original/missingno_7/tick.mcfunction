...
execute as @a[gamemode=!spectator,x=686,y=17,z=-23,dx=10,dy=9,dz=8] unless entity @s[scores={Room=23}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=686,y=17,z=-23,dx=10,dy=9,dz=8] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 23

execute as @a[scores={Room=23}] run function e3_demo:room/original/missingno_7/tick_per_player

function #e3_demo:room/original/missingno_7/interactions/room

execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=23},limit=1] run function e3_demo:room/original/missingno_7/ghosts
