...
execute as @a[x=637,y=10,z=-62,dx=12,dy=7,dz=33] unless entity @s[scores={Room=38}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=637,y=10,z=-62,dx=12,dy=7,dz=33] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 38
...
execute if entity @a[tag=!pause_dialog,scores={Room=38},tag=!spectator,limit=1] run function e3_demo:room/original/missingno_22/ghosts
