...
execute as @a[gamemode=!spectator,x=653,y=117,z=-42,dx=30,dy=25,dz=78] unless entity @s[scores={Room=53}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=653,y=117,z=-42,dx=30,dy=25,dz=78] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 53
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=53},limit=1] run function e3_demo:room/original/missingno_37/ghosts
