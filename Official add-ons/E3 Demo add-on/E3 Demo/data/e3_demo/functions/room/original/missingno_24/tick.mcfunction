...
execute as @a[gamemode=!spectator,x=645,y=90,z=-9,dx=6,dy=7,dz=10] unless entity @s[scores={Room=40}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=645,y=90,z=-9,dx=6,dy=7,dz=10] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 40
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=40},limit=1] run function e3_demo:room/original/missingno_24/ghosts
