...
execute as @a[x=734,y=19,z=-39,dx=14,dy=7,dz=20] unless entity @s[scores={Room=70}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=734,y=19,z=-39,dx=14,dy=7,dz=20] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 70
...
execute if entity @a[tag=!pause_dialog,scores={Room=70},tag=!spectator,limit=1] run function e3_demo:room/original/missingno_54/ghosts
