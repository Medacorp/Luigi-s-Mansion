...
execute if entity @s[scores={Room=1..73,OpenMapFocus=1..},tag=!regenerate_map] run function e3_demo:items/game_boy_horror/map/display/original/focus_point
execute unless entity @a[tag=looking_at_map,scores={Room=1..73}] if entity @s[tag=!cancel,tag=!regenerate_map] run function e3_demo:items/game_boy_horror/map/display/original/generate