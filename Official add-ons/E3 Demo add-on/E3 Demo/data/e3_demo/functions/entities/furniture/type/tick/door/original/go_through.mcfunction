...
execute in minecraft:overworld if entity @s[x=720.0,y=102,z=7.0,dx=0,dy=1,dz=0,y_rotation=0..-180] in e3_demo:original if block 753 11 9 minecraft:air run tellraw @a[tag=same_room] {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.mansion","color":"green"},{"translate":"e3_demo:message.no_mansion_inside_found"}]}
execute in minecraft:overworld if entity @s[x=720.0,y=102,z=7.0,dx=0,dy=1,dz=0,y_rotation=0..-180] in e3_demo:original if block 753 11 9 minecraft:air at @s run teleport @s ^ ^ ^-3