execute if entity @s[tag=dead] run playsound e3_demo:entity.boo.vacuumed hostile @a[tag=same_room] ~ ~ ~ 1
...
execute if entity @e[tag=same_room,tag=!spectator,tag=player,distance=..0.7,limit=1] if entity @s[tag=visible] run function e3_demo:entities/boo/collide

execute if entity @s[tag=!visible,tag=!turned] run function e3_demo:entities/boo/initial_rotation
execute store result score #temp Room run scoreboard players get @s Room
execute as @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"e3_demo",id:"boo"}}}]},tag=visible] if score @s Room = #temp Room run scoreboard players add #temp ActionTime 1
execute unless score #temp ActionTime matches 2.. unless entity @e[distance=..0.7,tag=same_room,tag=!spectator,tag=player,limit=1] unless entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"e3_demo",id:"boo"}}}]},tag=visible,distance=..0.7] run tag @s add visible
...
scoreboard players set @s[tag=!shy,tag=was_shy] AnimationProgress 0
execute at @s[tag=!fleeing,tag=!shy,tag=visible] run function luigis_mansion:old_animations/boo/idle
execute at @s[tag=!fleeing,tag=shy,tag=visible] run function e3_demo:old_animations/boo/shy
execute at @s[tag=fleeing,tag=visible] run function luigis_mansion:old_animations/boo/flee
...
execute at @s[tag=visible,tag=fleeing] facing entity @e[tag=same_room,tag=!spectator,tag=player,sort=nearest,limit=1] feet run teleport @s ~ ~ ~ ~-180 ~
execute at @s[tag=visible,tag=fleeing] store result entity @s Rotation[1] float -1 run data get entity @s Rotation[1]
execute at @s[tag=visible] unless entity @s[tag=shy,tag=!fleeing] run function e3_demo:entities/boo/move
...
execute at @s[tag=visible,tag=shy,tag=!fleeing] facing entity @e[tag=same_room,tag=!spectator,tag=player,sort=nearest,limit=1] feet run teleport @s ~ ~ ~ ~ ~
execute at @s[scores={Turn=0},tag=visible,tag=!shy,tag=!fleeing] run function e3_demo:entities/boo/turn
