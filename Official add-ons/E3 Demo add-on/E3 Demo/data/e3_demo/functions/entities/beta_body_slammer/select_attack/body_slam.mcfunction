execute if entity @s[tag=!element_hurt,tag=!hurt,tag=!fleeing,tag=!attack,tag=!collided,tag=!laugh,tag=!complain,tag=!vanish,tag=!appear,scores={StunTime=0}] positioned ^ ^ ^0.7 positioned ~ ~-6 ~ align xyz if entity @e[tag=same_room,tag=!spectator,tag=player,dx=0,dy=6,dz=0,limit=1] run tag @s add attack
