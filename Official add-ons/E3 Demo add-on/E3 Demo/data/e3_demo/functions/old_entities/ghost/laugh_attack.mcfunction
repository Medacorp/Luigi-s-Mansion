...
execute if entity @s[scores={ActionTime=1}] run playsound e3_demo:entity.ghost.laugh hostile @a[tag=same_room] ~ ~ ~ 1
...
execute if score #3ds_remake Loaded matches 1.. at @s facing entity @e[tag=same_room,tag=!spectator,tag=player,sort=nearest,limit=1] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
execute if score #3ds_remake Loaded matches 1.. at @s positioned ^ ^ ^0.7 unless entity @a[distance=..0.7,scores={Invulnerable=0},tag=!spectator] run function luigis_mansion:old_entities/ghost/move
