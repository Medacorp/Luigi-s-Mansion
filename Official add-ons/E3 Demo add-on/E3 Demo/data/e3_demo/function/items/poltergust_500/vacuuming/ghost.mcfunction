...
execute if entity @s[scores={HurtTime=30..,Health=1..},tag=!no_ai] at @e[type=minecraft:marker,tag=position,limit=1] unless entity @a[tag=me,distance=..0.1,limit=1] unless score #temp Time > #temp PullAngle at @s run function e3_demo:items/poltergust_500/vacuuming/hurt_ghost
