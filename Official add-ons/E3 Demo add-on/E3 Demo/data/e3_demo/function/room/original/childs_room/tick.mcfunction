...
execute if entity @a[tag=!door_animation,scores={Room=7},limit=1] if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.childs_room{cleared:0b} store result storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.childs_room.time_spent_in int 1 run scoreboard players add #temp Time 1
