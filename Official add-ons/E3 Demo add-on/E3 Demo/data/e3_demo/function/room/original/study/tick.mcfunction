...
execute if entity @a[tag=!door_animation,scores={Room=5},limit=1] if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.study{cleared:0b} store result storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.study.time_spent_in int 1 run scoreboard players add #temp Time 1
