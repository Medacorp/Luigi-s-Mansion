...
execute store result score #temp Time run data get storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.missingno_2.time_spent_in
execute if entity @a[tag=!door_animation,scores={Room=18},limit=1] if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.missingno_2{cleared:0b} store result storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.missingno_2.time_spent_in int 1 run scoreboard players add #temp Time 1
