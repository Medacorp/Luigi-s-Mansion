...
execute if entity @a[tag=!door_animation,scores={Room=45},limit=1] if data storage luigis_mansion:data current_state.current_data.rooms.missingno_29{cleared:0b} store result storage luigis_mansion:data current_state.current_data.rooms.missingno_29.time_spent_in int 1 run scoreboard players add #temp Time 1
