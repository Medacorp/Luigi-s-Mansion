...
execute unless data storage luigis_mansion:data dialogs[{room:1}] unless data storage luigis_mansion:data current_state.luigis_mansion.current_data{obtained_keys:["living_room"]} unless entity @e[nbt={data:{item:{namespace:"luigis_mansion",id:"key"},variant:"living_room"}},limit=1] if entity @e[x=736,y=19,z=8,dx=0,dy=3,dz=1,tag=door,scores={AnimationProgress=39}] run playsound luigis_mansion:entity.ghost.mumble hostile @a 751.0 13 9.0 3
...
execute store result score #temp Time run data get storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.hallway_1.time_spent_in
execute if data storage luigis_mansion:data current_state.luigis_mansion.rooms.underground_lab{cleared:1b} if entity @a[tag=!door_animation,scores={Room=2},limit=1] if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.hallway_1{cleared:0b} store result storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.hallway_1.time_spent_in int 1 run scoreboard players add #temp Time 1
