...
execute unless data storage luigis_mansion:data current_state.current_data{obtained_keys:["hallway_3"]} run data modify storage luigis_mansion:data furniture.loot set value {contents:[{name:{namespace:"luigis_mansion",id:"item/key {door:'hallway_3'}"},variables:{rotation:[-90.0f,0.0f]}}]}
...
execute if data storage luigis_mansion:data current_state.current_data{money_spawned:["childs_room_room_clear_chest"]} run data modify storage luigis_mansion:data furniture merge value {searched:1b,no_search_animation:1b,no_spawn_sound:1b}
