...
execute at @s[tag=!flee] facing entity @e[tag=same_room,tag=luigi,sort=nearest,limit=1] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
...
execute at @s[tag=!dead,tag=!removed_from_existence,tag=!in_vacuum] if entity @e[tag=same_room,tag=luigi,distance=..0.7,limit=1] run function e3_demo:entities/shining_ghost/collide with entity @s data.entity