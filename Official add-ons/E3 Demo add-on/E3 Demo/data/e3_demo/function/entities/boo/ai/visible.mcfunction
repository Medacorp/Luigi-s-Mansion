...
execute at @s[tag=shy,tag=!flee] at @e[tag=same_room,tag=luigi,sort=nearest,limit=1] run function luigis_mansion:entities/ghost/set_target
...
tag @s remove shy
execute at @e[tag=same_room,tag=luigi] positioned ^ ^ ^5 run tag @s[distance=..5] add shy

scoreboard players set @s[tag=shy] TargetTask 0
scoreboard players set @s[tag=!shy] TargetTask 3

execute at @s[tag=!dead,tag=!removed_from_existence,tag=!captured] if entity @e[tag=same_room,tag=luigi,distance=..0.7,limit=1] run function e3_demo:entities/boo/collide with entity @s data.entity