...
summon minecraft:item_display ~ ~ ~ {CustomName:{type:"translatable",translate:"e3_demo:entity.mouse"},item:{id:"minecraft:leather_chestplate",count:1,components:{"minecraft:item_model":"luigis_mansion:entity/mouse","minecraft:custom_model_data":{flags:[1b],colors:[I;16769024]}}},item_display:"head",Tags:["model_piece","mouse_model","found_owner","this_entity"],brightness:{block:15,sky:15}}
...
execute as @e[tag=this_entity,limit=1] store result score @s Move run data get storage luigis_mansion:data current_state.luigis_mansion.current_data.ghosts.e3_demo.mouse.speed
data modify entity @e[tag=this_entity,limit=1] data.loot_storage set from storage luigis_mansion:data current_state.luigis_mansion.current_data.ghosts.e3_demo.mouse.loot
data modify entity @e[tag=this_entity,limit=1] data.damage set from storage luigis_mansion:data current_state.luigis_mansion.current_data.ghosts.e3_demo.mouse.damage
