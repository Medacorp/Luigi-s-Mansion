...
execute at @s[tag=!furniture_scanned] unless entity @e[tag=target,limit=1] if block ~ ~ ~ minecraft:oak_button[face=floor] run function luigis_mansion:items/gameboy_horror/scan/block
...
execute at @s[tag=!furniture_scanned] if block ~ ~ ~ minecraft:air unless entity @e[tag=target,limit=1] positioned ^ ^ ^0.5 if score #temp Move matches ..200 run function luigis_mansion_3ds_remake:items/gameboy_horror/scan/target_gooigi