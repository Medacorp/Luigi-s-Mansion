summon minecraft:armor_stand ~ ~-1.4 ~ {CustomName:'{"translate":"luigis_mansion_3ds_remake:furniture.gallery_chest"}',Tags:["furniture","generic","immobile","chest","standing_furniture","scan_ignore","this_entity"],Pose:{Head:[0.0f,0.01f,0.0f]},NoGravity:1b,Marker:1b,ArmorItems:[{},{},{},{id:"minecraft:diamond_pickaxe",Count:1b,tag:{Unbreakable:1b,Damage:19,CustomModelData:-11}}],Invisible:1b,DisabledSlots:2039583}
...
execute if data storage luigis_mansion:data furniture{open:1b} run data modify entity @e[tag=this_entity,limit=1] ArmorItems[3].tag.CustomModelData set value -7
...
scoreboard players set @e[tag=this_entity,limit=1] FurnitureSizeL 22
scoreboard players set @e[tag=this_entity,limit=1] FurnitureSizeU 14
scoreboard players set @e[tag=this_entity,limit=1] FurnitureSizeF 14
