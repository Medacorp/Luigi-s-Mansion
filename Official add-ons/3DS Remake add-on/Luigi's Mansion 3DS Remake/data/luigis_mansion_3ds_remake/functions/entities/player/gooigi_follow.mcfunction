...
execute unless entity @s[scores={OpenMapTime=1..}] at @a[tag=follow_me,tag=!push_open_door,tag=!pull_open_door,limit=1] unless entity @a[tag=follow_me,scores={KnockbackType=1..},limit=1] run function luigis_mansion_3ds_remake:entities/player/gooigi_follow_player
