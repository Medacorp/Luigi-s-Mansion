...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] unless data storage luigis_mansion:data current_state.current_data{money_spawned:["fortune_tellers_room_wooden_cabinet_mounted_wooden_colorful_candelabra_1"]} run data modify storage luigis_mansion:data furniture.loot set value {name:"fortune_tellers_room_wooden_cabinet_mounted_wooden_colorful_candelabra_1",contents:{luigis_mansion:{gold_coin:10,bill:10}}}
kill @e[type=minecraft:marker,tag=chance]
execute positioned 736 11 -21.2 rotated -90 0 run function luigis_mansion:spawn_furniture/wooden_cabinet_mounted_wooden_colorful_candelabra/blue_green_red