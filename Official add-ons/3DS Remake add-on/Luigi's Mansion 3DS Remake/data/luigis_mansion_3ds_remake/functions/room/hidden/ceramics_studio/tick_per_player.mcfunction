...
execute if data storage luigis_mansion:data current_state.current_data.rooms.ceramics_studio{seen:0b} unless entity @s[gamemode=spectator] run function luigis_mansion_3ds_remake:room/hidden/ceramics_studio/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function luigis_mansion_3ds_remake:room/hidden/ceramics_studio/spectator_tick
