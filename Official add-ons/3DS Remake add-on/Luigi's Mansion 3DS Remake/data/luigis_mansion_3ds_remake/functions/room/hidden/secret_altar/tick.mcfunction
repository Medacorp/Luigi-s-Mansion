...
execute as @a[gamemode=!spectator,x=634,y=90,z=-27,dx=20,dy=8,dz=18] unless entity @s[scores={Room=72}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=634,y=90,z=-27,dx=20,dy=8,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 72
...
execute as @a[scores={Room=72}] run function luigis_mansion_3ds_remake:room/hidden/secret_altar/tick_per_player
