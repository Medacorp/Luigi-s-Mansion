...
execute as @e[x=713,y=-1,z=-22,dx=22,dy=9,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 59
scoreboard players set #temp Room 59
tag @e[tag=ghost,scores={Room=59}] add no_hidden_move

execute as @a[scores={Room=59}] run function luigis_mansion_3ds_remake:room/hidden/hallway_20/tick_per_player
