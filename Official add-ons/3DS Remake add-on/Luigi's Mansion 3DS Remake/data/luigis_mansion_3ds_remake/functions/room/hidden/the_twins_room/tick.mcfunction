...
execute as @e[x=736,y=17,z=29,dx=14,dy=9,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 49
scoreboard players set #temp Room 49

execute as @a[scores={Room=49}] run function luigis_mansion_3ds_remake:room/hidden/the_twins_room/tick_per_player

function #luigis_mansion_3ds_remake:room/hidden/the_twins_room/interactions/room

execute if score #the_twins_room Wave matches 5 run function luigis_mansion_3ds_remake:room/hidden/the_twins_room/hide_and_seek
