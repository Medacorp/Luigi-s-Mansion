...
execute as @a[gamemode=!spectator,x=673,y=8,z=30,dx=15,dy=9,dz=16] unless entity @s[scores={Room=48}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=673,y=8,z=30,dx=15,dy=9,dz=16] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 48
...
execute as @a[scores={Room=48}] run function luigis_mansion_3ds_remake:room/hidden/projection_room/tick_per_player
