...
execute as @e[x=637,y=17,z=-62,dx=12,dy=9,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 34
scoreboard players set #temp Room 34

execute as @a[scores={Room=34}] run function luigis_mansion_3ds_remake:room/hidden/tea_room/tick_per_player
