...
execute as @a[gamemode=!spectator,x=637,y=17,z=-62,dx=12,dy=10,dz=33] unless entity @s[scores={Room=34}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=637,y=17,z=-62,dx=12,dy=10,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 34
