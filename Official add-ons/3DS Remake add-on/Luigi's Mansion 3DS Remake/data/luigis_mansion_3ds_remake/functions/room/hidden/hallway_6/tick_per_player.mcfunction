...
execute if data storage luigis_mansion:data current_state.current_data.rooms.hallway_6{seen:0b} unless entity @s[gamemode=spectator] run function luigis_mansion_3ds_remake:room/hidden/hallway_6/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function luigis_mansion_3ds_remake:room/hidden/hallway_6/spectator_tick

tag @s add already_ticked

execute if entity @s[x=669.5,y=11,z=8.5,tag=!spectator,distance=..4] if data storage luigis_mansion:data current_state.current_data{obtained_keys:["courtyard"]} run function luigis_mansion_3ds_remake:room/hidden/hallway_6/clear_blockade
