...
execute if data storage luigis_mansion:data current_state.current_data.rooms.hallway_18{seen:0b} unless entity @s[gamemode=spectator] run function luigis_mansion_3ds_remake:room/hidden/hallway_18/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function luigis_mansion_3ds_remake:room/hidden/hallway_18/spectator_tick
