...
execute if entity @s[tag=gooigi] unless score #temp Step matches 1 run tellraw @a {"type":"translatable","translate":"chat.type.text","with":[{"type":"selector","selector":"@s","color":"green"},{"type":"translatable","translate":"luigis_mansion_3ds_remake:message.player.scan_furniture.11"}]}
