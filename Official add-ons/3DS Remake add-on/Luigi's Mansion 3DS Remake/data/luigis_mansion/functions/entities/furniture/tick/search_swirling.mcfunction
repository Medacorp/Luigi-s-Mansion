...
execute unless entity @e[distance=..0.1,tag=hidden_boo,tag=caught_by_boo_radar,limit=1] rotated ~ 0 if entity @s[nbt={ArmorItems:[{tag:{loot:{}}}]}] unless entity @a[tag=portrait_battle,limit=1] run function luigis_mansion:entities/furniture/tick/spawn_contents
