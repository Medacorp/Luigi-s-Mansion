...
execute at @s[tag=!furniture_scanned,tag=!entity_scanned] if block ~ ~ ~ minecraft:oak_button[face=floor] run function luigis_mansion:selection_menu/game_boy_horror/scan/block
...
execute at @s[tag=!furniture_scanned,tag=!entity_scanned,tag=!scanned_block] if score #temp Move matches 201 if entity @s[y_rotation=87..93,x_rotation=-23..-16] run advancement grant @p[tag=scanning_player,gamemode=!spectator] only luigis_mansion:vanilla/to_infinity_and_beyond
execute at @s[tag=!furniture_scanned,tag=!entity_scanned,tag=!scanned_block] if block ~ ~ ~ minecraft:air positioned ^ ^ ^0.5 if score #temp Move matches ..200 run function 3ds_remake:selection_menu/game_boy_horror/scan/target_gooigi