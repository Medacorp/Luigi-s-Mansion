...
execute if entity @a[advancements={luigis_mansion:vanilla/clear_hidden_mansion=false}] unless data storage 3ds_remake:data current_state.trophy.master{done:1b} run advancement revoke @a only luigis_mansion:vanilla/unlock_hidden_mansion
execute if data storage luigis_mansion:data current_state.high_scores[{data_index:0}] run advancement grant @a only 3ds_remake:3ds_remake/unlock_hidden_mansion
