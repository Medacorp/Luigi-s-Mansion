...
execute unless entity @s[scores={HomeX=0,HomeZ=0}] at @s[tag=!stop_model] positioned ^ ^-1.65 ^.25 rotated as @e[tag=gooigi,limit=1] rotated ~ 0 positioned ^ ^ ^-0.5 if entity @e[tag=gooigi,limit=1,distance=..0.49] run tag @s add moving_backwards
execute unless entity @s[scores={HomeX=0,HomeZ=0}] at @s[tag=!stop_model,tag=!moving_backwards] facing entity @e[tag=gooigi,limit=1] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
execute at @s[tag=!stop_model,tag=moving_backwards] rotated as @e[tag=gooigi,limit=1] rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
...
execute unless entity @s[scores={RotationDifference=-50000..50000},tag=!stop_model] run function 3ds_remake:animations/gooigi/rotate_body
scoreboard players remove @s[scores={PlayerRotation=360000..}] PlayerRotation 360000
