...
execute if data storage luigis_mansion:data rooms.gallery{seen:0b} unless entity @s[gamemode=spectator] run function 3ds_remake:room/gallery/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function 3ds_remake:room/gallery/wall_warp
