...
execute if entity @s[scores={PortraitBattle=0}] run data modify storage 3ds_remake:data loaded_mansion.high_scores set from storage 3ds_remake:data current_state.portrait_ghosts_defeated_in.shivers[{data_index:0,mansion_type:0}]
...
execute if entity @s[scores={PortraitBattle=0}] run data modify storage luigis_mansion:data current_state.current_data.money_spawned set value ["butlers_room_metal_bucket","butlers_room_tool_cabinet","butlers_room_chain_suspended_lamp"]