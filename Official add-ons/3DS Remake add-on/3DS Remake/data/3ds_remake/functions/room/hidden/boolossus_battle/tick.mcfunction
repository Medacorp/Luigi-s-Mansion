...
execute as @e[x=562,y=13,z=-63,dx=48,dy=78,dz=87] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 75
...
execute if score #boolossus_battle ReturnTimer matches 100.. as @a[gamemode=!spectator,scores={Room=75}] run function 3ds_remake:room/hidden/boolossus_battle/clear_balcony_2
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=75},limit=1] run function 3ds_remake:room/hidden/boolossus_battle/ghosts