...
execute unless data storage luigis_mansion:data current_state.current_data{money_spawned:["sitting_room_speedy_spirit"]} run data modify storage luigis_mansion:data entity set value {room:69,loot:{name:"sitting_room_speedy_spirit"},tags:["hiding_in_furniture"]}
