...
execute if score #bogmire_battle ReturnTimer matches 100.. as @a[gamemode=!spectator,scores={Room=74}] run function 3ds_remake:room/hidden/bogmire_battle/clear_graveyard
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=74},limit=1] run function 3ds_remake:room/hidden/bogmire_battle/ghosts