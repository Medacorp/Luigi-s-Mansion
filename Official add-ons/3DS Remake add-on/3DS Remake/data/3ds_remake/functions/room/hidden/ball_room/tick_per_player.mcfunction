...
execute if data storage luigis_mansion:data current_state.current_data.rooms.ball_room{seen:0b} unless entity @s[gamemode=spectator] run function 3ds_remake:room/hidden/ball_room/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function 3ds_remake:room/hidden/ball_room/wall_warp
