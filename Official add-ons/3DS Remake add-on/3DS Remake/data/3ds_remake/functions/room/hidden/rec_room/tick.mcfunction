...
execute as @e[x=637,y=10,z=-62,dx=12,dy=7,dz=33] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 32
...
execute if entity @a[tag=!pause_dialog,scores={Room=32},tag=!spectator,limit=1] run function 3ds_remake:room/hidden/rec_room/ghosts