execute unless score #breaker_room Ticking matches 1 run function #3ds_remake:room/hidden/breaker_room/load
execute as @a[gamemode=!spectator,x=686,y=-1,z=-63,dx=14,dy=9,dz=21] unless entity @s[scores={Room=57}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=686,y=-1,z=-63,dx=14,dy=9,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 57
scoreboard players set #temp Room 57

execute as @e[scores={Room=57},type=!minecraft:marker] unless entity @s[gamemode=spectator] run scoreboard players set @s MirrorX 699

execute as @a[scores={Room=57}] run function 3ds_remake:room/hidden/breaker_room/tick_per_player

function #3ds_remake:room/hidden/breaker_room/interactions/room

scoreboard players set #temp Room 57
execute as @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door] run function #luigis_mansion:get_same_room
scoreboard players reset #temp Room
execute if entity @a[tag=exact_same_room,limit=1] run function 3ds_remake:room/hidden/breaker_room/ghosts
tag @a[tag=same_room] remove same_room
tag @a[tag=exact_same_room] remove exact_same_room