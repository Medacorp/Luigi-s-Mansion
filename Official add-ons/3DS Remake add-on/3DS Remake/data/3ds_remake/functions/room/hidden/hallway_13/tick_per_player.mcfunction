...
execute if data storage luigis_mansion:data current_state.current_data.rooms.hallway_13{seen:0b} unless entity @s[gamemode=spectator] run function 3ds_remake:room/hidden/hallway_13/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function 3ds_remake:room/hidden/hallway_13/wall_warp

tag @s add already_ticked

execute if entity @s[x=718,y=20,z=-19,tag=!spectator,distance=..8,tag=!chance_check] if data storage luigis_mansion:data current_state.current_data{blackout:0b} unless data storage luigis_mansion:data current_state.current_data.rooms.hallway_13{cleared:1b} unless data storage luigis_mansion:data current_state.current_data{money_spawned:["hallway_9_hallway_15_gold_mouse"]} unless entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"gold_mouse"}}}}]},scores={Room=35..41},limit=1] run function 3ds_remake:room/hidden/hallway_13/gold_mouse
