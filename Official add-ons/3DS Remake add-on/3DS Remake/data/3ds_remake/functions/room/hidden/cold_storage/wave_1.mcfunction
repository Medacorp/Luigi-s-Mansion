...
data modify storage luigis_mansion:data entity set value {mansion:{namespace:"3ds_remake",id:"hidden"},scan_message:{sender:"me",message:'{"type":"translatable","translate":"luigis_mansion:message.sir_weston.scan"}'},affected_by:["fire"]}
execute positioned 724 2 9 rotated -135 0 run function luigis_mansion:spawn_entities/portrait_ghost/sir_weston
execute unless data storage luigis_mansion:data current_state.current_data{money_spawned:["cold_storage_speedy_spirit"]} run data modify storage luigis_mansion:data entity set value {loot:{name:"cold_Storage_speedy_spirit"},tags:["hiding_in_furniture"]}
