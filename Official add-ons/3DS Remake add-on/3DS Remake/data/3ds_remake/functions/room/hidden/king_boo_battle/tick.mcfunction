...
execute as @a[gamemode=!spectator,x=465,y=23,z=-40,dx=49,dy=61,dz=85] run scoreboard players set @s LastRoom 76
execute as @e[x=465,y=23,z=-40,dx=49,dy=61,dz=85] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 76
...
execute if score #king_boo_battle ReturnTimer matches 100.. as @a[gamemode=!spectator,scores={Room=76}] run function 3ds_remake:room/hidden/king_boo_battle/clear_secret_altar
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=76},limit=1] run function 3ds_remake:room/hidden/king_boo_battle/ghosts