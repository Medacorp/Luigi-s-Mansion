...
execute if data storage luigis_mansion:data current_state.current_data.rooms.astral_hall{seen:0b} unless entity @s[gamemode=spectator] run function 3ds_remake:room/hidden/astral_hall/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function 3ds_remake:room/hidden/astral_hall/wall_warp
