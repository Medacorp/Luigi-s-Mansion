...
execute if data storage luigis_mansion:data current_state.current_data.rooms.hallway_17{seen:0b} unless entity @s[gamemode=spectator] run function 3ds_remake:room/hidden/hallway_17/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function 3ds_remake:room/hidden/hallway_17/wall_warp

tag @s add already_ticked

execute store result score #temp Boos run data get storage luigis_mansion:data current_state.current_data.boo_counter
execute if entity @s[x=726.0,y=29,z=-15.0,dx=4,dy=7,dz=7,tag=!spectator] unless score #temp Boos matches 20.. unless entity @e[tag=mysterious_power,limit=1] run data modify storage luigis_mansion:data entity set value {mansion:{namespace:"3ds_remake",id:"hidden"}}
