...
execute if data storage luigis_mansion:data current_state.current_data.rooms.hallway_17{seen:0b} unless entity @s[tag=spectator] run function 3ds_remake:room/hidden/hallway_17/set_seen
...
execute if entity @s[x=726.0,y=29,z=-15.0,dx=4,dy=7,dz=7,tag=!spectator] unless score #temp Boos matches 20.. unless entity @e[tag=mysterious_power,limit=1] run data modify storage luigis_mansion:data entity set value {room:52,mansion:{namespace:"3ds_remake",id:"hidden"}}
