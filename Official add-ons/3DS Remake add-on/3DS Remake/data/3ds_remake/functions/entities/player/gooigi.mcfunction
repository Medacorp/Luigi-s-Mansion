...
execute if entity @s[scores={Walk=0,Run=0,Sneak=0,Jump=0,IdleTime=0..},tag=!sneak_pos,tag=!spectator,tag=player,tag=!warp,tag=!scanning,tag=!poltergust_selected,tag=!riding_poltergust] run function 3ds_remake:entities/player/gooigi_idle
...
execute if entity @s[scores={Animation=1..},tag=!looking_at_map,tag=!idle,tag=!animation_may_move] run function luigis_mansion:entities/player/animation/freeze_player
execute if entity @s[scores={IdleTime=..-1}] run function 3ds_remake:entities/player/gooigi_idle
