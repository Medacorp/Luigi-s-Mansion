execute store result score #temp Time run data get storage luigis_mansion:data current_state.luigis_mansion.current_data.lowest_difficulty
execute store result storage luigis_mansion:data current_state.luigis_mansion.current_data.lowest_difficulty int 1 run scoreboard players operation #temp Time < #global_difficulty Selected
...
# Remove wind warp and furniture dust
execute as @e[tag=furniture,tag=!no_dust] if data entity @s data.scan_result.x run data modify entity @s data.scan_result set value ""
tag @e[tag=furniture,tag=!no_dust] add no_dust

function 3ds_remake:room/portrait_battle/tick with storage luigis_mansion:data current_state.luigis_mansion.current_data.mansion_id

execute unless entity @e[tag=portrait_ghost,tag=same_room,limit=1] run function luigis_mansion:room/ghost_portrificationizer_room/portrificationize with storage luigis_mansion:data current_state.luigis_mansion.current_data.mansion_id
...
execute if entity @s[tag=spectator] run function 3ds_remake:selection_menu/gallery_portrait/end_portrait_battle with storage luigis_mansion:data current_state.luigis_mansion.current_data.portrait_battle.position
