...
execute unless data storage luigis_mansion:data current_state.current_data.rooms.safari_room{cleared:1b} if data storage luigis_mansion:data current_state.current_data{blackout:0b} unless score #safari_room Wave matches 1..99 if entity @e[tag=deer_head,tag=lit,scores={Room=50}] run function 3ds_remake:room/hidden/safari_room/wave_1
