...
execute unless score #tea_room_drawer_2 Searched matches 1 unless data storage luigis_mansion:data current_state.current_data{money_spawned:["tea_room_giant_cabinet_2"]} run data modify storage luigis_mansion:data furniture.loot set value {name:"tea_room_giant_cabinet_2",contents:{luigis_mansion:{red_ruby:1}}}
