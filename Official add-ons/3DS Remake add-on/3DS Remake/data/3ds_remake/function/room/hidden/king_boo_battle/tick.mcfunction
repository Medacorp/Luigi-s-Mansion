...
execute if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.secret_altar{cleared:0b} as @a unless entity @s[scores={Room=76}] run function 3ds_remake:room/hidden/king_boo_battle/warp_to

execute if entity @e[tag=luigi,tag=!door_animation,scores={Room=76},limit=1] run function 3ds_remake:room/hidden/king_boo_battle/ghosts