data modify storage luigis_mansion:data entity set value {room:25,room_search:{non_players:1b},scan_result:{namespace:"luigis_mansion",id:"scan/entity/melody_pianissima"}}
...
execute unless data storage luigis_mansion:data current_state.current_data{money_spawned:["conservatory_speedy_spirit"]} run data modify storage luigis_mansion:data entity set value {room:25,loot:{drop_at_0:{name:"conservatory_speedy_spirit"}},spawn:3b}
