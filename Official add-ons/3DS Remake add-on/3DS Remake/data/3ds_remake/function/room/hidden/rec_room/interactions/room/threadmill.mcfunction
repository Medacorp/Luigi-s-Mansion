execute if entity @e[tag=luigi,x=643.5,y=12,z=-31.5,distance=..0.7,limit=1] run scoreboard players add #rec_room_threadmill Run 1
execute as @e[tag=luigi,x=643.5,y=12,z=-31.5,distance=..0.7,scores={TeleportDelayTimer=0},limit=1] run function luigis_mansion:entities/luigi/move/teleport {teleport:"~ ~ ~-0.2"}
execute unless entity @e[tag=luigi,x=643.5,y=12,z=-31.3,distance=..0.7,limit=1] run scoreboard players reset #rec_room_threadmill Run
