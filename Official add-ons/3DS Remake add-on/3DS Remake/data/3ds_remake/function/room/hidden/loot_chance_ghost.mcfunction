...
execute if score #temp Time matches 1..15 unless entity @a[tag=killer,limit=1,scores={GhostCount=2..}] if entity @s[tag=!portrait_ghost,nbt=!{data:{entity:{namespace:"luigis_mansion",id:"boo"}}}] unless data entity @s data.loot run data modify entity @s data.loot.contents.luigis_mansion.poison_mushroom set value 1
