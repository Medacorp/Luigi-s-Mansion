...
execute unless entity @s[tag=!walking,tag=!riding_poltergust] if entity @s[scores={Sound=0,Shrunk=0}] run playsound 3ds_remake:entity.gooigi.walk player @a[tag=same_room] ~ ~ ~ 0.5
execute unless entity @s[tag=!walking,tag=!riding_poltergust] if entity @s[scores={Sound=0,Shrunk=1..}] run playsound 3ds_remake:entity.gooigi.walk player @a[tag=same_room] ~ ~ ~ 0.5 2
execute unless entity @s[tag=!walking,tag=!riding_poltergust] run scoreboard players set @s[scores={Sound=0}] Sound 5

execute if entity @s[tag=!spectator] run function 3ds_remake:entities/player/gooigi_idle
execute if entity @s[scores={AnimationProgress=1..},tag=idle] unless entity @s[tag=!walking,tag=!spectator] run function luigis_mansion:entities/player/animation/set/none
