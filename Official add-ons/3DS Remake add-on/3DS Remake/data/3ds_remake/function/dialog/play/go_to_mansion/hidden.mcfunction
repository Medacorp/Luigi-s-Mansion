...
execute if score #dialog Dialog matches 1 unless data storage luigis_mansion:data current_state.luigis_mansion.current_data.mansion_id{namespace:"3ds_remake",id:"hidden"} if entity @a[scores={Room=1..},tag=!spectator,limit=1] run data modify storage luigis_mansion:data dialogs append value {name:{namespace:"luigis_mansion",id:"go_to_mansion/failure"},progress:0,room:-1}
execute if score #dialog Dialog matches 1 unless data storage luigis_mansion:data current_state.luigis_mansion.current_data.mansion_id{namespace:"3ds_remake",id:"hidden"} if entity @a[scores={Room=1..},tag=!spectator,limit=1] run scoreboard players set #dialog Dialog -1
