...
execute at @s[tag=!furniture_scanned,tag=!entity_scanned,tag=!scanned_block] if score #temp Move matches 1001 if entity @s[y_rotation=87..93,x_rotation=-23..-16] run tellraw @a[tag=same_room] {"type":"translatable","translate":"chat.type.text","with":[{"type":"selector","selector":"@p[tag=scanning_player]","color":"green"},{"type":"translatable","translate":"3ds_remake:message.player.scan_furniture.11"}]}
execute at @s[tag=!furniture_scanned,tag=!entity_scanned,tag=!scanned_block] if score #temp Move matches 1001 if entity @s[y_rotation=87..93,x_rotation=-23..-16] run advancement grant @p[tag=scanning_player] only luigis_mansion:vanilla/to_infinity_and_beyond
execute at @s[tag=!furniture_scanned,tag=!entity_scanned,tag=!scanned_block] if block ~ ~ ~ minecraft:air positioned ^ ^ ^0.1 if score #temp Move matches ..1000 run function 3ds_remake:selection_menu/game_boy_horror/scan/target_gooigi