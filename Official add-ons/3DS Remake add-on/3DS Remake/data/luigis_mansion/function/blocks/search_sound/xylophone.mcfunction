...
execute if data storage luigis_mansion:data my_memory.animation{namespace:"luigis_mansion",id:"search/hump"} if entity @a[tag=searcher,scores={InteractionTime=7},limit=1] run playsound luigis_mansion:furniture.search.xylophone block @a[tag=same_room] ~ ~ ~ 1 1
