execute unless entity @a[tag=portrait_battle,limit=1] run data modify storage luigis_mansion:data drop_loot set from entity @s ArmorItems[3].components."minecraft:custom_data".loot
...
execute unless data storage luigis_mansion:data drop_loot if data entity @s ArmorItems[3].components."minecraft:custom_data".loot if score #debug_messages Selected matches 1.. run tellraw @a {"type":"translatable","translate":"luigis_mansion:message.debug.format","with":[{"type":"translatable","translate":"luigis_mansion:message.debug","color":"gold"},{"type":"translatable","translate":"luigis_mansion:message.debug.loot_spawn_fail.previously_spawned","color":"red"}]}
