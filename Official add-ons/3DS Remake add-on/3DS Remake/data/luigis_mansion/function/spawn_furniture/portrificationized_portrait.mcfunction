...
$execute if data storage luigis_mansion:data current_state.luigis_mansion.current_data.portrait_ghosts[{name:{namespace:"$(namespace)",id:"$(id)"},rank:3b}] as @e[tag=this_portrait,limit=1] run data modify entity @s item.components."minecraft:custom_model_data".strings[0] set value "portrificationizing_platinum"
