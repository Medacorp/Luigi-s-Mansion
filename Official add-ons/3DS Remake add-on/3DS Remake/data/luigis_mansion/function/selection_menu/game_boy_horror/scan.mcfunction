...
execute if entity @s[tag=gooigi] if entity @e[tag=interact,tag=!furniture_scanned,tag=!entity_scanned,tag=!scanned_block,limit=1] if score #temp Move matches ..200 run tellraw @a {"type":"translatable","translate":"chat.type.text","with":[{"type":"selector","selector":"@s","color":"green"},{"type":"translatable","translate":"3ds_remake:message.player.scan_furniture.11"}]}
