...
execute if entity @e[tag=e_gadd,scores={Room=-1},tag=talk_this_tick] if data storage luigis_mansion:data rooms.underground_lab{cleared:1b} unless data storage 3ds_remake:data {shown_gameboy_horror_part:1b,spoke_with_future_e_gadd:0b} unless data storage luigis_mansion:data dialogs[{room:-1}] run data modify storage luigis_mansion:data dialogs append value {name:{namespace:"luigis_mansion",id:"underground_lab"},room:-1,progress:0}
