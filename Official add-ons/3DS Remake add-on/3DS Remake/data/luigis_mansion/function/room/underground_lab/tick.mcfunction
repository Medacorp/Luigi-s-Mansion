...
execute if data storage luigis_mansion:data rooms.underground_lab{cleared:1b} if data storage 3ds_remake:data {shown_gameboy_horror_part:1b,spoke_with_future_e_gadd:0b} if entity @a[scores={Room=-1},tag=!spectator] unless data storage luigis_mansion:data dialogs[{room:-1}] run data modify storage luigis_mansion:data dialogs append value {name:{namespace:"3ds_remake",id:"future_e_gadd"},room:-1,progress:0}
