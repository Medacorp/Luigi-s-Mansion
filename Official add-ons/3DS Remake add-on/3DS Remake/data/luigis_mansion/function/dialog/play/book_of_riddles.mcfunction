...
execute if score #dialog Dialog matches 7 if entity @a[tag=next_dialog_line,limit=1] run data modify storage 3ds_remake:data current_state.trophy.normal.read_books.book_of_riddles set value 1b
execute if score #dialog Dialog matches 7 if entity @a[tag=next_dialog_line,limit=1] if data storage 3ds_remake:data current_state.trophy.normal.read_books{book_of_riddles:1b,darkness_is_their_cheese:1b,e_gadds_guide_to_ghosts:1b,lydias_child_care_diary:1b,moldy_old_journal:1b,nevilles_big_baby_care_diary:1b,the_book_of_pericles:1b} run function 3ds_remake:selection_menu/gallery_trophy/complete/normal/task_8
