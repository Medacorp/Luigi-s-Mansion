...
execute at @s[scores={DeathTime=2,Health=-2147483648..},tag=!dead,tag=!remove_from_existence] if entity @a[tag=killer,limit=1,scores={GhostCount=2..}] if data storage 3ds_remake:data current_state.trophy.easy{task_4:0b} run function 3ds_remake:selection_menu/gallery_trophy/complete/easy/task_4
...
execute if entity @s[tag=captured] run tag @a[tag=killer,limit=1] add capturing_ghost
