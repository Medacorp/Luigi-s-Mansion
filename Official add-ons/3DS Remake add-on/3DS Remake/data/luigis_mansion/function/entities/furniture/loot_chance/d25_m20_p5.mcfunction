...
execute if score #temp Time matches 26..45 run data modify storage luigis_mansion:data furniture.loot set value {contents:{luigis_mansion:{small_heart:[{tags:["random_turn_on_bounce"]}]}}}
execute if score #temp Time matches 46..50 run data modify storage luigis_mansion:data furniture.loot set value {contents:{luigis_mansion:{poison_mushroom:1}}}
