...
execute if entity @s[scores={Dialog=1..99}] unless data storage luigis_mansion:data current_state.current_data.portrait_battle as @e[tag=luigi,tag=same_room,nbt=!{data:{animation:{}}}] run function luigis_mansion:entities/luigi/animation/set/high_health_idle_no_sound
