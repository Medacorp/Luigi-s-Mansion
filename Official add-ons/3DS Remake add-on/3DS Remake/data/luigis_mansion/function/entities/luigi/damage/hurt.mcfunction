...
execute unless score #global_difficulty Selected matches 0 if entity @s[scores={Room=1..},tag=!gooigi] run function luigis_mansion:entities/luigi/damage/drop_item with storage luigis_mansion:data current_state.luigis_mansion.current_data.drop_item_on_damage
