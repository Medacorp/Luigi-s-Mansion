...
$execute if entity @s[scores={Dialog=1}] unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"bowser_head"},owner:$(UUID)}},tag=same_room,limit=1] positioned ^ ^3.217 ^-0.533 run function luigis_mansion:spawn_entities/bowser/head
...
execute if entity @s[scores={Dialog=20}] unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"king_boo"}}},tag=same_room,limit=1] positioned ^ ^3.217 ^0.533 run function luigis_mansion:spawn_entities/portrait_ghost/king_boo
tag @s[scores={Dialog=60}] remove decapitate
data modify entity @s[scores={Dialog=60}] data.animation set value {namespace:"luigis_mansion",id:"decapitated"}
