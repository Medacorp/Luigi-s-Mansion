...
execute if data storage luigis_mansion:data current_state.current_data{boo_counter:4} if score #mansion_data_index Selected matches 1 run tag @a[tag=looking_at_map,nbt={Dimension:"luigis_mansion:normal"},limit=1] add regenrate_map
execute if data storage luigis_mansion:data current_state.current_data{boo_counter:4} if score #mansion_data_index Selected matches 0 run tag @a[tag=looking_at_map,nbt={Dimension:"luigis_mansion:hidden"},limit=1] add regenrate_map
...
execute if entity @s[nbt=!{ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"boolossus"}}}}]}] unless entity @a[scores={GBHCall=1..},limit=1] run scoreboard players add @a GBHCall 5