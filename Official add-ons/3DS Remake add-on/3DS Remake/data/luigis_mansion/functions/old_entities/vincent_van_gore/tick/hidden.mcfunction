...
execute if entity @s[scores={Dialog=333}] at @e[tag=same_room,tag=!spectator,tag=player] positioned ^ ^ ^8 if entity @s[distance=..8] run function luigis_mansion:old_entities/ghost/turn_invisible
...
execute if entity @s[scores={Dialog=332}] if score #mansion_data_index Selected matches 1 run function 3ds_remake:room/hidden/the_artists_studio/wave_2 
