execute unless entity @a[tag=portrait_battle,limit=1] run data modify storage luigis_mansion:data drop_loot set from entity @s ArmorItems[3].tag.luigis_mansion.loot
...
data remove entity @s ArmorItems[3].tag.luigis_mansion.loot
execute unless data storage luigis_mansion:data drop_loot if data entity @s ArmorItems[3].tag.luigis_mansion.loot if score #debug_messages Selected matches 1.. run tellraw @a {"translate":"luigis_mansion:message.debug.format","with":[{"translate":"luigis_mansion:message.debug","color":"gold"},{"translate":"luigis_mansion:message.debug.loot_spawn_fail.previously_spawned","color":"red"}]}
