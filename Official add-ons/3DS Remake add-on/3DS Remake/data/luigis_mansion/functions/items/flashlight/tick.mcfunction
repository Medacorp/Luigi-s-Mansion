...
execute if entity @s[scores={Animation=1..},tag=!idle,nbt={Inventory:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"flashlight"}}}]}] unless entity @s[scores={ScareType=1}] unless entity @s[scores={KnockbackType=1..2}] run function luigis_mansion:items/flashlight/effects/turn_off_for_animation
