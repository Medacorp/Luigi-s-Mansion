...
execute if entity @s[scores={Animation=1..},tag=!idle,nbt={Inventory:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"flashlight"}}}]}] unless entity @s[scores={Animation=39}] unless entity @s[scores={Animation=27..28}] run function luigis_mansion:items/flashlight/effects/turn_off_for_animation
