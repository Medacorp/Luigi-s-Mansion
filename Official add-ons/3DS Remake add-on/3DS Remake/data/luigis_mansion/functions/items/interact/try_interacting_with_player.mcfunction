execute if entity @e[distance=..2,scores={DeathTime=0},limit=1] if entity @s[tag=!gooigi] run function luigis_mansion:entities/player/animation/set/talk
execute if entity @e[distance=..2,scores={DeathTime=0},limit=1] run tag @s[tag=!gooigi] add talk
...
execute if score #can_revive Selected matches 1 as @e[distance=..2,scores={DeathTime=60..},limit=1] run function luigis_mansion:entities/player/memory/get with entity @s
