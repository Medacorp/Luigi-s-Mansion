$execute if data entity @s ArmorItems[3].tag.luigis_mansion{attacked_by:[$(id)]} run tag @s add already_included

execute if entity @s[tag=!already_included] unless data entity @s ArmorItems[3].id run item replace entity @s armor.head with minecraft:stone_button
execute if entity @s[tag=!already_included] unless data entity @s ArmorItems[3].tag.luigis_mansion.attacked_by run data modify entity @s ArmorItems[3].tag.luigis_mansion.attacked_by set value []
$execute if entity @s[tag=!already_included] run data modify entity @s ArmorItems[3].tag.luigis_mansion.attacked_by append value $(id)
