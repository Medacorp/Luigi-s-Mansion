...
execute if entity @s[scores={DeathTime=120},tag=!revived_by_polterpup] if score #can_revive Selected matches 1 run function luigis_mansion:entities/player/turn_into_spectator
scoreboard players set @s[scores={DeathTime=120},tag=!revived_by_polterpup,tag=!portrait_battle,tag=!gooigi] Offline -1
tag @s[scores={DeathTime=120},tag=!revived_by_polterpup] remove death_animation
scoreboard players reset @s[scores={DeathTime=120},tag=!revived_by_polterpup] DeathTime
