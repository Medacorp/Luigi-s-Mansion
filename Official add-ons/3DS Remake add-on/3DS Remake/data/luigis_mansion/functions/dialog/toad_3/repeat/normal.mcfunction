...
tag @s[scores={Dialog=72}] remove explain
...
execute if entity @s[scores={Dialog=75}] if score #toad_amiibo Selected matches 1 if score #players Totals matches 2.. run tellraw @a[tag=same_room] {"type":"translatable","translate":"chat.type.text","with":[{"type":"translatable","translate":"luigis_mansion:entity.toad","color":"green"},{"type":"translatable","translate":"3ds_remake:dialog.toad.heal.more"}]}
...
tag @s[tag=!explain] remove explain