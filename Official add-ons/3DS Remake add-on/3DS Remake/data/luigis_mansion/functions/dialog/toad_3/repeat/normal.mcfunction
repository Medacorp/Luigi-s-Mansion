...
execute if entity @s[scores={Dialog=1..169}] as @a[tag=same_room,gamemode=!spectator,tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/stand_still
...
execute if entity @s[scores={Dialog=75}] if score #toad_amiibo Selected matches 1 if score #players Totals matches 1 run tellraw @a[tag=same_room] {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.toad","color":"green"},{"translate":"3ds_remake:dialog.toad.heal","with":[{"selector":"@p[gamemode=!spectator]"}]}]}
execute if entity @s[scores={Dialog=75}] if score #toad_amiibo Selected matches 1 if score #players Totals matches 2.. run tellraw @a[tag=same_room] {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.toad","color":"green"},{"translate":"3ds_remake:dialog.toad.heal.more"}]}
execute if entity @s[scores={Dialog=170}] if score #toad_amiibo Selected matches 1 at @a[gamemode=!spectator,tag=!looking_at_map] run function 3ds_remake:spawn_entities/item/huge_heart
execute if entity @s[scores={Dialog=170}] if score #toad_amiibo Selected matches 1 at @e[tag=game_boy_horror_location] run function 3ds_remake:spawn_entities/item/huge_heart
