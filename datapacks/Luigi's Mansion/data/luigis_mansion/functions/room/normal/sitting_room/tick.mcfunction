...
execute as @a[gamemode=!spectator,x=734,y=17,z=-39,dx=14,dy=10,dz=20] unless entity @s[scores={Room=69}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=734,y=17,z=-39,dx=14,dy=10,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 69
