...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] unless data storage luigis_mansion:data current_state.current_data{money_spawned:["hallway_5_vase_1"]} run data modify storage luigis_mansion:data give_loot set value {name:"hallway_5_vase_1",contents:{luigis_mansion:{gold_coin:10}}}
