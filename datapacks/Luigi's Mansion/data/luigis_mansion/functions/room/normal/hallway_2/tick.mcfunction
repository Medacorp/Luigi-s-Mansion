...
execute as @a[gamemode=!spectator,x=722,y=17,z=42,dx=8,dy=10,dz=38] unless entity @s[scores={Room=7}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=722,y=17,z=42,dx=8,dy=10,dz=38] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 7
