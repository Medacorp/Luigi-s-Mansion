...
execute as @a[gamemode=!spectator,x=764,y=77,z=-15,dx=14,dy=6,dz=38] unless entity @s[scores={Room=-3}] run scoreboard players set @s LastRoom -3
execute as @e[tag=!spectator,x=764,y=77,z=-15,dx=14,dy=6,dz=38] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room -3
