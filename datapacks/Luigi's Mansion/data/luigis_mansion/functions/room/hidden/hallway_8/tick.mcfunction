...
execute as @a[gamemode=!spectator,x=659,y=8,z=51,dx=8,dy=19,dz=26] unless entity @s[scores={Room=33}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=659,y=8,z=51,dx=8,dy=19,dz=26] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 33
