...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] unless data storage luigis_mansion:data current_state.current_data{money_spawned:["hallway_9_jar_1"]} run data modify storage luigis_mansion:data give_loot set value {name:"hallway_9_jar_1",contents:{luigis_mansion:{bill:10}}}
