...
execute as @e[type=!minecraft:item_frame,x=736,y=28,z=-20,dx=24,dy=8,dz=35] unless entity @s[type=!minecraft:player,tag=!model_piece,scores={Room=-2147483648..}] run scoreboard players set @s Room 56
...
execute unless entity @a[tag=toad_5_dialog,limit=1] if entity @a[tag=blackout,limit=1] unless data storage luigis_mansion:data current_state.current_data.technical_data{telephone_1:1b} positioned 750.5 30 -1.5 run playsound luigis_mansion:block.telephone.ring ambient @a[scores={Room=56,RoomNoise=0}] ~ ~ ~ 1
