...
execute as @e[x=734,y=1,z=57,dx=14,dy=8,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 58
...
execute if entity @a[gamemode=!spectator,scores={Room=58},limit=1] run function luigis_mansion:room/hidden/cellar/ghosts
