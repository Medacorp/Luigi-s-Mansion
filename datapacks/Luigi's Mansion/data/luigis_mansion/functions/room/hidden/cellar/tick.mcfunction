...
execute as @a[gamemode=!spectator,x=734,y=-1,z=57,dx=14,dy=10,dz=21] unless entity @s[scores={Room=58}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=734,y=-1,z=57,dx=14,dy=10,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 58
