...
execute as @a[gamemode=!spectator,x=687,y=26,z=70,dx=16,dy=10,dz=25] unless entity @s[scores={Room=71}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=687,y=26,z=70,dx=16,dy=10,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 71
...
execute as @e[tag=!spectator,x=697.0,y=27,z=72.0,dx=5,dy=8,dz=21,tag=ghost,tag=hidden,y_rotation=-180..0] at @s run function luigis_mansion:room/hidden/the_artists_studio/stay_near_painting
