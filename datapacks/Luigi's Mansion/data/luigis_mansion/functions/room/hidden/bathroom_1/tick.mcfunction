...
execute as @a[gamemode=!spectator,x=648,y=8,z=-14,dx=12,dy=10,dz=14] unless entity @s[scores={Room=16}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=648,y=8,z=-14,dx=12,dy=10,dz=14] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 16
...
execute if entity @a[tag=same_room,limit=1] run function luigis_mansion:room/hidden/bathroom_1/ghosts
