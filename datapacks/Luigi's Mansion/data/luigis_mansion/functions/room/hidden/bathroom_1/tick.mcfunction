...
execute as @e[tag=!spectator,x=648,y=10,z=-14,dx=12,dy=8,dz=14] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 16
...
execute if entity @a[gamemode=!spectator,scores={Room=16}] run function luigis_mansion:room/hidden/bathroom_1/ghosts