...
execute as @e[tag=!spectator,x=724,y=28,z=24,dx=16,dy=8,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 52
...
execute if entity @a[gamemode=!spectator,scores={Room=52},limit=1] run function luigis_mansion:room/hidden/hallway_17/ghosts