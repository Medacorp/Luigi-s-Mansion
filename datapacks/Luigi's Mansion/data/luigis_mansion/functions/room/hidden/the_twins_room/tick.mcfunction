...
execute as @e[tag=!spectator,x=736,y=19,z=-35,dx=14,dy=8,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 49
...
execute if entity @a[gamemode=!spectator,scores={Room=49},limit=1] run function luigis_mansion:room/hidden/the_twins_room/ghosts