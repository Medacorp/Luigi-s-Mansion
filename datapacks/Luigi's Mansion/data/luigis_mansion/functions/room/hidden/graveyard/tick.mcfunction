...
execute as @e[x=644,y=101,z=-40,dx=27,dy=8,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 29
...
execute if entity @a[gamemode=!spectator,scores={Room=29},limit=1] run function luigis_mansion:room/hidden/graveyard/ghosts