...
execute as @e[x=682,y=110,z=-42,dx=17,dy=8,dz=20] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 9
...
execute if entity @a[gamemode=!spectator,scores={Room=9},limit=1] run function luigis_mansion:room/hidden/master_bedroom/ghosts
