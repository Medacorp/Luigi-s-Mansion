...
execute as @e[type=!minecraft:item_frame,x=699,y=1,z=29,dx=13,dy=8,dz=8] unless entity @s[type=!minecraft:player,tag=!model_piece,scores={Room=-2147483648..}] run scoreboard players set @s Room 60
tag @e[tag=ghost,scores={Room=60}] add no_hidden_move

execute as @a[gamemode=!spectator,scores={Room=60}] run function luigis_mansion:room/normal/hallway_21/tick_per_player
