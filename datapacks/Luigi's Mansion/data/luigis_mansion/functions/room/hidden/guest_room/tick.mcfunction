...
execute as @e[tag=!spectator,x=734,y=19,z=57,dx=14,dy=8,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 70
...
execute if entity @a[gamemode=!spectator,scores={Room=70}] run function luigis_mansion:room/hidden/guest_room/ghosts
