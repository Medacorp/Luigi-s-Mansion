...
execute as @e[tag=!spectator,x=653,y=118,z=-23,dx=30,dy=24,dz=80] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 53
...
execute if entity @a[gamemode=!spectator,scores={Room=53},limit=1] run function luigis_mansion:room/hidden/balcony_2/ghosts
