...
execute as @e[type=!minecraft:item_frame,x=736,y=19,z=-35,dx=14,dy=8,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 49

execute as @a[gamemode=!spectator,scores={Room=49}] run function luigis_mansion:room/hidden/twins_room/tick_per_player
