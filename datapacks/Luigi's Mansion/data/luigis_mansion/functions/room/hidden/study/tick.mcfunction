...
execute as @e[tag=!spectator,x=700,y=19,z=-48,dx=17,dy=8,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 8
...
execute if entity @a[gamemode=!spectator,scores={Room=8},limit=1] run function luigis_mansion:room/hidden/study/ghosts
