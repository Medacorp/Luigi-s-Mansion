...
execute as @a[gamemode=!spectator,x=740,y=28,z=-33,dx=23,dy=8,dz=8] unless entity @s[scores={Room=55}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=740,y=28,z=-33,dx=23,dy=8,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 55
tag @e[tag=ghost,scores={Room=55}] add no_hidden_move

execute as @a[gamemode=!spectator,scores={Room=55}] run function luigis_mansion:room/hidden/hallway_19/tick_per_player
