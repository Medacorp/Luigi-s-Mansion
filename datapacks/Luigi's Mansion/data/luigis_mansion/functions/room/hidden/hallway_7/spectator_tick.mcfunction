scoreboard players set @s Room 15
scoreboard players set @s PrevRoom 15

teleport @s[scores={HomeY=11..15,HomeX=719}] ~17 ~ ~
teleport @s[scores={HomeY=7..10,HomeX=719}] ~-1 ~ ~
teleport @s[scores={HomeY=2..6,HomeX=719}] ~17 ~ ~
teleport @s[scores={HomeY=11..15,HomeX=713}] ~-3 ~ ~7
teleport @s[scores={HomeY=7..10,HomeX=713}] ~1 ~ ~
teleport @s[scores={HomeY=2..6,HomeX=713}] ~-15 ~ ~
teleport @s[scores={HomeY=11..15,HomeZ=58}] ~ ~ ~-10
teleport @s[scores={HomeY=7..10,HomeZ=67}] ~ ~ ~1
teleport @s[scores={HomeY=2..6,HomeZ=58}] ~ ~ ~-23
teleport @s[scores={HomeZ=77}] ~ ~ ~-1

teleport @s[scores={HomeY=16}] ~ ~3 ~-8
teleport @s[scores={HomeY=9,HomeZ=59..67}] ~ ~-3 ~
teleport @s[scores={HomeY=7,HomeZ=59..67}] ~ ~3 ~
teleport @s[scores={HomeY=0}] ~ ~1 ~ 

tag @s add already_ticked