...
execute as @e[tag=!spectator,x=736,y=10,z=-35,dx=14,dy=8,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 24
...
execute if entity @a[gamemode=!spectator,scores={Room=24}] run function luigis_mansion:room/hidden/hidden_room/ghosts
