...
execute as @e[x=732,y=28,z=49,dx=14,dy=8,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 50
...
execute if entity @a[gamemode=!spectator,scores={Room=50},limit=1] run function luigis_mansion:room/hidden/safari_room/ghosts
