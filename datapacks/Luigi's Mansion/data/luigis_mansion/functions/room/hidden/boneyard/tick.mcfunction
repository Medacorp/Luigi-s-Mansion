...
execute as @a[gamemode=!spectator,x=671,y=99,z=-41,dx=14,dy=10,dz=19] unless entity @s[scores={Room=28}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=671,y=99,z=-41,dx=14,dy=10,dz=19] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 28
