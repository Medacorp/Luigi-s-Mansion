...
execute as @e[x=743,y=10,z=-4,dx=15,dy=17,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 1
...
execute if entity @a[gamemode=!spectator,scores={Room=1},limit=1] run function luigis_mansion:room/hidden/foyer/ghosts
