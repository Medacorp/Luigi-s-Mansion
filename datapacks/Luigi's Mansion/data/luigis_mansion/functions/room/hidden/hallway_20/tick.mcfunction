...
execute as @e[tag=!spectator,x=713,y=1,z=29,dx=22,dy=8,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 59
...
execute if entity @a[gamemode=!spectator,scores={Room=59},limit=1] run function luigis_mansion:room/hidden/hallway_20/ghosts