...
execute as @e[tag=!spectator,x=697,y=19,z=30,dx=15,dy=8,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 38
...
execute if entity @a[gamemode=!spectator,scores={Room=38},limit=1] run function luigis_mansion:room/hidden/hallway_12/ghosts
