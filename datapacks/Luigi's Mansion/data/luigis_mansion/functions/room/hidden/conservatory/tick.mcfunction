...
execute as @e[tag=!spectator,x=652,y=10,z=16,dx=12,dy=8,dz=28] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 25
...
execute if entity @a[gamemode=!spectator,scores={Room=25},limit=1] run function luigis_mansion:room/hidden/conservatory/ghosts