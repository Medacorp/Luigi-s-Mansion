...
execute as @e[x=673,y=10,z=-31,dx=15,dy=8,dz=16] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 48
...
execute if entity @a[gamemode=!spectator,scores={Room=48},limit=1] run function luigis_mansion:room/hidden/projection_room/ghosts
