...
execute as @a[gamemode=!spectator,x=686,y=-1,z=3,dx=14,dy=10,dz=18] unless entity @s[scores={Room=67}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=686,y=-1,z=3,dx=14,dy=10,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 67
