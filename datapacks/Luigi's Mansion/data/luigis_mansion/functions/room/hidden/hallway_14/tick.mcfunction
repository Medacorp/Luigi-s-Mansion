...
execute as @e[tag=!spectator,x=712,y=19,z=58,dx=8,dy=17,dz=12] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 40
...
execute if entity @a[gamemode=!spectator,scores={Room=40},limit=1] run function luigis_mansion:room/hidden/hallway_14/ghosts