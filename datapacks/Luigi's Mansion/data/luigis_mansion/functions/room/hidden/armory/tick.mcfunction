...
execute as @e[tag=!spectator,x=744,y=28,z=-64,dx=22,dy=8,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 64
...
execute if entity @a[gamemode=!spectator,scores={Room=64},limit=1] run function luigis_mansion:room/hidden/armory/ghosts