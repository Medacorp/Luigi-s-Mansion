...
execute if entity @s[scores={WarpTime=10}] run playsound luigis_mansion:item.gameboy_horror.warp player @a[tag=same_room] ~ ~ ~ 1
tag @s[scores={WarpTime=50..}] remove grabbed
execute if entity @s[scores={WarpTime=95..129}] run function #luigis_mansion:items/gameboy_horror/warp_location
