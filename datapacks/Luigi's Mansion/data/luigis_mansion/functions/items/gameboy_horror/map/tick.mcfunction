...
execute unless entity @s[scores={OpenMapFocus=1..}] unless entity @s[scores={Walk=0..2,Run=0..2,Sneak=0}] run playsound luigis_mansion:item.gameboy_horror.map.move player @s[scores={Sound=0}] ~ ~ ~ 1
execute unless entity @s[scores={OpenMapFocus=1..}] unless entity @s[scores={Walk=0..2,Run=0..2,Sneak=0}] run scoreboard players set @s[scores={Sound=0}] Sound 7
