...
execute if entity @s[tag=dying,scores={DeathTime=1}] run playsound luigis_mansion:entity.nana.vacuumed hostile @a[tag=same_room] ~ ~ ~ 1
...
execute if entity @e[tag=same_room,tag=!spectator,distance=..1.6,limit=1] if entity @s[tag=!vanish,tag=!dying,tag=!dead,tag=!removed_from_existence,tag=!hurt,tag=!fleeing] run function luigis_mansion:entities/nana/collide
