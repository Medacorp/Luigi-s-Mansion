scoreboard players add @s[scores={Dialog=22..},tag=!vanish] Dialog 1
...
execute positioned ~ ~-1.1875 ~ if entity @e[type=minecraft:armor_stand,tag=billiards_ball,tag=spit,distance=..0.7] run scoreboard players add @s[scores={Dialog=11}] Dialog 1
...
execute if entity @s[scores={Dialog=11}] run function luigis_mansion:entities/slim_bankshot/tick/hidden/path
...
scoreboard players set @s[scores={Dialog=30}] VulnerableTime 70
