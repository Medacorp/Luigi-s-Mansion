...
execute at @s[scores={ActionTime=1..20}] positioned ^ ^ ^0.7 unless entity @e[tag=same_room,tag=!spectator,distance=..0.7,limit=1] run tag @s add stop_attack
...
execute at @s[scores={ActionTime=41}] as @a[distance=..1,tag=!spectator] run function luigis_mansion:entities/player/knockback/large
