execute if entity @s[tag=dying,tag=normal_death,scores={DeathTime=1}] run playsound luigis_mansion:entity.speedy_spirit.vacuumed hostile @a[tag=same_room] ~ ~ ~ 1
execute if entity @s[tag=dying,tag=element_death,scores={DeathTime=1}] run playsound luigis_mansion:entity.speedy_spirit.element_death hostile @a[tag=same_room] ~ ~ ~ 1
execute if entity @s[tag=dying,scores={DeathTime=1}] if data storage luigis_mansion:data current_state.current_data.ghosts.speedy_spirit{loot_at_0:1b} run function luigis_mansion:entities/speedy_spirit/drop_loot
execute if entity @s[tag=dead] unless data storage luigis_mansion:data current_state.current_data.ghosts.speedy_spirit{loot_at_0:1b} run function luigis_mansion:entities/speedy_spirit/drop_loot
...
execute if entity @s[tag=!element_hurt,tag=!hurt,tag=!fleeing,tag=!attack,tag=!laugh,tag=!vanish,tag=!appear,tag=basher,scores={StunTime=0}] positioned ^ ^ ^0.7 if entity @e[tag=same_room,tag=!spectator,distance=..0.7,limit=1] run tag @s add attack
...
execute at @s[tag=!hurt,tag=!element_hurt,tag=!fleeing,tag=!attack,tag=!vanish,tag=!laugh,scores={StunTime=0}] unless entity @s[tag=appear,tag=beta_appear] facing entity @e[tag=same_room,tag=!spectator,sort=nearest,limit=1] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
