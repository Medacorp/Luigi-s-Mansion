...
execute at @s[tag=dead,scores={Owner=-2147483648..}] if entity @a[distance=..1,gamemode=!spectator,limit=1] as @e[tag=!model_piece,tag=ghost] if score @s GhostNr = #temp GhostNr run tag @s add laugh
execute at @s[tag=dead,scores={Owner=-2147483648..}] unless entity @a[distance=..1,gamemode=!spectator,limit=1] as @e[tag=!model_piece,tag=ghost] if score @s GhostNr = #temp GhostNr run tag @s add complain
