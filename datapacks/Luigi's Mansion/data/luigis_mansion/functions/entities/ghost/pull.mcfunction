...
execute as @a[tag=!spectator,tag=vacuuming_this_ghost,distance=3..] run tag @s add target
execute as @a[tag=!spectator,tag=vacuuming_this_ghost,distance=..3] run scoreboard players set @s Pull 0
...
execute if entity @s[scores={Pull=220}] run tag @a[tag=!spectator,tag=vacuuming_this_ghost] add poltergust_malfunction
