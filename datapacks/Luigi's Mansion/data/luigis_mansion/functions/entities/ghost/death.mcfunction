...
execute if entity @s[scores={HurtTime=1}] run scoreboard players operation @s KillerID = @p[tag=!spectator,tag=vacuuming_this_ghost] ID
...
execute at @s[scores={HurtTime=2}] if entity @a[tag=!spectator,distance=..0.7] run tag @s add dead
tag @a remove killer
