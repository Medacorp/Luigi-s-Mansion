...
execute at @s[scores={DeathTime=1},tag=!dont_drop_heart] unless entity @s[scores={Room=..-1}] unless entity @s[scores={Health=-2147483648..}] if entity @a[tag=killer,limit=1,scores={GhostCount=2..}] if score #heart_coin_count Selected matches 1.. run function luigis_mansion:spawn_entities/item/small_heart
execute at @s[scores={DeathTime=1,Room=1..}] run function luigis_mansion:entities/ghost/capture
execute at @s[scores={DeathTime=1..,Room=-2}] if entity @a[tag=!spectator,distance=..0.7] run scoreboard players add #training_room GhostCaught 1
execute at @s[scores={DeathTime=1..}] if entity @a[tag=!spectator,distance=..0.7] run tag @s add captured
