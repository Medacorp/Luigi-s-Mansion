execute if entity @s[tag=dying,tag=normal_death,scores={DeathTime=1}] run playsound luigis_mansion:entity.purple_bomber.vacuumed hostile @a[tag=same_room] ~ ~ ~ 1
execute if entity @s[tag=dying,tag=element_death,scores={DeathTime=1}] run playsound luigis_mansion:entity.purple_bomber.element_death hostile @a[tag=same_room] ~ ~ ~ 1
execute if entity @s[tag=dying,scores={DeathTime=1}] if data storage luigis_mansion:data current_state.current_data.ghosts.purple_bomber{loot_at_0:1b} run function luigis_mansion:entities/purple_bomber/drop_loot
execute if entity @s[tag=dead] unless data storage luigis_mansion:data current_state.current_data.ghosts.purple_bomber{loot_at_0:1b} run function luigis_mansion:entities/purple_bomber/drop_loot
