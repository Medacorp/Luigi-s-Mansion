...
execute if entity @s[tag=dying,scores={DeathTime=1}] run playsound luigis_mansion:entity.melody_pianissima.vacuumed hostile @a[tag=same_room] ~ ~ ~ 1
execute if entity @s[tag=dying,scores={DeathTime=1}] if data storage luigis_mansion:data current_state.current_data.portrait_ghosts.melody_pianissima{loot_at_0:1b} run function luigis_mansion:entities/melody_pianissima/drop_loot
...
execute if entity @s[scores={Sound=0},tag=fleeing,tag=!dying] run playsound luigis_mansion:entity.melody_pianissima.flee hostile @a[tag=same_room] ~ ~ ~ 1
