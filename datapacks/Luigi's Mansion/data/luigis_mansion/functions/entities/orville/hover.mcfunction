execute facing entity @e[tag=same_room,tag=!spectator,sort=nearest,limit=1] feet rotated ~ 0 run summon minecraft:marker ^-6 ^ ^ {Tags:["target"]}
...
execute if block ~ ~ ~ #luigis_mansion:ghosts_ignore unless block ~ ~1 ~ #luigis_mansion:ghosts_ignore run tag @s add at_height
