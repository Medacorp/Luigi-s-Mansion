...
execute as @e[tag=partner,limit=1] at @s if entity @e[tag=!me,tag=!partner,tag=!hidden,distance=..0.7] if block ~ ~ ~ #luigis_mansion:all_ignore run tag @s remove partner
execute as @e[tag=partner,limit=1] at @s if entity @e[tag=!me,tag=!partner,tag=!hidden,distance=..0.7] unless entity @e[type=minecraft:item_frame,distance=..0.7] run tag @s remove partner
