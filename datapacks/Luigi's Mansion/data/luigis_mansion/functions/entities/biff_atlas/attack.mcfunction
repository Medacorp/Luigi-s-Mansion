...
effect give @a[distance=..1.6,tag=!spectator] minecraft:instant_damage 1 0 true
scoreboard players set @a[distance=..1.6,tag=!spectator] ForcedDamage 14
execute as @a[distance=..1.6,tag=!spectator] run function luigis_mansion:entities/player/knockback/large
execute if entity @a[distance=..1.6,tag=!spectator] run tag @s add laugh
