scoreboard players add @s[scores={Dialog=162..},tag=!vanish] Dialog 1
...
execute if entity @s[scores={Dialog=121},tag=!angry,tag=!attack] run playsound luigis_mansion:entity.biff_atlas.complain hostile @a[tag=same_room] ~ ~ ~ 1
tag @s[scores={Dialog=121},tag=!angry,tag=!attack] add angry
execute if entity @s[scores={Dialog=121}] facing entity @e[tag=same_room,tag=!spectator,sort=nearest,limit=1] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
execute at @s[scores={Dialog=121},tag=!laugh] run function luigis_mansion:entities/ghost/move_forward
execute if entity @s[scores={Dialog=121},tag=!laugh] positioned ^ ^ ^0.7 if entity @e[tag=same_room,tag=!spectator,distance=..2,limit=1] run tag @s add attack
