...
data modify storage luigis_mansion:data current_state.my_money set value {uuid:[I;],total:0,money:{green_emerald:0}}
...
execute in minecraft:overworld run data modify block 27 0 0 Items[0].tag.value set from entity @s UUID
data modify storage luigis_mansion:data current_state.new_money_grabbed set value []
data modify storage luigis_mansion:data current_state.my_money_data set value {uuid:[I;],total:0,money:{green_emerald:0}}
...
execute store result score #temp ActionTime run data get storage luigis_mansion:data current_state.my_money_data.money.luigis_mansion.green_emerald
execute store result score #temp2 ActionTime run data get storage luigis_mansion:data current_state.my_money.money.luigis_mansion.green_emerald
...
scoreboard players operation #temp3 Money *= #800 Constants
scoreboard players operation #temp Money -= #temp3 Money
scoreboard players operation #temp4 Money = #temp2 ActionTime
scoreboard players operation #temp4 Money *= #800 Constants
