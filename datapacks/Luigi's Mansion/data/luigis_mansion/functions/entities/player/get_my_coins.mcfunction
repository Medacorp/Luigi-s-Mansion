...
data modify storage luigis_mansion:data current_state.my_money set value {uuid:[I;],total:0,money:{gold_coin:0}}
data modify storage luigis_mansion:data current_state.my_money.uuid set from entity @s UUID
execute in minecraft:overworld if data storage luigis_mansion:data current_state.money_grabbed[-1] run function luigis_mansion:entities/player/get_my_coins_index
...
execute in minecraft:overworld run data modify block 27 0 0 Items[0].tag.value set from entity @s UUID
data modify storage luigis_mansion:data current_state.new_money_grabbed set value []
data modify storage luigis_mansion:data current_state.my_money_data set value {uuid:[I;],total:0,money:{gold_coin:0}}
data modify storage luigis_mansion:data current_state.my_money_data.uuid set from entity @s UUID
execute in minecraft:overworld if data storage luigis_mansion:data current_state.current_data.money_grabbed[-1] run function luigis_mansion:entities/player/get_my_coins_index_mansion
scoreboard players reset #temp ActionTime
execute in minecraft:overworld run fill 27 0 0 26 0 0 minecraft:bedrock
...
execute store result score #temp ActionTime run data get storage luigis_mansion:data current_state.my_money_data.money.gold_coin
execute store result score #temp2 ActionTime run data get storage luigis_mansion:data current_state.my_money.money.gold_coin
