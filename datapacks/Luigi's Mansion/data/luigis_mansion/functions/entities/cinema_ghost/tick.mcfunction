execute if entity @s[tag=dying,tag=normal_death,scores={DeathTime=1}] run playsound luigis_mansion:entity.cinema_ghost.vacuumed hostile @a[tag=same_room] ~ ~ ~ 1
execute if entity @s[tag=dying,tag=element_death,scores={DeathTime=1}] run playsound luigis_mansion:entity.cinema_ghost.element_death hostile @a[tag=same_room] ~ ~ ~ 1
execute if entity @s[tag=dying,scores={DeathTime=1}] if data storage luigis_mansion:data current_state.current_data._ghosts.cinema_ghost{loot_at_0:1b} run function luigis_mansion:entities/cinema_ghost/drop_loot
execute if entity @s[tag=dead] unless data storage luigis_mansion:data current_state.current_data._ghosts.cinema_ghost{loot_at_0:1b} run function luigis_mansion:entities/cinema_ghost/drop_loot
...
execute if entity @e[tag=same_room,tag=!spectator,distance=..0.7,limit=1] unless entity @s[tag=!element_hurt,tag=!complain,tag=!laugh] if entity @s[tag=!vanish,tag=!appear,tag=!basher,tag=!attack,tag=!dying,tag=!dead,tag=!removed_from_existence,tag=!hurt,tag=!fleeing,tag=!element_hurt] run function luigis_mansion:entities/cinema_ghost/collide
execute if entity @s[tag=!visible,tag=!vanish,tag=!basher] unless entity @s[tag=!element_hurt,tag=!attack,tag=!collided,tag=!laugh,tag=!complain,scores={VulnerableTime=0}] run function luigis_mansion:entities/cinema_ghost/turn_visible
execute if entity @s[tag=visible,tag=!vanish,tag=!basher] if entity @s[tag=!element_hurt,tag=!attack,tag=!collided,tag=!laugh,tag=!complain,scores={VulnerableTime=0}] run function luigis_mansion:entities/cinema_ghost/turn_invisible

execute if entity @s[tag=!basher] run function luigis_mansion:entities/cinema_ghost/select_attack/grab
execute if entity @s[tag=basher] run function luigis_mansion:entities/cinema_ghost/select_attack/basher
