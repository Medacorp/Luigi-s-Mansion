execute if entity @s[tag=dying,tag=normal_death,scores={DeathTime=1}] run playsound luigis_mansion:entity.garbage_can_ghost.vacuumed hostile @a[tag=same_room] ~ ~ ~ 1
execute if entity @s[tag=dying,tag=element_death,scores={DeathTime=1}] run playsound luigis_mansion:entity.garbage_can_ghost.element_death hostile @a[tag=same_room] ~ ~ ~ 1
