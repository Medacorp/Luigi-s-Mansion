...
execute at @s[scores={Move=10}] run execute as @e[distance=..3,tag=gameboy_horror_location] run function luigis_mansion:entities/gameboy_horror_location/bring_player_back
execute at @s[scores={Move=10}] run effect give @a[distance=..3,tag=!spectator] minecraft:instant_damage 1 0 true
execute at @s[scores={Move=10}] run scoreboard players set @a[distance=..3,tag=!spectator] ForcedDamage 4
