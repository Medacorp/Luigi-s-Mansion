...
execute if entity @s[scores={Dialog=1..217}] as @a[tag=same_room,gamemode=!spectator,scores={IdleTime=0..},tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/stand_still
...
execute if entity @s[scores={Dialog=1..218}] as @a[tag=same_room] unless entity @s[scores={MusicGroup=0,MusicType=23}] run function luigis_mansion:other/music/set/talking_ghost
...
execute if entity @s[scores={Dialog=154}] run tellraw @a[tag=same_room] {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.madame_clairvoya","color":"green"},{"translate":"luigis_mansion:dialog.madame_clairvoya.star.1"}]}
tag @s[scores={Dialog=218}] remove nod
tag @s[scores={Dialog=218}] add next
scoreboard players set @s[scores={Dialog=218}] AnimationProg 0
