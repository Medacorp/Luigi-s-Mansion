...
execute if entity @s[scores={Dialog=1..362}] as @a[tag=same_room] unless entity @s[scores={MusicGroup=0,MusicType=23}] run function luigis_mansion:other/music/set/talking_ghost
...
execute if entity @s[scores={Dialog=154}] run tellraw @a[tag=same_room] {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.madame_clairvoya","color":"green"},{"translate":"luigis_mansion:dialog.madame_clairvoya.hat.1"}]}
tag @s[scores={Dialog=362}] remove nod
tag @s[scores={Dialog=362}] add next
execute if entity @s[scores={Dialog=362}] run scoreboard players set @e[tag=this_model,tag=madame_clairvoya_head,limit=1] AnimationProg 0
