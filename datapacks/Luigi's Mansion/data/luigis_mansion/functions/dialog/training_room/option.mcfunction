...
execute if score #training_choice Selected matches 1 unless entity @a[scores={TrainingOption=0..}] as @a[tag=same_room,gamemode=!spectator] unless entity @s[scores={TrainingOption=-1}] run scoreboard players set @s TrainingOption -1
