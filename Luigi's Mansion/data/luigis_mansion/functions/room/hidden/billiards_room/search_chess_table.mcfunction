...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=small_heart] positioned 683 103 0 run function luigis_mansion:spawn_entities/item/small_heart
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 683 102 0 run function luigis_mansion:blocks/dust
...
tag @e[x=683.5,y=102,z=0.5,distance=..0.7,tag=ghost,tag=hidden] add spawn