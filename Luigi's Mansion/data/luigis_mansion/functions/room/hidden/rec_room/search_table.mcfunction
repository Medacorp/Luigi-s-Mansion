...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/hidden/rec_room/table
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 655 102 38.0 run function luigis_mansion:blocks/dust
...
tag @e[x=655.5,y=102,z=38.5,distance=..0.7,tag=ghost,tag=hidden] add spawn