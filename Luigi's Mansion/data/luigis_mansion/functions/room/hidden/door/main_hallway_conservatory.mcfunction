...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=665.0,y=102,z=2.5] if data storage luigis_mansion:data current_state.current_data.obtained_keys{conservatory:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{conservatory:1b} run function luigis_mansion:room/hidden/conservatory/unlock_door
execute if block 665 102 2 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{conservatory:1b} positioned 665 102 2 run function luigis_mansion:blocks/closed_door
