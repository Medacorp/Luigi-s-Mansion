...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=692.5,y=102,z=18.0] if data storage luigis_mansion:data current_state.current_data.obtained_keys{dining_room:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{dining_room:1b} run function luigis_mansion:room/hidden/dining_room/unlock_door
execute if block 692 102 18 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{dining_room:1b} positioned 692 102 18 run function luigis_mansion:blocks/closed_door
