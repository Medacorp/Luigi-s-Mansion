...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/hidden/hidden_room/chest_6
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 707 104 -7.0 run function luigis_mansion:blocks/dust
...
tag @e[x=707.5,y=104,z=-7.5,distance=..0.7,tag=ghost,tag=hidden] add spawn