...
execute as @a[gamemode=!spectator,x=659,y=10,z=51,dx=8,dy=17,dz=26] unless entity @s[scores={Room=29}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=659,y=10,z=51,dx=8,dy=17,dz=26] run scoreboard players set @s Room 29
tag @e[tag=ghost,scores={Room=29}] add no_hidden_move

execute as @a[gamemode=!spectator,x=659,y=10,z=51,dx=8,dy=17,dz=26] run function luigis_mansion:room/hidden/main_stairs/tick_per_player
