...
execute as @a[gamemode=!spectator,x=671,y=102,z=24,dx=12,dy=6,dz=33] unless entity @s[scores={Room=14}] run scoreboard players operation @s LastRoom = @s Room
...
execute if block 673 104 33 minecraft:stone_button[powered=true] if block 677 101 33 minecraft:oak_slab unless entity @a[tag=release_boos,limit=1] unless entity @a[gamemode=!spectator,x=676.0,y=102,z=32.0,dx=2,dy=6,dz=2,limit=1] positioned 673 104 33 run tag @p[gamemode=!spectator] add release_boos
