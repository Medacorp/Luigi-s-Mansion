...
execute as @a[gamemode=!spectator,x=671,y=111,z=24,dx=12,dy=6,dz=22] unless entity @s[scores={Room=35}] run scoreboard players operation @s LastRoom = @s Room
...
execute as @a[gamemode=!spectator,x=677,y=111,z=46,dx=0,dy=1,dz=0] unless data storage luigis_mansion:data current_state.current_data.rooms.astral_hall{cleared:1b} at @s run function luigis_mansion:room/hidden/astral_hall/trick_door
