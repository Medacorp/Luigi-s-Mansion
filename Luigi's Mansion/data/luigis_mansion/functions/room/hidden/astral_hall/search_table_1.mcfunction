...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/hidden/astral_hall/table_1
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 673 111 28 run function luigis_mansion:blocks/dust
...
tag @e[x=673.5,y=111,z=28.5,distance=..0.7,tag=ghost,tag=hidden] add spawn