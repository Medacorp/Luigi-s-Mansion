...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/hidden/secret_altar/candle_5
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] 650 93 39 run function luigis_mansion:blocks/dust
