...
execute if entity @e[type=minecraft:area_effect_cloud,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/hidden/sealed_room/chest_7
execute unless entity @e[type=minecraft:area_effect_cloud,tag=chance,tag=selected,tag=!nothing] positioned 692.0 112 53 run function luigis_mansion:blocks/dust
...
tag @e[x=691.5,y=112,z=53.5,distance=..0.7,tag=ghost_marker] add spawn