...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/hidden/kitchen/closet
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 687 103 -15 run function luigis_mansion:blocks/dust
...
tag @e[x=687.5,y=102,z=-14.5,distance=..0.7,tag=ghost,tag=hidden] add spawn