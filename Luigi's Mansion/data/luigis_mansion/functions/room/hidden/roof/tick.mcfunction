...
execute as @a[gamemode=!spectator,x=682,y=131,z=-31,dx=31,dy=15,dz=77] unless entity @s[scores={Room=50}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=682,y=131,z=-31,dx=31,dy=15,dz=77] run scoreboard players set @s Room 50

execute as @a[gamemode=!spectator,x=683,y=131,z=-29,dx=28,dy=15,dz=73] run function luigis_mansion:room/hidden/roof/tick_per_player

function #luigis_mansion:room/hidden/roof/interactions/room

function luigis_mansion:room/hidden/roof/ghosts
