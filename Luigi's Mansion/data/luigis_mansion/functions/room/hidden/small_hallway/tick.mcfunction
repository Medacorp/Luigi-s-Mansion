...
execute as @a[gamemode=!spectator,x=722,y=19,z=-65,dx=8,dy=8,dz=38] unless entity @s[scores={Room=6}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=722,y=19,z=-65,dx=8,dy=8,dz=38] run scoreboard players set @s Room 6
tag @e[tag=ghost,scores={Room=6}] add no_hidden_move

execute as @a[gamemode=!spectator,x=722,y=19,z=-65,dx=8,dy=8,dz=38] run function luigis_mansion:room/hidden/small_hallway/tick_per_player
