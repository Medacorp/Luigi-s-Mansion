...
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] positioned 713 94 46 run function luigis_mansion:blocks/dust
kill @e[type=minecraft:marker,tag=chance]
scoreboard players set #cellar_shelf_1 Searched 1
tag @e[x=713.5,y=93,z=46.5,distance=..0.7,tag=ghost,tag=hidden] add spawn
