execute if score #boneyard Wave matches 7 unless entity @e[tag=ghost,scores={Room=28},tag=!optional_ghost] run function luigis_mansion:room/hidden/boneyard/clear
execute if score #boneyard Wave matches 6 if entity @e[tag=ghost,tag=dying,scores={Room=28},tag=!optional_ghost] run function luigis_mansion:room/hidden/boneyard/wave_7
