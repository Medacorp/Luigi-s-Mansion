...
execute if entity @e[type=minecraft:area_effect_cloud,tag=chance,tag=selected,tag=small_heart] positioned 701 123 2 run function luigis_mansion:spawn_entities/item/small_heart
execute unless entity @e[type=minecraft:area_effect_cloud,tag=chance,tag=selected,tag=!nothing] positioned 700 123 2 run function luigis_mansion:blocks/dust
scoreboard players set #telephone_room_shelf_3 Searched 1
tag @e[x=700.5,y=123,z=2.5,distance=..0.7,tag=ghost_marker] add spawn
tag @e[x=700.5,y=123,z=2.5,distance=..0.7,tag=hidden_boo] add spawn