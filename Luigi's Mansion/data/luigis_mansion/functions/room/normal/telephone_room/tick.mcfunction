...
execute as @a[gamemode=!spectator,x=736,y=28,z=0,dx=24,dy=8,dz=35] unless entity @s[scores={Room=44}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=736,y=28,z=0,dx=24,dy=8,dz=35] run scoreboard players set @s Room 44

execute as @a[gamemode=!spectator,x=736,y=28,z=0,dx=24,dy=8,dz=35] run function luigis_mansion:room/normal/telephone_room/tick_per_player
...
execute unless entity @a[tag=toad_5_dialog,limit=1] if entity @a[tag=blackout,limit=1] unless data storage luigis_mansion:data current_state.current_data.technical_data{telephone_1:1b} run playsound luigis_mansion:block.telephone.ring ambient @a[scores={Room=44,HallwayNoise=0}] 750.5 30 17.5 1
execute unless entity @a[tag=toad_5_dialog,limit=1] if entity @a[tag=blackout,limit=1] if data storage luigis_mansion:data current_state.current_data.technical_data{telephone_1:1b} unless data storage luigis_mansion:data current_state.current_data.technical_data{telephone_2:1b} run playsound luigis_mansion:block.telephone.ring ambient @a[scores={Room=44,HallwayNoise=0}] 750.5 30 27.5 1
execute unless entity @a[tag=uncle_grimmly_dialog,limit=1] if entity @a[tag=blackout,limit=1] if data storage luigis_mansion:data current_state.current_data.technical_data{telephone_2:1b} unless data storage luigis_mansion:data current_state.current_data.technical_data{telephone_3:1b} run playsound luigis_mansion:block.telephone.ring ambient @a[scores={Room=44,HallwayNoise=0}] 750.5 30 8.5 1
