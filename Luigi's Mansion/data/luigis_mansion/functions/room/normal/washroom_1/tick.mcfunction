...
execute as @a[gamemode=!spectator,x=660,y=10,z=34,dx=8,dy=8,dz=14] unless entity @s[scores={Room=15}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=660,y=10,z=34,dx=8,dy=8,dz=14] run scoreboard players set @s Room 15

scoreboard players set @e[scores={Room=15},type=!minecraft:marker] MirrorX 661

execute as @a[gamemode=!spectator,x=660,y=10,z=34,dx=8,dy=8,dz=14] run function luigis_mansion:room/normal/washroom_1/tick_per_player
