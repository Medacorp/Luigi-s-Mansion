...
execute if score #temp Searched matches 1..2 as @a[gamemode=!spectator,distance=..3,x=711.5,y=120,z=32.0] if data storage luigis_mansion:data current_state.current_data.obtained_keys{armory:1b} unless data storage luigis_mansion:data current_state.current_data.used_keys{armory:1b} run function luigis_mansion:room/normal/armory/unlock_door
execute if block 711 120 31 #minecraft:doors[open=true] unless data storage luigis_mansion:data current_state.current_data.used_keys{armory:1b} positioned 711 120 31 run function luigis_mansion:blocks/closed_door
