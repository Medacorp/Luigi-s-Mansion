...
execute if entity @e[type=minecraft:area_effect_cloud,tag=chance,tag=selected,tag=small_heart] positioned 735 4 9 run function luigis_mansion:spawn_entities/item/small_heart
execute unless entity @e[type=minecraft:area_effect_cloud,tag=chance,tag=selected,tag=!nothing] positioned 735 4 10 run function luigis_mansion:blocks/dust
scoreboard players set #cold_storage_shelf_1 Searched 1
tag @e[x=735.5,y=4,z=10.5,distance=..0.7,tag=hidden_boo] add spawn
tag @e[x=735.5,y=4,z=10.5,distance=..0.7,tag=ghost_marker] add spawn