...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/normal/sealed_room/chest_1
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] 698 22 -58 run function luigis_mansion:blocks/dust
kill @e[type=minecraft:marker,tag=chance]
scoreboard players set #sealed_room_chest_1 Searched 1
data merge block 698 22 -58 {LootTable:"luigis_mansion:search"}
tag @e[x=698.5,y=22,z=-57.5,distance=..0.7,tag=ghost_marker] add spawn