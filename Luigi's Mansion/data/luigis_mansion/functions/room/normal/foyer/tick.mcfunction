...
execute as @a[gamemode=!spectator,x=735,y=10,z=-2,dx=21,dy=17,dz=21] unless entity @s[scores={Room=1}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=735,y=10,z=-2,dx=21,dy=17,dz=21] run scoreboard players set @s Room 1
tag @e[tag=ghost,scores={Room=1}] add hallway

execute as @a[gamemode=!spectator,x=735,y=11,z=-2,dx=21,dy=15,dz=21] run function luigis_mansion:room/normal/foyer/tick_per_player

execute unless entity @e[type=minecraft:armor_stand,tag=key,tag=parlor,limit=1] unless data storage luigis_mansion:data {obtained_parlor_key:1b} if entity @e[x=736,y=21,z=8,dx=0,dy=1,dz=1,tag=door,nbt={Age:60}] positioned 751.0 13 9.0 run playsound luigis_mansion:entity.ghost.cry hostile @a ~ ~ ~ 3
execute unless entity @e[type=minecraft:armor_stand,tag=key,tag=parlor,limit=1] unless entity @e[type=minecraft:armor_stand,tag=first_key,limit=1] unless data storage luigis_mansion:data {obtained_parlor_key:1b} if entity @e[x=736,y=20,z=8,dx=0,dy=1,dz=1,tag=door] run summon minecraft:armor_stand 751.0 13 9.0 {CustomName:'{"translate":"luigis_mansion:entity.gold_ghost"}',Tags:["first_key"],NoGravity:1b,Invulnerable:1b,Silent:1b,Invisible:1b,DisabledSlots:2039583,ArmorItems:[{},{},{},{id:"minecraft:brick",Count:1b,tag:{CustomModelData:1}}]}
