...
execute as @a[gamemode=!spectator,x=686,y=1,z=-6,dx=14,dy=8,dz=18] unless entity @s[scores={Room=54}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[type=!minecraft:item_frame,x=686,y=1,z=-6,dx=14,dy=8,dz=18] run scoreboard players set @s Room 54

execute as @a[gamemode=!spectator,x=686,y=1,z=-6,dx=14,dy=8,dz=18] run function luigis_mansion:room/normal/pipe_room/tick_per_player
