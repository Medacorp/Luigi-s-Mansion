...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/normal/guest_room/table_2
execute unless entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=!nothing] 744 20 -61 run function luigis_mansion:blocks/dust
kill @e[type=minecraft:marker,tag=chance]
scoreboard players set #guest_room_table_2 Searched 1
data merge block 744 20 -61 {LootTable:"luigis_mansion:search"}
tag @e[x=744.5,y=20,z=-60.5,distance=..0.7,tag=ghost_marker] add spawn
