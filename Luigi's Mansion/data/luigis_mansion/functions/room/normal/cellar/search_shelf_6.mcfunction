...
execute if entity @e[type=minecraft:area_effect_cloud,tag=chance,tag=selected,tag=money] run function luigis_mansion:room/normal/cellar/shelf_6
execute unless entity @e[type=minecraft:area_effect_cloud,tag=chance,tag=selected,tag=!nothing] positioned 714 98 -40 run function luigis_mansion:blocks/dust
scoreboard players set #cellar_shelf_6 Searched 1
tag @e[x=714.5,y=97,z=-39.5,distance=..0.7,tag=ghost_marker] add spawn
tag @e[x=714.5,y=97,z=-39.5,distance=..0.7,tag=hidden_boo] add spawn