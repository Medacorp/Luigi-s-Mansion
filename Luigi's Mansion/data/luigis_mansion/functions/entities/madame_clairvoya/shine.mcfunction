...
execute positioned ^ ^1 ^1.5 if entity @e[tag=flashlight,distance=..3,limit=1] if entity @s[scores={PathStep=1}] run playsound luigis_mansion:block.crystal_ball.flash block @a[tag=same_room] ~ ~ ~ 1
execute positioned ^ ^1 ^1.5 if entity @e[tag=flashlight,distance=..3,limit=1] if entity @s[scores={PathStep=10}] run playsound luigis_mansion:block.crystal_ball.shine block @a[tag=same_room] ~ ~ ~ 1
