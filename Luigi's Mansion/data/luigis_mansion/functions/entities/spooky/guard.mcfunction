...
execute unless entity @s[scores={Dialog=1..}] at @a[tag=same_room,gamemode=!spectator] positioned ^ ^ ^8 if entity @s[distance=..8,tag=visible] run function luigis_mansion:entities/spooky/turn_invisible
...
execute at @s[scores={Dialog=40..80},tag=!attack] facing entity @p[gamemode=!spectator,limit=1] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
...
tag @s[scores={Dialog=80}] remove bark
tag @s[scores={Dialog=80}] add move
execute if entity @e[tag=spooky_bone,limit=1] run scoreboard players set @s[scores={Dialog=80}] Dialog 163
execute at @s[scores={Dialog=80}] run function luigis_mansion:entities/ghost/move_forward
...
execute at @s[scores={Dialog=122..161},tag=!attack] facing entity @p[gamemode=!spectator,limit=1] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
