execute as @a[tag=same_room,gamemode=!spectator] unless entity @s[scores={MusicType=25}] run scoreboard players set @s Music 0
execute as @a[tag=same_room,gamemode=!spectator] unless entity @s[scores={MusicType=25}] run scoreboard players set @s MusicType 25
