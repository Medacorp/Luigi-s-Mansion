...
execute if entity @s[tag=dead] run function luigis_mansion:entities/nana/drop_loot
execute if entity @s[tag=dead] run particle minecraft:dust 0.7 1 1 1 ~-0.1 ~ ~0.1 0.2 0.6 0.2 1 30
execute if entity @s[tag=dead] run teleport @s ~ -100 ~
execute if entity @s[tag=dead] store result storage luigis_mansion:data current_state.current_data.portrait_ghosts.nana.health int 1 run scoreboard players set @s Health 0
...
execute if entity @s[scores={HurtTime=1},tag=hurt] run playsound luigis_mansion:entity.nana.hurt hostile @a[tag=same_room] ~ ~ ~ 1
