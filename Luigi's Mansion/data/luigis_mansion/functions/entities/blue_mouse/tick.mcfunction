...
execute if entity @e[tag=same_room,tag=!spectator,distance=..0.7,limit=1] if entity @s[tag=visible] if entity @s[tag=!dead,tag=!removed_from_existence] run function luigis_mansion:entities/blue_mouse/collide
...
data merge entity @s[tag=visible] {ArmorItems:[{},{},{},{id:"minecraft:diamond_pickaxe",Count:1b,tag:{Unbreakable:1b,Damage:1,CustomModelData:56}}]}
execute if entity @s[tag=!rotated,tag=visible] run function luigis_mansion:entities/blue_mouse/move
