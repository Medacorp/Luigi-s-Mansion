...
execute unless score #temp ActionTime matches 3.. unless entity @e[tag=pink_flying_fish,tag=visible,distance=..0.7] if entity @s[tag=!visible] run playsound luigis_mansion:entity.pink_flying_fish.spawn hostile @a[tag=same_room] ~ ~ ~ 1
execute unless score #temp ActionTime matches 3.. unless entity @e[tag=pink_flying_fish,tag=visible,distance=..0.7] run tag @s add visible
scoreboard players reset #temp ActionTime
scoreboard players reset #temp Room

data merge entity @s[tag=visible] {ArmorItems:[{},{},{},{id:"minecraft:diamond_pickaxe",Count:1b,tag:{Unbreakable:1b,Damage:1,CustomModelData:85}}]}
