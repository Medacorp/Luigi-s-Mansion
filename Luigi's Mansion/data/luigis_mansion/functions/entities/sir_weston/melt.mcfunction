execute unless entity @s[scores={Dialog=1..}] at @a[tag=same_room,gamemode=!spectator] positioned ^ ^ ^8 run effect give @s[distance=..8] minecraft:invisibility 1 0 true
...
execute if entity @s[scores={Dialog=1}] run scoreboard players set @a[tag=same_room,gamemode=!spectator,scores={Music=..30}] Music 30
...
execute if entity @s[scores={Dialog=31..421}] facing entity @p[tag=same_room,gamemode=!spectator] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
execute if entity @s[scores={Dialog=31..421}] as @a[tag=same_room,gamemode=!spectator] unless entity @s[scores={MusicType=23}] run scoreboard players set @s Music 0
execute if entity @s[scores={Dialog=31..421}] as @a[tag=same_room,gamemode=!spectator] unless entity @s[scores={MusicType=23}] run scoreboard players set @s MusicType 23
...
execute if entity @s[scores={Dialog=422..}] as @a[tag=same_room,gamemode=!spectator] unless entity @s[scores={MusicType=36}] run function luigis_mansion:other/music/set/danger
scoreboard players add @s[scores={Dialog=422..482}] PathStep 1
execute if entity @s[scores={PathStep=20..50,VulnerableTime=0}] facing entity @p[tag=same_room,gamemode=!spectator] feet rotated ~ 0 run teleport @s ~ ~ ~ ~ ~
