...
execute if entity @s[scores={Dialog=1}] run scoreboard players set @a[tag=same_room,gamemode=!spectator] MusicType 7
...
execute if entity @s[scores={Dialog=175..299}] at @a[tag=same_room,gamemode=!spectator] positioned ^ ^ ^8 run effect clear @s[distance=..8] minecraft:invisibility
...
execute if entity @s[scores={Dialog=299..}] as @a[tag=same_room,gamemode=!spectator] unless entity @s[scores={MusicType=36}] run function luigis_mansion:other/music/set/danger
