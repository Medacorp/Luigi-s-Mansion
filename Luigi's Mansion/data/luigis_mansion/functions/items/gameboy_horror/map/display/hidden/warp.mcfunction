...
execute unless entity @s[scores={Room=1..72}] run tag @s[nbt={Dimension:"luigis_mansion:hidden"}] add cancel
execute unless entity @a[tag=looking_at_map,scores={Room=1..72}] if entity @s[tag=!cancel] run function luigis_mansion:items/gameboy_horror/map/display/hidden/generate