execute rotated ~ 0 positioned ^ ^-0.5 ^1.2 if entity @e[type=minecraft:marker,tag=position,distance=..1.19999,limit=1] if entity @s[scores={HurtTime=30..},tag=!dying,tag=!dialog] run function luigis_mansion:entities/ghost/hurt_pull
