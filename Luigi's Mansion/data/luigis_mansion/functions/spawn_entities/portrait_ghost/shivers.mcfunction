...
execute as @e[tag=shivers,tag=this_entity,limit=1] store result score @s LastHealth run data get storage luigis_mansion:data current_state.current_data.portrait_ghosts.shivers.health
execute as @e[tag=shivers,tag=this_entity,limit=1] run scoreboard players set @s MaxHealth 10000
scoreboard players operation @e[tag=shivers,tag=this_entity,limit=1] MaxHealth *= #multiply_health Selected
scoreboard players operation @e[tag=shivers,tag=this_entity,limit=1] MaxHealth /= #100 Constants
scoreboard players operation @e[tag=shivers,tag=this_entity,limit=1] MaxHealth += #extra_health Selected
execute as @e[tag=shivers,tag=this_entity,limit=1] if score @s LastHealth matches -1 store result storage luigis_mansion:data current_state.current_data.portrait_ghosts.shivers.health int 1 run scoreboard players operation @s LastHealth = @s MaxHealth
execute as @e[tag=shivers,tag=this_entity,limit=1] run scoreboard players operation @s Health = @s LastHealth
