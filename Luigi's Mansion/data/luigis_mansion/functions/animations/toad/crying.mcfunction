teleport @s ~ ~-0.75 ~ ~ ~
teleport @e[tag=this_model,tag=toad_body,limit=1] ~ ~-1.25 ~ ~ ~
...
execute if entity @s[scores={AnimationProg=1}] run data merge entity @e[tag=this_model,tag=toad_body,limit=1] {Pose:{RightArm:[-70.0f,0.0f,0.01f],LeftArm:[-70.0f,0.0f,0.01f],Head:[0.0f,0.0f,0.01f]}}
...
execute store result score #temp Time run data get entity @e[tag=this_model,tag=toad_body,limit=1] Pose.RightArm[0]
execute if entity @s[scores={AnimationProg=1..5}] store result entity @e[tag=this_model,tag=toad_body,limit=1] Pose.RightArm[0] float 1 run scoreboard players add #temp Time 2
execute if entity @s[scores={AnimationProg=6..15}] store result entity @e[tag=this_model,tag=toad_body,limit=1] Pose.RightArm[0] float 1 run scoreboard players remove #temp Time 2
execute if entity @s[scores={AnimationProg=16..20}] store result entity @e[tag=this_model,tag=toad_body,limit=1] Pose.RightArm[0] float 1 run scoreboard players add #temp Time 2
execute store result score #temp Time run data get entity @e[tag=this_model,tag=toad_body,limit=1] Pose.LeftArm[0]
execute if entity @s[scores={AnimationProg=1..5}] store result entity @e[tag=this_model,tag=toad_body,limit=1] Pose.LeftArm[0] float 1 run scoreboard players remove #temp Time 2
execute if entity @s[scores={AnimationProg=6..15}] store result entity @e[tag=this_model,tag=toad_body,limit=1] Pose.LeftArm[0] float 1 run scoreboard players add #temp Time 2
execute if entity @s[scores={AnimationProg=16..20}] store result entity @e[tag=this_model,tag=toad_body,limit=1] Pose.LeftArm[0] float 1 run scoreboard players remove #temp Time 2
