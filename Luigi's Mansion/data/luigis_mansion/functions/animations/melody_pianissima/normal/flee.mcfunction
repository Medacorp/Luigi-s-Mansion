...
execute if entity @s[scores={AnimationProg=1}] run data merge entity @e[tag=this_melody_pianissima_body,limit=1] {Pose:{Head:[20.0f,0.0f,0.01f]}}
...
execute store result score #temp Time run data get entity @e[tag=this_melody_pianissima_body,limit=1] Pose.Head[0]
execute if entity @s[scores={AnimationProg=1..4}] store result entity @e[tag=this_melody_pianissima_body,limit=1] Pose.Head[0] float 1 run scoreboard players add #temp Time 2
execute if entity @s[scores={AnimationProg=4..8}] store result entity @e[tag=this_melody_pianissima_body,limit=1] Pose.Head[0] float 1 run scoreboard players remove #temp Time 2
