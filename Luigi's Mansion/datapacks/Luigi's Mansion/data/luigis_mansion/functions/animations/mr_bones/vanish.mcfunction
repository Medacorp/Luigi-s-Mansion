...
execute if entity @s[scores={AnimationProgress=1..30}] store result entity @s Pose.RightArm[0] float 1 run scoreboard players add #temp Time 3
execute store result score #temp Time run data get entity @s Pose.LeftArm[0]
execute if entity @s[scores={AnimationProgress=1..30}] store result entity @s Pose.LeftArm[0] float 1 run scoreboard players add #temp Time 3
...
teleport @e[tag=this_model,limit=1] ~ ~-0.3 ~ ~ ~
