...
execute if entity @s[scores={AnimationProg=1}] run data merge entity @e[tag=this_model,limit=1] {Pose:{RightArm:[90.0f,0.0f,0.01f],LeftArm:[90.0f,0.0f,0.01f],Head:[90.0f,0.0f,0.01f]}}
...
execute if entity @s[scores={AnimationProg=11..20}] store result entity @e[tag=this_model,limit=1] Pose.RightArm[0] float 1 run scoreboard players remove #temp Time 2
execute store result score #temp Time run data get entity @e[tag=this_model,limit=1] Pose.LeftArm[0]
execute if entity @s[scores={AnimationProg=11..20}] store result entity @e[tag=this_model,limit=1] Pose.LeftArm[0] float 1 run scoreboard players remove #temp Time 2
