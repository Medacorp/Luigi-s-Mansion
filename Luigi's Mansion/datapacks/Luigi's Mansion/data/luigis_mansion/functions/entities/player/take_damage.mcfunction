...
execute if entity @s[scores={Damage=1..,Invulnerable=0},gamemode=!creative,gamemode=!spectator] run function luigis_mansion:entities/player/damage_effect
execute store result score @s Damage run data get storage luigis_mansion:data damage.limit_health
scoreboard players operation @s MaxHealth < @s Damage
