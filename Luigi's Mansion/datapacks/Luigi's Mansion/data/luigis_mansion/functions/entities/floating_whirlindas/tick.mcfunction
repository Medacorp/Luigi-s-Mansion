...
execute if entity @s[tag=dying,scores={DeathTime=1}] if data storage luigis_mansion:data current_state.current_data.portrait_ghosts.floating_whirlindas{loot_at_0:1b} run function luigis_mansion:entities/floating_whirlindas/drop_loot
execute if entity @s[tag=dead] unless data storage luigis_mansion:data current_state.current_data.portrait_ghosts.floating_whirlindas{loot_at_0:1b} run function luigis_mansion:entities/floating_whirlindas/at_death
...
execute if entity @e[tag=same_room,tag=!spectator,distance=..1.6,limit=1] if entity @s[tag=!vanish,tag=!dying,tag=!dead,tag=!captured,tag=!removed_from_existence,tag=visible,tag=!hurt,tag=!fleeing] run function luigis_mansion:entities/floating_whirlindas/collide
