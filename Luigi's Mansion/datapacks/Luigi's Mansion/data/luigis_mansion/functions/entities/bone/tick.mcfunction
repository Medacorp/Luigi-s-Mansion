...
execute if entity @s[tag=dead,scores={Owner=-2147483648..}] if entity @a[distance=..0.7,scores={Invulnerable=0},tag=!spectator,limit=1] as @e[tag=!model_piece,tag=ghost] if score @s GhostNr = #temp GhostNr run tag @s add laugh
execute if entity @s[tag=dead,scores={Owner=-2147483648..}] unless entity @a[distance=..0.7,scores={Invulnerable=0},tag=!spectator,limit=1] as @e[tag=!model_piece,tag=ghost] if score @s GhostNr = #temp GhostNr run tag @s add complain
