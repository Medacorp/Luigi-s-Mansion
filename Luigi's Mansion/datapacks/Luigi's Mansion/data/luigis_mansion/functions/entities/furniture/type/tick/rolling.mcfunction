...
execute if entity @s[scores={FurnitureVacuum=0},tag=!shake,tag=!long_shake,tag=was_shaking] run function luigis_mansion:entities/furniture/revert_to_default_orientation
tag @s remove in_vacuum

execute if score @s[tag=!searched,tag=searchable_by_vacuum] FurnitureVacuum >= @s FurnitureSearch run function luigis_mansion:entities/furniture/search/roll
