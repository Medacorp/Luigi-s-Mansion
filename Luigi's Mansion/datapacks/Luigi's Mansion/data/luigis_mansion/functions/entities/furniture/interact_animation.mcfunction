...
execute unless entity @s[scores={InteractionTime=-2147483648..}] as @a[tag=searcher,tag=!not_facing,limit=1] run function luigis_mansion:entities/player/animation/set/none
execute unless entity @s[scores={InteractionTime=-2147483648..}] run scoreboard players operation @a[tag=searcher,tag=!not_facing,limit=1] Animation = @s InteractionTime