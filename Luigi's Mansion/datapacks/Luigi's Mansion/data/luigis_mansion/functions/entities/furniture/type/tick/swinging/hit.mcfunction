...
teleport @s ~ ~ ~ 0 0
data modify entity @e[tag=collision_check,tag=z,limit=1] Pos[2] set from entity @e[tag=collision_check,tag=x,limit=1] Pos[2]
...
execute store result entity @e[tag=collision_check,tag=z,limit=1] Pos[1] double 0.1 run scoreboard players operation #temp Time > #temp2 Time
kill @e[tag=collision_check,tag=x,limit=1]
scoreboard players reset #temp Time
scoreboard players reset #temp2 Time
tag @s add me
scoreboard players operation #y Steps = @s FurnitureSizeH
execute at @e[tag=collision_check,tag=z,limit=1] facing entity @s eyes positioned ~-0.5 ~-0.5 ~-0.5 run function luigis_mansion:entities/furniture/type/tick/swinging/harm
