...
execute if entity @s[scores={ActionTime=10}] positioned ^ ^0.2 ^ as @e[distance=..0.1,tag=bowling_ball,limit=1] unless entity @s[scores={GhostNr=1..}] positioned ^ ^-0.2 ^ run scoreboard players operation @s Owner = @e[tag=bowling_ghost,distance=..0.1,limit=1] GhostNr
