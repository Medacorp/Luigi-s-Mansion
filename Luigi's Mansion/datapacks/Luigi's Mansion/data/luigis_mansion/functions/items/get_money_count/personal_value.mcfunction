data modify block 26 0 0 Items[0].tag.value set from storage luigis_mansion:data current_state.current_data.money_grabbed[-1].uuid
execute store success score #temp ActionTime if blocks 27 0 0 27 0 0 26 0 0 all
execute if score #temp ActionTime matches 1 run data modify storage luigis_mansion:data current_state.my_money set from storage luigis_mansion:data current_state.current_data.money_grabbed[-1]
...
execute unless score #temp ActionTime matches 1 if data storage luigis_mansion:data current_state.current_data.money_grabbed[-1] run function luigis_mansion:items/game_boy_horror/show_items/check_money/personal_value