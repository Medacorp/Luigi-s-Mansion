...
execute at @s[scores={Shrunk=0},tag=!show_credits] rotated ~ 0 positioned ^ ^ ^1 align xyz positioned ~0.5 ~ ~0.5 if entity @e[distance=..1,tag=door,tag=frame,tag=!open_door,tag=!barricade,tag=!unopenable,limit=1] run function luigis_mansion:items/interact/try_opening_door
execute at @s[scores={Shrunk=0},tag=!show_credits,tag=!try_opening_door] rotated ~ 0 positioned ^ ^1.1 ^2 if entity @e[distance=..2,tag=can_talk_to,tag=!talk,limit=1] run function luigis_mansion:items/interact/try_talking
