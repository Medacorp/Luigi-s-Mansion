...
execute as @e[type=minecraft:marker,tag=this_interact,limit=1] positioned as @s positioned ^ ^ ^0.5 run function luigis_mansion:items/game_boy_horror/scan/target
execute as @e[tag=target,limit=1] if score #temp Room matches 1.. run function #luigis_mansion:items/game_boy_horror/scan/entity_mansion
execute as @e[tag=target,limit=1] if score #temp Room matches ..-1 run function #luigis_mansion:items/game_boy_horror/scan/entity
