...
execute as @e[tag=same_room,tag=!model_piece,tag=!hidden,tag=!item,tag=!no_poltergust_interaction,tag=!hit_by_poltergust] unless entity @s[tag=furniture,tag=!searchable_by_ice] run function luigis_mansion:items/poltergust_3000/effect_on_entity/ice