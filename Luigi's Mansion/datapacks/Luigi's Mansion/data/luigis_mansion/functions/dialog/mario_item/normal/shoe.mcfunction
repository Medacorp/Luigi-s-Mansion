...
execute if entity @s[scores={Dialog=1..298}] as @a[tag=same_room] run function luigis_mansion:other/music/set/talking_ghost
...
execute if entity @s[scores={Dialog=154}] run tellraw @a[tag=same_room] {"type":"translatable","translate":"chat.type.text","with":[{"type":"translatable","translate":"luigis_mansion:entity.madame_clairvoya","color":"green"},{"type":"translatable","translate":"luigis_mansion:dialog.madame_clairvoya.shoe.1"}]}
tag @s[scores={Dialog=298}] remove nod
tag @s[scores={Dialog=298}] add next
scoreboard players set @s[scores={Dialog=298}] AnimationProg 0
