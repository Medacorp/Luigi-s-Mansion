summon minecraft:armor_stand ~ ~-1.4 ~ {CustomName:'{"translate":"luigis_mansion:furniture.treasure_chest"}',Tags:["furniture","affected_by_interact","affected_by_vacuum","generic","immobile","chest","standing_furniture","this_entity"],Pose:{Head:[0.0f,0.01f,0.0f]},NoGravity:1b,Marker:1b,ArmorItems:[{},{},{},{id:"minecraft:diamond_pickaxe",Count:1b,tag:{Unbreakable:1b,Damage:19,CustomModelData:52}}],Invisible:1b,DisabledSlots:2039583}
...
execute if data storage luigis_mansion:data furniture{searched:1b,no_search_animation:1b} run data modify entity @e[tag=this_entity,limit=1] ArmorItems[3].tag.CustomModelData set value 56
...
scoreboard players set @e[tag=this_entity,limit=1] FurnitureSizeL 26
scoreboard players set @e[tag=this_entity,limit=1] FurnitureSizeU 19
...
scoreboard players set @e[tag=this_entity,limit=1] FurnitureElemenU 12
