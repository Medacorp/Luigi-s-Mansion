...
execute as @a[gamemode=!spectator,x=562,y=13,z=-9,dx=48,dy=77,dz=87] run scoreboard players set @s LastRoom 75
execute as @e[tag=!spectator,x=562,y=13,z=-9,dx=48,dy=77,dz=87] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 75
...
execute if entity @a[tag=same_room,limit=1] run function luigis_mansion:room/hidden/boolossus_battle/ghosts
