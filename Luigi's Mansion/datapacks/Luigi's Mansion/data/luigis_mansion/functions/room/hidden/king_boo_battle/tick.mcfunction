...
execute as @e[tag=!spectator,x=465,y=23,z=-40,dx=49,dy=61,dz=85] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 76
...
execute as @a[gamemode=!spectator,scores={Room=76}] run function luigis_mansion:room/hidden/king_boo_battle/tick_per_player
