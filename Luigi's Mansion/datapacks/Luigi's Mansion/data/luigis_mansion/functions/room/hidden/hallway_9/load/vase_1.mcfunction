...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] unless data storage luigis_mansion:data current_state.current_data{money_spawned:["hallway_9_vase_1"]} run data modify storage luigis_mansion:data furniture.loot set value {name:"hallway_9_vase_1",contents:{luigis_mansion:{bill:10}}}
kill @e[type=minecraft:marker,tag=chance]
execute positioned 680 21.5 15 rotated -90 0 run function luigis_mansion:spawn_furniture/vase