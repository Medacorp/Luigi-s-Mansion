...
execute as @a[gamemode=!spectator,x=743,y=8,z=-4,dx=15,dy=19,dz=21] unless entity @s[scores={Room=1}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=743,y=8,z=-4,dx=15,dy=19,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 1
