...
execute as @a[gamemode=!spectator,x=653,y=-1,z=27,dx=33,dy=9,dz=12] unless entity @s[scores={Room=61}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=653,y=-1,z=27,dx=33,dy=9,dz=12] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 61
