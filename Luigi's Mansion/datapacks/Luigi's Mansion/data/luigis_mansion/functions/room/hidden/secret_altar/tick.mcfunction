...
execute as @a[gamemode=!spectator,x=634,y=90,z=24,dx=20,dy=8,dz=18] unless entity @s[scores={Room=72}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=634,y=90,z=24,dx=20,dy=8,dz=18] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 72
