...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] unless data storage luigis_mansion:data current_state.current_data{money_spawned:["hallway_5_vase_3"]} run data modify storage luigis_mansion:data furniture.loot set value {name:"hallway_5_vase_3",contents:{luigis_mansion:{gold_bar:1}}}
kill @e[type=minecraft:marker,tag=chance]
execute positioned 680 12.5 40 rotated -90 0 run function luigis_mansion:spawn_furniture/vase