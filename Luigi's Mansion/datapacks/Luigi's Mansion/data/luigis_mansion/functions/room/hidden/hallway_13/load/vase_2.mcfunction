...
execute if entity @e[type=minecraft:marker,tag=chance,tag=selected,tag=money] unless data storage luigis_mansion:data current_state.current_data{money_spawned:["hallway_13_vase_2"]} run data modify storage luigis_mansion:data furniture.loot set value {name:"hallway_13_vase_2",contents:{luigis_mansion:{gold_bar:1}}}
