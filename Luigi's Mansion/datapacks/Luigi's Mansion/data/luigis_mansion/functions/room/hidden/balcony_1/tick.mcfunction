...
execute as @a[gamemode=!spectator,x=672,y=108,z=-29,dx=10,dy=10,dz=8] unless entity @s[scores={Room=6}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=672,y=108,z=-29,dx=10,dy=10,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 6
