...
execute as @a[gamemode=!spectator,x=687,y=26,z=-80,dx=16,dy=10,dz=25] unless entity @s[scores={Room=71}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=687,y=26,z=-80,dx=16,dy=10,dz=25] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 71
scoreboard players set #temp Room 71

execute as @a[gamemode=!spectator,scores={Room=71}] run function luigis_mansion:room/normal/the_artists_studio/tick_per_player
...
execute as @e[tag=!spectator,x=697.0,y=26,z=-78.0,dx=5,dy=10,dz=21,tag=ghost,tag=hidden,y_rotation=-180..0] at @s run function luigis_mansion:room/normal/the_artists_studio/stay_near_painting
