...
execute as @e[tag=!spectator,x=743,y=8,z=-2,dx=15,dy=18,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 1
...
execute if entity @a[tag=same_room,limit=1] run function luigis_mansion:room/normal/foyer/ghosts
