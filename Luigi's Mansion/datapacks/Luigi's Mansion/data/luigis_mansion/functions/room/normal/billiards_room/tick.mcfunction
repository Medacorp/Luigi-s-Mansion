...
execute as @e[tag=!spectator,x=673,y=8,z=64,dx=15,dy=9,dz=27] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 47
...
execute if entity @a[tag=same_room,limit=1] run function luigis_mansion:room/normal/billiards_room/ghosts
