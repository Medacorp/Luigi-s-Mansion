...
execute as @a[gamemode=!spectator,x=627,y=14,z=-145,dx=24,dy=13,dz=72] unless entity @s[scores={Room=46}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=627,y=14,z=-145,dx=24,dy=13,dz=72] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 46
...
execute if entity @a[tag=same_room,limit=1] run function luigis_mansion:room/normal/observatory/ghosts
