...
execute as @a[gamemode=!spectator,x=679,y=8,z=-70,dx=12,dy=10,dz=33] unless entity @s[scores={Room=18}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=679,y=8,z=-70,dx=12,dy=10,dz=33] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 18
...
execute if block 681 13 -46 minecraft:stone_button[powered=true] if block 685 10 -46 minecraft:oak_slab unless entity @a[tag=release_boos,limit=1] unless entity @e[tag=!spectator,scores={Room=18},x=684.0,y=8,z=-47.0,dx=2,dy=10,dz=2,limit=1] positioned 681 13 -46 run tag @p[gamemode=!spectator] add release_boos
