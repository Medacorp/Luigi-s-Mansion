...
execute as @a[gamemode=!spectator,x=734,y=17,z=-63,dx=14,dy=10,dz=21] unless entity @s[scores={Room=70}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=734,y=17,z=-63,dx=14,dy=10,dz=21] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 70
...
execute as @a[gamemode=!spectator,scores={Room=70}] run function luigis_mansion:room/normal/guest_room/tick_per_player
