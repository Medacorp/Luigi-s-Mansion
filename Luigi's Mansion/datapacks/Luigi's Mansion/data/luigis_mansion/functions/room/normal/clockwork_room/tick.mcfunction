...
execute as @a[gamemode=!spectator,x=682,y=117,z=-10,dx=17,dy=8,dz=35] unless entity @s[scores={Room=62}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[tag=!spectator,x=682,y=117,z=-10,dx=17,dy=8,dz=35] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 62
scoreboard players set #temp Room 62

execute as @a[gamemode=!spectator,scores={Room=62}] run function luigis_mansion:room/normal/clockwork_room/tick_per_player
...
execute if entity @a[tag=same_room,limit=1] run function luigis_mansion:room/normal/clockwork_room/ghosts
