...
execute if data storage luigis_mansion:data current_state.current_data.rooms.telephone_room{seen:0b} unless entity @s[gamemode=spectator] run function luigis_mansion:room/normal/telephone_room/set_seen

execute unless entity @s[tag=!wall_warp,gamemode=!spectator] if entity @s[tag=!already_ticked] run function luigis_mansion:room/normal/telephone_room/spectator_tick
