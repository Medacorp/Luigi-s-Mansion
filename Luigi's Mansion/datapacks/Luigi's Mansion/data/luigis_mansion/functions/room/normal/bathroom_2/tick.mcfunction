...
execute as @e[tag=!spectator,x=648,y=17,z=15,dx=12,dy=9,dz=14] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 43
...
execute if entity @a[tag=same_room,limit=1] run function luigis_mansion:room/normal/bathroom_2/ghosts
