data modify entity @e[tag=this_entity,limit=1] data.shake_sound set value {namespace:"luigis_mansion",id:"none"}
execute if data storage luigis_mansion:data furniture.sound run data modify entity @e[tag=this_entity,limit=1] data.shake_sound.id set value "heavy_generic"
...
scoreboard players set @e[tag=this_entity,limit=1] RoomNoise 0