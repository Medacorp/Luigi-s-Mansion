...
execute if entity @s[tag=hit] if entity @e[tag=interact,tag=manual,limit=1,tag=feet] as @e[tag=searcher,limit=1] unless data storage luigis_mansion:data my_memory.animation run function luigis_mansion:entities/luigi/animation/set/search/hump
tag @s remove hit