...
tag @s add me
execute if entity @s[scores={UseItem=1..},tag=!poltergust_selected,nbt={data:{selected_item:{components:{"minecraft:custom_data":{namespace:"luigis_mansion",id:"interact"}}}}}] unless entity @s[nbt={data:{animation:{}}},tag=!idle] unless entity @s[scores={InteractionTime=1..}] as @a[tag=this_player,limit=1] run function luigis_mansion:items/interact/use_item
