execute if entity @s[scores={HurtTime=10,Health=1..},tag=!no_ai] run scoreboard players set @e[tag=me,scores={Sound=0},limit=1] Sound 1
...
execute unless entity @s[scores={HurtTime=30..,Health=1..}] run tag @e[tag=me,limit=1] add is_pulling
...
execute at @e[type=minecraft:marker,tag=position,limit=1] facing entity @e[tag=me,limit=1] feet run teleport @e[type=minecraft:marker,tag=position,limit=1] ~ ~ ~ ~ 0
...
execute if entity @s[scores={HurtTime=30..,Health=1..},tag=!no_ai] at @e[type=minecraft:marker,tag=position,limit=1] unless entity @e[tag=me,distance=..0.1,limit=1] unless score #temp Time > #temp PullAngle at @s run function luigis_mansion:items/poltergust_3000/vacuuming/hurt_ghost
...
execute rotated as @e[tag=me,limit=1] positioned ^ ^ ^0.5 facing entity @e[tag=target_direction,limit=1] feet run function luigis_mansion:items/poltergust_3000/vacuuming/ghost_step
kill @e[tag=target_direction,limit=1]
scoreboard players reset #temp PositionY
scoreboard players reset #temp Steps
execute if entity @s[scores={Health=..0}] run tag @e[tag=me,limit=1] add capturing_ghost