...
execute if entity @s[tag=flashlight] positioned ~ ~1 ~ positioned ^ ^ ^0.8 if block ~ ~ ~ #luigis_mansion:flashlight_path run summon minecraft:marker ~ ~ ~ {Tags:["flashlight","new"],data:{entity:{namespace:"luigis_mansion",id:"flashlight"}}}
...
execute if entity @s[tag=flashlight] as @e[type=minecraft:marker,tag=flashlight,tag=new,limit=1] store result score @s HomeRotationX run data get entity @s Rotation[0]
execute if entity @s[tag=flashlight] as @e[type=minecraft:marker,tag=flashlight,tag=new,limit=1] store result score @s HomeRotationY run data get entity @s Rotation[1]
