data modify storage luigis_mansion:data ghost set value {tags:[],room:0,mirror:{},initial_animation_progress:0,animation:{namespace:"luigis_mansion",id:"idle"},rotation:[0.0f,0.0f],size:1}
...
execute if entity @s[tag=merged] store result storage luigis_mansion:data ghost.size int 1 run scoreboard players get @s Health
tag @s add this_entity
$execute if entity @s[tag=freeze_animation] as @e[type=minecraft:item_display,tag=$(name)_model,scores={GhostNr=$(id)}] run function luigis_mansion:animations/ghost/frozen
$execute if entity @s[tag=!freeze_animation] as @e[type=minecraft:item_display,tag=$(name)_model,scores={GhostNr=$(id)}] rotated ~ 0 run function luigis_mansion:animations/ghost/main with storage luigis_mansion:data macro.ghost
