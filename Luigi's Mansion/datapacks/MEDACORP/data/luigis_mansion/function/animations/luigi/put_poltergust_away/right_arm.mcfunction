...
execute unless score #mirrored Selected matches 1 run scoreboard players set @s[scores={AnimationProgress=2}] AnimationRotationX 250
...
execute unless score #mirrored Selected matches 1 run scoreboard players set @s[scores={AnimationProgress=10}] AnimationRotationX 0
execute if entity @s[tag=held_item,tag=poltergust_grabbed] unless data entity @s item.components."minecraft:custom_data".is_nozzle unless score #mirrored Selected matches 1 run data modify entity @s item set from storage luigis_mansion:data luigi.inventory[{components:{"minecraft:custom_data":{is_poltergust:1b}}}]
execute if entity @s[tag=held_item,tag=!poltergust_grabbed] if data entity @s item.components."minecraft:custom_data".is_nozzle unless score #mirrored Selected matches 1 run data remove entity @s item
