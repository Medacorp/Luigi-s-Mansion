...
execute unless score #mirrored Selected matches 1 run data modify entity @s[tag=held_item,scores={AnimationProgress=40..55}] ArmorItems[3].components."minecraft:dyed_color".rgb set from storage luigis_mansion:data luigi.animation.hat_model.color
execute unless score #mirrored Selected matches 1 if data storage luigis_mansion:data luigi.animation.hat_model{variant:1b} run data modify entity @s[tag=held_item,scores={AnimationProgress=40..55}] ArmorItems[3].components."minecraft:custom_model_data".flags[1] set value 1b
execute unless score #mirrored Selected matches 1 run data modify entity @s[tag=held_item,scores={AnimationProgress=56..}] ArmorItems[3] set value {}
execute store result score #temp Time run data get entity @s Pose.Head[0]
execute unless score #mirrored Selected matches 1 if entity @s[tag=held_item,scores={AnimationProgress=1}] run scoreboard players add #temp Time 180
