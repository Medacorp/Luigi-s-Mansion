...
execute unless entity @s[scores={HomeX=0,HomeZ=0}] at @s positioned ^ ^-1.65 ^.25 rotated as @e[tag=this_luigi,tag=!model_piece,limit=1] rotated ~ 0 positioned ^ ^ ^-0.5 if entity @e[tag=this_luigi,tag=!model_piece,limit=1,distance=..0.49] run tag @s add moving_backwards
