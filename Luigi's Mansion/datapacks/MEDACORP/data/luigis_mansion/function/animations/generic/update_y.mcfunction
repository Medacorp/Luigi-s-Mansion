...
execute store result score @s AnimationRotationCosY run data get entity @e[tag=temp,limit=1] Pos[2] 1000
execute as @e[tag=temp,limit=1] at @s positioned 0.001 0.0 0.0 run teleport @s ^ ^ ^-1
execute store result score @s AnimationRotationSinY run data get entity @e[tag=temp,limit=1] Pos[0] 1000
kill @e[tag=temp]