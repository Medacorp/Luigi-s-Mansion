summon minecraft:marker ~ ~ ~ {Tags:["game_boy_horror_location","new","player","same_room_players_only"],data:{entity:{namespace:"luigis_mansion",id:"game_boy_horror_location"}}}
...
execute in minecraft:overworld run data modify entity @e[tag=game_boy_horror_location,tag=new,limit=1] CustomName set from block 27 0 0 front_text.messages[0]
...
scoreboard players operation @e[tag=game_boy_horror_location,tag=new,limit=1] Health = @s Health
