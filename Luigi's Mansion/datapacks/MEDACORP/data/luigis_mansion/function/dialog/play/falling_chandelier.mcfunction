...
execute if score #dialog Dialog matches 1..39 as @a[tag=same_room] run function luigis_mansion:entities/player/camera/execute {execute:"at @e[tag=chandelier,tag=falling,scores={ActionTime=1..39},tag=same_room,limit=1]",teleport:"~4 ~-1.8 ~ 90 32"}
