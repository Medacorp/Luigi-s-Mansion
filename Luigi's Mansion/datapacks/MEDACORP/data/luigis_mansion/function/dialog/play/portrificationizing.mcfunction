...
execute if score #dialog Dialog matches 1 run function luigis_mansion:spawn_entities/portrificationizing_ghosts with storage luigis_mansion:data current_state.luigis_mansion.current_data.mansion_id
...
execute if score #dialog Dialog matches 668 if entity @e[tag=e_gadd,tag=same_room,limit=1,tag=cannot_be_removed] if entity @a[tag=same_room,tag=next_dialog_line,limit=1] at @e[tag=e_gadd,tag=same_room,limit=1] run playsound luigis_mansion:entity.e_gadd.talk.ohmoogah_luigi_odoh neutral @a[tag=same_room] ~ ~ ~ 1
...
execute if score #dialog Dialog matches 669 if entity @e[tag=e_gadd,tag=same_room,limit=1,tag=cannot_be_removed] run data modify storage luigis_mansion:data dialogs[-1].ending set from storage luigis_mansion:data current_state.luigis_mansion.current_data.ending_dialog
