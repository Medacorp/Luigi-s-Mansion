...
execute if score #dialog Dialog matches 22 if entity @a[tag=same_room,tag=select_dialog_branch_no,limit=1] run scoreboard players set #dialog Dialog -1
execute if score #dialog Dialog matches 22 if entity @a[tag=same_room,tag=select_dialog_branch_yes,limit=1] run scoreboard players set #dialog Dialog -1
execute if score #dialog Dialog matches 22 if entity @a[tag=same_room,tag=skip_dialog,limit=1] run scoreboard players set #dialog Dialog -1
...
execute if score #dialog Dialog matches 22 as @a[tag=same_room,tag=!spectator,tag=!dialog_choice_menu] unless entity @s[tag=using_selection_menu,tag=!dialog_menu] run function luigis_mansion:selection_menu/dialog/choice/underground_lab_back_to_title
...
execute if score #dialog Dialog matches -1 run stopsound @a[tag=same_room] neutral luigis_mansion:entity.e_gadd.talk.ohyah_ohyahmah_hey_hey_oui
execute if score #dialog Dialog matches -1 if entity @a[tag=select_dialog_branch_yes,limit=1] run function luigis_mansion:data/back_to_title
