...
execute if score #dialog Dialog matches ..26 unless score #dialog Dialog matches 5..25 as @a[tag=same_room,tag=!spectator,tag=!dialog_menu,tag=!using_selection_menu] run function luigis_mansion:selection_menu/dialog/original_menu
...
execute if score #dialog Dialog matches 1 if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.balcony_1{cleared:1b} run data modify storage luigis_mansion:data dialogs append value {name:{namespace:"luigis_mansion",id:"toad_1_repeat"},room:0,progress:0}
execute if score #dialog Dialog matches 1 if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.balcony_1{cleared:1b} run data modify storage luigis_mansion:data dialogs[-1].room set from storage luigis_mansion:data dialogs[0].room
execute if score #dialog Dialog matches 1 if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.balcony_1{cleared:1b} run scoreboard players set #dialog Dialog -1
...
execute if score #dialog Dialog matches 4 if entity @a[tag=same_room,tag=next_dialog_line,limit=1] run data modify entity @e[tag=toad,tag=same_room,limit=1] data.sound set value "wow"
...
execute if score #dialog Dialog matches 5 as @e[tag=toad,tag=same_room,limit=1] run function luigis_mansion:room/clear_room_mansion with storage luigis_mansion:data current_state.luigis_mansion.current_data.mansion_id
