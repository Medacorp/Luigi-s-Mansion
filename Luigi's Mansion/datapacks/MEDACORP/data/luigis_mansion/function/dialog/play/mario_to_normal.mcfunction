...
execute if score #dialog Dialog matches 1 run summon minecraft:armor_stand 771.01 76.47 -12 {Rotation:[-90.0f,0.0f],Pose:{Head:[0.0f,-180.0f,0.0f]},Tags:["mario","same_room","portrificationizing","cannot_be_removed"],Marker:1b,Invisible:1b,Silent:1b,data:{entity:{namespace:"luigis_mansion",id:"mario"}},equipment:{head:{id:"minecraft:diamond_pickaxe",count:1,components:{"minecraft:item_model":"luigis_mansion:furniture/marios_portrait","minecraft:custom_model_data":{flags:[1b,1b]}}}},DisabledSlots:2039583}
...
execute if score #dialog Dialog matches 803 as @a[tag=same_room] run function luigis_mansion:other/music/set/silence
