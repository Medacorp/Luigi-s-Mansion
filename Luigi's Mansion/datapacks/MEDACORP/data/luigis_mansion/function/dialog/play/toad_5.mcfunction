...
execute as @e[tag=luigi,tag=same_room] if score @s ID = #temp ID run tag @s add phone_caller
...
execute if score #dialog Dialog matches 22 if data storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data{telephone_1:1b} run tag @a[tag=same_room,limit=1] add select_dialog_branch_yes
execute if score #dialog Dialog matches 22 if data storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data{telephone_1:1b} run scoreboard players set #dialog Dialog 23
...
execute if score #dialog Dialog matches 23 if score #players Totals matches 2.. if entity @a[tag=same_room,tag=select_dialog_branch_yes,limit=1] run tellraw @a[tag=same_room] {type:"translatable",translate:"chat.type.text",with:[{type:"translatable",translate:"luigis_mansion:entity.unknown",color:"green"},{type:"translatable",translate:"luigis_mansion:dialog.toad_5.yes.1.more"}]}
...
execute if score #dialog Dialog matches 29 run data modify storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data merge value {telephone_2:1b}
...
execute if score #dialog Dialog matches -1 run data modify storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data merge value {telephone_1:1b}
execute if score #dialog Dialog matches -1 unless data storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data{telephone_2:1b} run tag @e[tag=telephone_2,tag=same_room,limit=1] add can_approach
execute if score #dialog Dialog matches -1 if data storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data{telephone_2:1b} run tag @e[tag=telephone_3,tag=same_room,limit=1] add can_approach
