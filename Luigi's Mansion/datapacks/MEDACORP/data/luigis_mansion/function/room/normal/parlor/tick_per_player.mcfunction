...
execute if data storage luigis_mansion:data current_state.current_data.rooms.parlor{seen:0b} if entity @s[tag=!spectator,tag=!fully_separated_camera] run function luigis_mansion:room/normal/parlor/set_seen
...
execute unless data storage luigis_mansion:data rooms.underground_lab{cleared:1b} if entity @s[tag=!spectator,tag=!door_animation] unless data storage luigis_mansion:data dialogs[{room:3}] run function luigis_mansion:room/normal/parlor/meet_e_gadd