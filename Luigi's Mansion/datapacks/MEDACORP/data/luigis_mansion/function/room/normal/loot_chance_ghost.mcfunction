...
execute if score #temp Time matches 1..15 if entity @a[tag=killer,limit=1,scores={Health=31..}] if entity @s[tag=!portrait_ghost,nbt=!{data:{entity:{namespace:"luigis_mansion",id:"boo"}}}] unless data entity @s data.loot run data modify entity @s data.loot.contents.luigis_mansion.small_heart set value 1
execute if score #temp Time matches 1..50 if entity @a[tag=killer,limit=1,scores={Health=..30}] if entity @s[tag=!portrait_ghost,nbt=!{data:{entity:{namespace:"luigis_mansion",id:"boo"}}}] unless data entity @s data.loot run data modify entity @s data.loot.contents.luigis_mansion.small_heart set value 1
