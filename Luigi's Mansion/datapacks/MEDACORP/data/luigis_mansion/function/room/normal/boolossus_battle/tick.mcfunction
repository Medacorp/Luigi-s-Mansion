...
execute if data storage luigis_mansion:data current_state.luigis_mansion.current_data.rooms.balcony_2{cleared:0b} as @a unless entity @s[scores={Room=75}] run function luigis_mansion:room/normal/boolossus_battle/warp_to

execute if entity @e[tag=luigi,tag=!door_animation,scores={Room=75},limit=1] run function luigis_mansion:room/normal/boolossus_battle/ghosts