...
execute as @a[x=634,y=92,z=-27,dx=20,dy=7,dz=18] unless entity @s[scores={Room=72}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=634,y=92,z=-27,dx=20,dy=7,dz=18] run scoreboard players set @s Room 72
...
execute as @a[x=648.5,y=93,z=-9.5,distance=..0.7,tag=!spectator,limit=1] unless data storage luigis_mansion:data current_state.current_data.technical_data{saw_mario_again:1b} unless data storage luigis_mansion:data dialogs[{room:72}] run function luigis_mansion:room/normal/secret_altar/look_at_marios_painting