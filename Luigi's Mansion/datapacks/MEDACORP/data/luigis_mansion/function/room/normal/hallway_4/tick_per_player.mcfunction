...
execute if data storage luigis_mansion:data current_state.current_data.rooms.hallway_4{seen:0b} if entity @s[tag=!spectator,tag=!fully_separated_camera] run function luigis_mansion:room/normal/hallway_4/set_seen
...
execute unless data storage luigis_mansion:data current_state.current_data.rooms.dining_room{cleared:1b} if data storage luigis_mansion:data current_state.current_data{blackout:0b} run function luigis_mansion:entities/player/run_command_at_model {command:'execute unless entity @e[tag=this_luigi,limit=1,x=713,y=10,z=-35,dx=7,dy=8,dz=60] run function luigis_mansion:room/normal/hallway_4/dining_room_noise'}