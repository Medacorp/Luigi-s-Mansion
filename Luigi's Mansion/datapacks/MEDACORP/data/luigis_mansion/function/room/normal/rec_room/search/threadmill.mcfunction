...
execute unless score #rec_room_threadmill Searched matches 1 unless data storage luigis_mansion:data current_state.current_data{obtained_keys:["rec_room"]} run data modify storage luigis_mansion:data furniture.loot set value {contents:{luigis_mansion:{key:[{door:"hallway_8",rotation:[-180.0f,0.0f]}]}}}
