execute unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"item"},item:{namespace:"luigis_mansion",id:"key"},variant:"hallway_8"}},limit=1] unless data storage luigis_mansion:data current_state.current_data{obtained_keys:["hallway_8"]} run scoreboard players set #temp Searched 1
