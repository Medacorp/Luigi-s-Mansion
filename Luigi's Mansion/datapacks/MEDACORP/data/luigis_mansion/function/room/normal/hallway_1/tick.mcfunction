...
execute unless data storage luigis_mansion:data dialogs[{room:1}] unless data storage luigis_mansion:data current_state.luigis_mansion.current_data{obtained_keys:["parlor"]} unless entity @e[nbt={data:{item:{namespace:"luigis_mansion",id:"key"},variant:"parlor"}},limit=1] if entity @e[x=736,y=19,z=8,dx=0,dy=3,dz=1,tag=door,scores={AnimationProgress=39}] run playsound luigis_mansion:entity.ghost.mumble hostile @a 751.0 13 9.0 3
...
execute if score #temp Wave matches ..7 if data storage luigis_mansion:data current_state.luigis_mansion.rooms.underground_lab{cleared:1b} unless data storage luigis_mansion:data current_state.luigis_mansion.current_data{money_spawned:["hallway_1_money"]} run data modify storage luigis_mansion:data current_state.luigis_mansion.current_data.money_spawned append value "hallway_1_money"
