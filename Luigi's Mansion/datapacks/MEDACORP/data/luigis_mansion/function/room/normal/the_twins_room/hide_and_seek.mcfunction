...
execute at @e[tag=ghost,tag=portrait_ghost,type=minecraft:marker,scores={Room=49},tag=spawn,tag=!remove_from_existence] run function luigis_mansion:room/normal/the_twins_room/right_box
...
execute unless score #the_twins_room Time matches 1.. at @e[tag=hide_and_seek_box,tag=open,scores={Room=49}] unless entity @e[distance=..0.7,tag=ghost,limit=1] unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"henry"}}},limit=1] run scoreboard players add #the_twins_room Time 1
...
execute if score #the_twins_room Time matches 40 at @e[tag=ghost,tag=portrait_ghost,type=minecraft:marker,scores={Room=49},tag=empty_marker] run function luigis_mansion:room/normal/the_twins_room/right_box
...
execute if score #the_twins_room Time matches 120 run function luigis_mansion:room/normal/the_twins_room/wrong_box