...
execute if data storage luigis_mansion:data current_state.current_data.rooms.hallway_22{seen:0b} if entity @s[tag=!spectator,tag=!fully_separated_camera] run function luigis_mansion:room/normal/hallway_22/set_seen
...
function luigis_mansion:entities/player/run_command_as_model {command:'execute if entity @s[x=655.0,y=1,z=-21.0,dx=4,dy=7,dz=7] unless score #temp Boos matches 40.. unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"king_boo"}}},scores={Room=61},limit=1] run data modify storage luigis_mansion:data entity set value {room:61,show_health:0b}'}
function luigis_mansion:entities/player/run_command_as_model {command:'execute if entity @s[x=655.0,y=1,z=-21.0,dx=4,dy=7,dz=7] unless score #temp Boos matches 40.. unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"king_boo"}}},scores={Room=61},limit=1] positioned 656 2 -18 rotated -90 0 run function luigis_mansion:spawn_entities/portrait_ghost/king_boo'}
scoreboard players reset #temp Boos
function luigis_mansion:entities/player/run_command_as_model {command:'execute if entity @s[x=655.5,y=2,z=-17.5,distance=..4] if data storage luigis_mansion:data current_state.current_data{obtained_keys:["secret_altar"]} run function luigis_mansion:room/normal/hallway_22/clear_blockade'}