...
execute unless score #foyer_table Searched matches 1 as @e[tag=luigi] if score #foyer_table SearcherID = @s ID run tag @s add searcher
execute unless score #foyer_table Searched matches 1 run function luigis_mansion:entities/furniture/loot_chance/h70_b10
execute unless score #foyer_table Searched matches 1 run tag @e[tag=searcher] remove searcher
