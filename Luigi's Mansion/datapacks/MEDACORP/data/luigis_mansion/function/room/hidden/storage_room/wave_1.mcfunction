...
execute unless data storage luigis_mansion:data current_state.current_data{money_spawned:["storage_room_speedy_spirit"]} run data modify storage luigis_mansion:data entity.loot.drop_at_0.contents set from storage luigis_mansion:data current_state.current_data.loot_collections.speedy_spirit.silver_diamond
