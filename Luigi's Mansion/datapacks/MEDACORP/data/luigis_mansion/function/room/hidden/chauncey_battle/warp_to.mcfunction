...
execute unless score #chauncey_battle Ticking matches 1 unless data storage luigis_mansion:data dialogs[{name:{namespace:"luigis_mansion",id:"hidden/chauncey_opening"}}] run data modify storage luigis_mansion:data dialogs append value {name:{namespace:"luigis_mansion",id:"hidden/chauncey_opening"},progress:0,room:73}
