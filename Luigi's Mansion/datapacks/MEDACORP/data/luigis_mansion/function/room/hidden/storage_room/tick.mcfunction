...
execute as @a[x=679,y=10,z=52,dx=12,dy=7,dz=33] unless entity @s[scores={Room=18}] run scoreboard players operation @s LastRoom = @s Room
scoreboard players set @a[x=679,y=10,z=52,dx=12,dy=7,dz=33] Room 18
#todelete - old furniture
scoreboard players set #temp Room 18
#/todelete

execute as @e[scores={Room=18}] unless entity @s[type=!minecraft:armor_stand,type=!minecraft:item_frame] if score #global_mirror_reflections Selected matches 1 run scoreboard players set @s MirrorX 680
...
execute if entity @a[tag=!pause_dialog,scores={Room=18},tag=!spectator,limit=1] run function luigis_mansion:room/hidden/storage_room/ghosts