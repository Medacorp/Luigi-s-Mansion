...
execute unless entity @s[scores={MapZoomTime=1..}] unless entity @s[predicate=luigis_mansion:movement/stand_still] run playsound luigis_mansion:item.game_boy_horror.map.move player @s[scores={MapSound=0}] ~ ~ ~ 1
execute unless entity @s[scores={MapZoomTime=1..}] unless entity @s[predicate=luigis_mansion:movement/stand_still] run scoreboard players set @s[scores={MapSound=0}] MapSound 7
