...
$execute if entity @s[scores={Dialog=1}] unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"bowser_head"},owner:$(UUID)}},tag=same_room,limit=1] run data modify storage luigis_mansion:data entity set value {damage:{},owner:$(UUID),tags:["decapitate","decapitate_move_up","can_decapitate"],room_search:{non_players:1b}}
...
$execute if entity @s[scores={Dialog=1}] unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"bowser_head"},owner:$(UUID)}},tag=same_room,limit=1] positioned ^ ^3.5 ^-0.4 run function luigis_mansion:spawn_entities/bowser/head
