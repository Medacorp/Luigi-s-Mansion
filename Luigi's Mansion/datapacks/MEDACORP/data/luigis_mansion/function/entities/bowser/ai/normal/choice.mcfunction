...
execute if score #temp Time matches -3..3 unless entity @s[scores={ActionTime=1..}] unless entity @a[distance=..12,tag=!spectator] if entity @a[distance=12..,tag=!spectator] unless entity @e[tag=spike_ball,limit=1] run tag @s add throw_spike_balls
