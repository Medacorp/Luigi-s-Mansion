...
execute if score #temp Time matches -3..3 unless entity @s[scores={ActionTime=1..}] if entity @e[distance=..12,tag=luigi,limit=1] run function luigis_mansion:entities/bowser/select_attack
execute if score #temp Time matches -3..3 unless entity @s[scores={ActionTime=1..}] unless entity @e[distance=..12,tag=luigi,limit=1] if entity @e[distance=12..,tag=luigi,limit=1] unless entity @e[tag=spike_ball,tag=same_room,limit=1] run tag @s add throw_fast_spike_balls
execute if score #temp Time matches -3..3 unless entity @s[scores={ActionTime=1..}] unless entity @e[distance=..12,tag=luigi,limit=1] if entity @e[distance=12..,tag=luigi,limit=1] if entity @e[tag=spike_ball,tag=same_room,limit=1] run function luigis_mansion:entities/bowser/select_move
...
execute unless score #temp Time matches -90..90 unless entity @s[scores={ActionTime=1..}] positioned ^ ^ ^-4 if entity @e[tag=same_room,tag=luigi,distance=..3,limit=1] run tag @s add tail_attack
