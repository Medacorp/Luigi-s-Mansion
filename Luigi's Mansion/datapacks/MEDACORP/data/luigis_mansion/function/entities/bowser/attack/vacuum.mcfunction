...
scoreboard players reset #temp Time
execute if entity @s[scores={ActionTime=21..30}] at @a[tag=got_sucked,distance=..4,sort=nearest,limit=1] run function luigis_mansion:entities/ghost/set_target
...
execute if entity @s[scores={ActionTime=90}] as @a[tag=target,limit=1] run function luigis_mansion:entities/player/damage
