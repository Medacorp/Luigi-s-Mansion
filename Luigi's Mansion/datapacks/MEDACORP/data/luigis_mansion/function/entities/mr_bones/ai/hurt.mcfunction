...
scoreboard players set @s[scores={ElementHurtTime=0},tag=element_hurt,tag=!freeze] Sound 10
...
scoreboard players set @s[scores={Sound=0},tag=element_hurt,tag=!freeze] Sound 10

data modify entity @s[tag=flee,tag=!second_flee_state] data.animation set value {namespace:"luigis_mansion",id:"flee"}
execute if entity @s[tag=flee,tag=second_flee_state] run function luigis_mansion:entities/mr_bones/second_flee_state with entity @s data.entity
