...
execute if score #temp Time matches 1..25000 unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"king_boo"}}},tag=same_room,limit=1] run tag @s add reattach_backwards
execute if score #temp Time matches 25001.. unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"king_boo"}}},tag=same_room,limit=1] run tag @s add reattach
