...
execute if entity @s[tag=must_face] store result score #temp Time run data get entity @e[tag=searcher,limit=1] Rotation[0]
scoreboard players add #temp Time 180
execute if score #temp Time matches 180.. run scoreboard players remove #temp Time 360
execute if entity @s[tag=must_face,tag=!inverted_check] if score #temp Time < @s RotationX run tag @e[tag=searcher,limit=1] add not_facing
execute if entity @s[tag=must_face,tag=!inverted_check] if score #temp Time > @s RotationY run tag @e[tag=searcher,limit=1] add not_facing
