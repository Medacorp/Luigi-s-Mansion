...
execute at @s[x_rotation=-84..90] run teleport @s ~ ~ ~ ~ ~-10
execute at @s[x_rotation=-90..-86] run teleport @s ~ ~ ~ ~ -85
...
execute store result score @s HomeRotationY run data get entity @s Rotation[0]
...
execute if entity @s[tag=moist] as @e[tag=temp,limit=1] at @s run function luigis_mansion:entities/furniture/spawn/particles/direct_water {power:0.6f}
