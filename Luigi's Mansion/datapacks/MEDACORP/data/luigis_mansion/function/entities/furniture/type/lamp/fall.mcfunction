...
execute at @s[scores={ActionTime=1..51}] align xz if entity @e[tag=luigi,distance=..2,scores={Invulnerable=0}] run tag @s add hit
...
execute at @s[scores={ActionTime=1..52}] align xz as @e[tag=luigi,distance=..2] run function luigis_mansion:entities/luigi/damage
