...
tag @s add vacuuming_cloth
$execute unless entity @e[tag=me,limit=1,scores={FurnitureSearch=40..}] at @s[scores={TeleportDelayTimer=0}] facing $(facing_x) $(facing_y) $(facing_z) positioned $(facing_x) $(facing_y) $(facing_z) run function luigis_mansion:entities/furniture/type/cloth/teleport with storage luigis_mansion:data macro
