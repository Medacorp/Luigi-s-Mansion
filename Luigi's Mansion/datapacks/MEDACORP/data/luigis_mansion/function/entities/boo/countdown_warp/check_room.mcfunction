...
$execute unless score #$(name) Ticking matches 1 run function $(namespace):room/$(id)/$(name)/load_boos
$execute unless score #$(name) Ticking matches 1 run function #$(namespace):room/$(id)/$(name)/load
...
$execute if data storage luigis_mansion:data temp{value:-1} if entity @s[tag=!can_hide] run scoreboard players set @s Room $(fallback_room)
