...
execute unless entity @s[scores={Offline=0}] if entity @s[tag=!using_selection_menu] run function luigis_mansion:selection_menu/mansion_reset/original_menu
...
execute if entity @s[tag=dead_player] if score #can_revive Selected matches 1 unless entity @e[tag=this_death_location,limit=1] run function luigis_mansion:room/underground_lab/warp_to
execute at @s[tag=dead_player] if score #can_revive Selected matches 1 unless entity @e[tag=this_death_location,limit=1] in minecraft:overworld positioned 790 77 15 rotated 45 0 run function luigis_mansion:spawn_entities/death_location
