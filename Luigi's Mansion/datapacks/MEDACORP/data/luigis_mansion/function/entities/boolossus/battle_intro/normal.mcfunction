...
execute if entity @s[scores={Dialog=1..99}] as @a[tag=same_room,tag=!spectator,tag=!looking_at_map] unless entity @s[scores={AnimationProgress=1..}] run function luigis_mansion:entities/player/animation/set/high_health_idle_no_sound
...
execute if entity @s[scores={Dialog=260..349}] run scoreboard players set @a[tag=same_room,tag=!spectator] AnimationProgress 30
