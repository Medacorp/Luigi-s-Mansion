execute if entity @s[tag=!remove_from_existence] unless data storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data{mysterious_power:1b} unless data storage luigis_mansion:data dialogs[{room:52}] run data modify storage luigis_mansion:data dialogs append value {name:{namespace:"luigis_mansion",id:"mysterious_power"},room:52,progress:0}
