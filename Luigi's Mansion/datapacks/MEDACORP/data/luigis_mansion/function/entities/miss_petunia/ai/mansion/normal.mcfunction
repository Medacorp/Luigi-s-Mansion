...
execute if entity @s[scores={Dialog=61},tag=in_ice] unless data storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data{miss_petunia_spoke:1b} run tellraw @a[tag=same_room] {type:"translatable",translate:"chat.type.text",with:[{type:"translatable",translate:"luigis_mansion:entity.miss_petunia",color:"green"},{type:"translatable",translate:"luigis_mansion:message.miss_petunia.freeze"}]}
execute if entity @s[scores={Dialog=61},tag=in_ice] run data modify storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data merge value {miss_petunia_spoke:1b}
