...
execute if entity @s[tag=in_dust,tag=!dead,scores={WaitTime=110..}] positioned ~ ~-0.5 ~ facing entity @a[tag=expelling_me,sort=nearest,limit=1] feet positioned ~ ~0.5 ~ if block ^ ^ ^-0.3 #luigis_mansion:ghosts_ignore run teleport @s ^ ^ ^-0.1
execute if entity @s[tag=in_vacuum,tag=!dead,scores={WaitTime=110..}] run scoreboard players operation #temp Move /= #2 Constants
execute if entity @s[tag=in_vacuum,tag=!dead,scores={WaitTime=110..}] positioned ~ ~-0.5 ~ facing entity @a[tag=vacuuming_me,sort=nearest,limit=1] feet positioned ~ ~0.5 ~ run function luigis_mansion:entities/ghost/move_forward
