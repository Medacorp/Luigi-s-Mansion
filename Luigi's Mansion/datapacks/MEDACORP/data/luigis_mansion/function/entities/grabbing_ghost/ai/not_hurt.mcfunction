...
execute if entity @e[tag=exact_same_room,tag=luigi,tag=!grabbed,distance=..0.7,limit=1] if entity @s[tag=!vanish,tag=!appear,tag=!attack,scores={AttackType=0..1}] run function luigis_mansion:entities/grabbing_ghost/collide with entity @s data.entity
