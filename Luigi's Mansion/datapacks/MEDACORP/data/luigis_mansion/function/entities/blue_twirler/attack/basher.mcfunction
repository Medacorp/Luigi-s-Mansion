...
execute if entity @s[scores={ActionTime=41}] run summon minecraft:text_display ~ ~0.75 ~ {CustomName:{type:"translatable",translate:"luigis_mansion:message.basher_scare"},text:{type:"translatable",translate:"luigis_mansion:message.basher_scare",color:"yellow",bold:true},brightness:{sky:15,block:15},Tags:["basher_scare"],data:{entity:{namespace:"luigis_mansion",id:"basher_scare"}}}
execute if entity @s[scores={ActionTime=41}] if entity @e[distance=..5,tag=luigi] run tag @s add laugh
