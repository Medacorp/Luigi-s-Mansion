...
execute if entity @s[scores={Dialog=131},tag=!attack,tag=!laugh,tag=!complain] unless entity @e[tag=haunted_teddy_bear,tag=same_room,scores={WaitTime=60..}] positioned ^ ^ ^0.7 if entity @e[tag=same_room,tag=!spectator,tag=player,distance=..0.7,limit=1] run tag @s add attack
