...
execute at @s[scores={ActionTime=11}] positioned ^ ^ ^0.5 if entity @e[tag=luigi,distance=..1,scores={Invulnerable=0}] run tag @s add laugh
execute at @s[scores={ActionTime=11}] run data modify storage luigis_mansion:data damage set value {method:{namespace:"luigis_mansion",id:"punch"},amount:0,animation:{namespace:"luigis_mansion",id:"knockback/fall"},no_delete:1b}
...
execute at @s[scores={ActionTime=11}] run data modify storage luigis_mansion:data damage.attacker set from entity @s UUID
execute at @s[scores={ActionTime=11}] positioned ^ ^ ^0.5 as @e[tag=luigi,distance=..1] run function luigis_mansion:entities/luigi/damage
