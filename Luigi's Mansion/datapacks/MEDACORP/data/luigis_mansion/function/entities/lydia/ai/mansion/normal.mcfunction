...
execute if entity @s[scores={Dialog=60}] unless data storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data{lydia_spoke:1b} run tellraw @a[tag=same_room] {type:"translatable",translate:"chat.type.text",with:[{type:"translatable",translate:"luigis_mansion:entity.lydia",color:"green"},{type:"translatable",translate:"luigis_mansion:message.lydia.draft"}]}
execute if entity @s[scores={Dialog=60}] run data modify storage luigis_mansion:data current_state.luigis_mansion.current_data.technical_data merge value {lydia_spoke:1b}
