execute unless block ~ ~ ~ #luigis_mansion:ghosts_ignore if block ^ ^ ^1 #luigis_mansion:ghosts_ignore run teleport @s ^ ^ ^0.01
execute if block ^ ^ ^0.6 #luigis_mansion:ghosts_ignore run teleport @s ^ ^ ^0.01
execute if block ^ ^ ^0.6 #minecraft:slabs[type=bottom] run teleport @s ^ ^ ^0.01
...
execute if score #temp Move matches 1.. positioned as @s unless entity @e[tag=target_pos,distance=..0.1] run function luigis_mansion:entities/wool/move_forward