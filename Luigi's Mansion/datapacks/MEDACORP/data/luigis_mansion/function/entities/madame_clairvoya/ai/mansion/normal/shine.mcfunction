...
scoreboard players add @s[scores={ActionTime=..39}] ActionTime 1
execute unless entity @s[scores={ActionTime=0..}] run scoreboard players set @s ActionTime 0
execute if entity @e[tag=was_illuminated_furniture,scores={Room=20}] run tag @s add illuminate_furniture
...
execute if entity @s[scores={ActionTime=1..40},tag=!illuminate_furniture] run stopsound @a[tag=same_room] block luigis_mansion:furniture.crystal_ball.shine
