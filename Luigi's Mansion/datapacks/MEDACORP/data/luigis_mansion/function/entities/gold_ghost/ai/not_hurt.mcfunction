...
execute if entity @e[tag=exact_same_room,tag=!spectator,tag=player,distance=..0.7,limit=1] if entity @s[tag=!vanish,tag=!appear] unless entity @s[scores={AttackType=1}] run function luigis_mansion:entities/gold_ghost/collide with entity @s data.entity
