...
scoreboard players set @s[scores={ElementHurtTime=0},tag=element_hurt,tag=!freeze] Sound 30
$execute if entity @s[scores={Sound=0},tag=element_hurt,tag=!freeze] run playsound $(namespace):entity.$(id).element_hurt hostile @a[tag=same_room] ~ ~ ~ 1
scoreboard players set @s[scores={Sound=0},tag=element_hurt,tag=!freeze] Sound 30

data modify entity @s[tag=flee,tag=!second_flee_state] data.animation set value {namespace:"luigis_mansion",id:"flee"}
execute if entity @s[tag=flee,tag=second_flee_state] run function luigis_mansion:entities/ghost_guy/second_flee_state with entity @s data.entity
