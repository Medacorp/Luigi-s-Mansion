...
execute if entity @s[scores={Dialog=1..99}] as @a[tag=same_room,tag=!spectator,tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/idle
...
execute if entity @s[scores={Dialog=60..90}] run scoreboard players set @a[x=651.5,y=103,z=-31.5,distance=..6,scores={Invulnerable=0..1}] Invulnerable 20
...
execute if entity @s[scores={Dialog=100}] as @a[tag=same_room] run function luigis_mansion:entities/player/animation/set/none
