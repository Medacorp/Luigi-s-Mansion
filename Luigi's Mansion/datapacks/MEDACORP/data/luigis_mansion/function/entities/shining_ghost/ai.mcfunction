tag @s add visible
particle minecraft:dust{color:16771072,scale:1f} ~ ~ ~ 0 0 0 1 1 normal @a[tag=same_room]

execute at @s[tag=!in_vacuum,tag=!can_spit_2,tag=!spit,tag=!dead] if entity @e[tag=same_room,tag=luigi,distance=..0.7,limit=1] run function luigis_mansion:entities/shining_ghost/collide with entity @s data.entity
...
execute at @s[tag=in_vacuum,tag=!spit] run scoreboard players operation @s KillerID = @e[distance=..1.5,tag=luigi,tag=vacuuming_me,sort=nearest,limit=1] ID
execute at @s[tag=in_vacuum,tag=!spit] if entity @e[distance=..1.5,tag=luigi,tag=vacuuming_me,sort=nearest,limit=1] run tag @s add can_spit
