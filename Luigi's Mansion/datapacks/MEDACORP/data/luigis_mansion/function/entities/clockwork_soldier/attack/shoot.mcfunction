...
execute at @s[scores={ActionTime=20}] positioned ^ ^-1 ^0.7 if entity @e[tag=luigi,distance=..1,scores={Invulnerable=0}] run scoreboard players set @s WaitTime 40
...
execute at @s[scores={ActionTime=20}] positioned ^ ^-1 ^0.7 as @e[tag=luigi,distance=..1] run function luigis_mansion:entities/luigi/damage
