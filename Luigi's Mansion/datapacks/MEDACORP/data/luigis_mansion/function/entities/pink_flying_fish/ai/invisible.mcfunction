...
$execute unless entity @s[scores={WaitTime=1..}] unless score #temp ActionTime matches 3.. unless entity @e[distance=..0.7,tag=!spectator,tag=player,limit=1] unless entity @e[nbt={data:{entity:{namespace:"$(namespace)",id:"$(id)"}}},tag=visible,distance=..0.7] run tag @s add visible
