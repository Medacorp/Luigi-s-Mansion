execute if entity @a[tag=vacuuming_me,distance=..2.99,limit=1] facing entity @p[tag=!spectator,tag=vacuuming_me] feet run teleport @s[scores={FleeTime=1}] ~ ~ ~ ~-180 0
execute unless entity @a[tag=vacuuming_me,distance=..2.99,limit=1] facing entity @p[tag=!spectator,tag=vacuuming_me] feet run teleport @s[scores={FleeTime=1}] ~ ~ ~ ~ 0
...
execute at @s[tag=out_of_range,tag=!was_out_of_range] facing entity @p[tag=!spectator,tag=vacuuming_me] feet run teleport @s ~ ~ ~ ~ 0
