...
execute if entity @s[scores={DeathTime=1}] as @a[tag=vacuuming_me] run function luigis_mansion:entities/ghost/find_shooters
execute if entity @s[scores={DeathTime=1}] if entity @a[tag=!spectator,tag=shooter] run tag @a[tag=vacuuming_me,tag=!shooter] remove vacuuming_me
execute if entity @s[scores={DeathTime=1}] if entity @a[tag=!spectator,tag=shooter] run scoreboard players operation @s KillerID = @a[sort=furthest,limit=1,tag=!spectator,tag=vacuuming_me] ID
