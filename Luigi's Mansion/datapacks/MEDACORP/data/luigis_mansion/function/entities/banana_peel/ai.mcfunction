...
tag @s[scores={SpawnTime=..90}] remove in_vacuum
tag @s[scores={SpawnTime=90}] add vacuumable
tag @s[scores={SpawnTime=600},tag=!eternal] add dead
execute if entity @s[tag=in_vacuum] facing entity @a[tag=vacuuming_me,sort=nearest,limit=1] feet positioned ~ ~0.6 ~ run function luigis_mansion:entities/banana_peel/pull
...
execute at @s[tag=!captured,tag=!dead] if entity @e[tag=!spectator,tag=player,distance=..0.7,limit=1] run function luigis_mansion:entities/banana_peel/slip