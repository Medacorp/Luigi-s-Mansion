...
execute unless block ~ ~0.2 ~ #luigis_mansion:ghosts_ignore if block ~ ~-0.2 ~ #luigis_mansion:ghosts_ignore run tag @s add flipped_gravity
execute if block ~ ~0.2 ~ #luigis_mansion:ghosts_ignore unless block ~ ~-0.2 ~ #luigis_mansion:ghosts_ignore run tag @s remove flipped_gravity
...
data remove entity @s[x_rotation=-45..45] data.animation{namespace:"luigis_mansion",id:"idle_wall"}
...
execute if entity @s[tag=reached_target] run function luigis_mansion:entities/blue_mouse/get_new_path