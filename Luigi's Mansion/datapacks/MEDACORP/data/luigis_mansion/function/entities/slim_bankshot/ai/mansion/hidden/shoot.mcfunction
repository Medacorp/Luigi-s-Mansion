...
execute if entity @s[scores={ActionTime=30}] positioned 681 10.8125 -64 unless entity @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"billiards_ball"}}},distance=..0.7,limit=1] positioned ~ ~1.1875 ~ rotated -15 0 run function luigis_mansion:spawn_entities/ball/billiards/white
...
execute if entity @s[scores={ActionTime=70}] run tag @e[nbt={data:{entity:{namespace:"luigis_mansion",id:"billiards_ball"}}},tag=white,limit=1] add move
