...
execute unless entity @s[scores={ForceRadar=1}] if entity @s[nbt={Inventory:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"game_boy_horror"}}}]}] run function #luigis_mansion:items/game_boy_horror/radar_type
...
execute if entity @s[scores={UseItem=1..},tag=game_boy_horror_selected,tag=!show_credits,tag=!using_selection_menu] run function luigis_mansion:selection_menu/game_boy_horror/original_menu
