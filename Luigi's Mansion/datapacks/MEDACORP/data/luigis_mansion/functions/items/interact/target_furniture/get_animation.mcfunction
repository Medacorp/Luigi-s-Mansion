...
execute if entity @s[tag=hit] if entity @e[tag=interact,tag=manual,limit=1,tag=feet] as @a[tag=searcher,limit=1] unless entity @s[scores={Animation=1..}] run function luigis_mansion:entities/player/animation/set/hump
tag @s remove hit