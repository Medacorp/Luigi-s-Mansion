...
execute store result score #temp Time run data get storage luigis_mansion:data inventory[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"flashlight"}}}].tag.CustomModelData
execute store result storage luigis_mansion:data inventory[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"flashlight"}}}].tag.CustomModelData int 1 run scoreboard players remove #temp Time 1
