...
execute at @s run function luigis_mansion:old_animations/twirler/body_slam
...
execute at @s[scores={ActionTime=40..60}] run data modify storage luigis_mansion:data damage set value {method:{namespace:"luigis_mansion",id:"body_slam"},amount:0,animation:"knockback/large",attacker:-1,no_delete:1b}
execute at @s[scores={ActionTime=40..60}] run data modify storage luigis_mansion:data damage.amount set from entity @s ArmorItems[3].tag.luigis_mansion.damage.attack
