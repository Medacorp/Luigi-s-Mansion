...
execute at @s[scores={Dialog=945,PathStep=10}] run data modify storage luigis_mansion:data damage set value {method:{namespace:"luigis_mansion",id:"bounced_on"},amount:10,animation:{namespace:"luigis_mansion",id:"knockback/large"},attacker:-1,no_delete:1b}
execute at @s[scores={Dialog=945,PathStep=10}] store result storage luigis_mansion:data damage.attacker int 1 run scoreboard players get @s GhostNr
execute at @s[scores={Dialog=945,PathStep=10}] run advancement grant @a[distance=..1,gamemode=!spectator] only luigis_mansion:vanilla/waluigi_pinball/chauncey
execute at @s[scores={Dialog=945,PathStep=10}] as @a[distance=..1,gamemode=!spectator] run function luigis_mansion:entities/player/damage
