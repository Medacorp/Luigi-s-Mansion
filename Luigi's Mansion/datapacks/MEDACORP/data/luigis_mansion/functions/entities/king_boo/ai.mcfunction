...
execute if entity @s[tag=!dying] unless entity @s[tag=!in_vacuum,tag=!hurt] run function luigis_mansion:entities/king_boo/ai/hurt
tag @s remove is_pulled

execute at @s run function luigis_mansion:entities/ghost/ai
tag @s remove flee

execute if entity @s[scores={Sound=0,ActionTime=0},tag=!dying,tag=!vanish,tag=!hidden] run function luigis_mansion:entities/king_boo/ambient_sound
