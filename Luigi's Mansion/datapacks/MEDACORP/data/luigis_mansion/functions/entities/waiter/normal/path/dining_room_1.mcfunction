...
execute if entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"mr_luggs"}}}]},scores={Dialog=..100}] run tag @s[scores={PathStep=5}] add placing_food
execute if entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"mr_luggs"}}}]},scores={Dialog=101..}] run tag @s[scores={PathStep=5}] add complain
scoreboard players set @s[scores={PathStep=5}] AnimationProgress 0
execute if entity @s[scores={PathStep=7}] run scoreboard players set @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"mr_luggs"}}}]},scores={Dialog=2..99},limit=1] Dialog 1
