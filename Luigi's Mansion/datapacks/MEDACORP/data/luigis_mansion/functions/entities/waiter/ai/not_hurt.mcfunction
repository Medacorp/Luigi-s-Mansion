...
execute unless data entity @s data.path.steps[0] if entity @s[scores={TargetTask=0,ActionTime=0,WaitTime=0}] run function luigis_mansion:entities/ghost/set_random_path
execute if data entity @s data.path.steps[0] run scoreboard players set @s[scores={TargetTask=0,ActionTime=0,WaitTime=0}] TargetTask 5

execute if entity @e[tag=exact_same_room,tag=!spectator,tag=player,distance=..0.7,limit=1] if entity @s[tag=!vanish] run function luigis_mansion:entities/waiter/collide with entity @s data.entity
