...
execute as @e[tag=ghost,tag=hurt] unless entity @s[nbt={ArmorItems:[{tag:{namespace:"luigis_mansion",id:"king_boo"}}]},tag=!dying] unless entity @s[nbt={ArmorItems:[{tag:{namespace:"luigis_mansion",id:"boo"}}]},tag=!dying] if score @s Room = #temp Room run function luigis_mansion:items/poltergust_3000/attacking_ghost
