execute unless score #gate ActionTime matches 20 if entity @a[x=771.5,y=90,z=8.0,distance=..7,tag=!spectator,limit=1] run scoreboard players add #gate ActionTime 1
execute if score #gate ActionTime matches 1.. unless entity @a[x=771.5,y=90,z=8.0,distance=..7,tag=!spectator,limit=1] run scoreboard players remove #gate ActionTime 1
...
execute as @a[x=771.5,y=90,z=8.0,distance=..7,tag=!spectator,limit=1] at @s if block ~ ~ ~ minecraft:gravel run teleport @s ~ ~0.0625 ~