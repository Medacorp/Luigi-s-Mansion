...
execute unless entity @e[nbt={ArmorItems:[{tag:{namespace:"luigis_mansion",id:"boolossus"}}]},tag=talk,limit=1] run tag @p[tag=!spectator,tag=player,distance=..2] add boolossus_target
execute unless entity @e[nbt={ArmorItems:[{tag:{namespace:"luigis_mansion",id:"boolossus"}}]},tag=talk,limit=1] if entity @a[tag=target,limit=1] run tag @s add talk
