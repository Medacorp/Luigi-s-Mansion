...
execute if score #temp Move matches -1 at @s if block ~ ~2 ~ #luigis_mansion:ghosts_ignore run teleport @s ~ ~0.1 ~
execute if score #temp Move matches -1 at @s if block ~ ~1 ~ #luigis_mansion:ghosts_ignore run teleport @s[tag=vacuumable] ~ ~0.1 ~
