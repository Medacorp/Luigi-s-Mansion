...
execute if score #temp Move matches ..-1 at @s[tag=!ground_bound] facing entity @p[tag=!spectator,tag=player,tag=vacuuming_this_ghost,tag=is_pulling] feet rotated ~ 0 run function luigis_mansion:entities/ghost/move_pulled
execute if score #temp Move matches ..-1 at @s[tag=ground_bound] facing entity @p[tag=!spectator,tag=player,tag=vacuuming_this_ghost,tag=is_pulling] feet rotated ~ 0 run function luigis_mansion:entities/ghost/move_pulled_ground_bound
