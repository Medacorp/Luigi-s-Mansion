...
execute if entity @s[scores={ActionTime=10}] run summon minecraft:armor_stand ^ ^0.2 ^ {CustomName:'{"translate":"luigis_mansion:entity.bowling_ball"}',Tags:["bowling_ball","held"],ArmorItems:[{},{},{},{id:"minecraft:diamond_pickaxe",Count:1b,tag:{Unbreakable:1b,Damage:4,CustomModelData:1}}],Pose:{Head:[0.0f,0.0f,0.01f]},Invisible:1b,NoGravity:1b,DisabledSlots:2039583}
execute if entity @s[scores={ActionTime=10}] positioned ^ ^0.2 ^ run data modify entity @e[tag=bowling_ball,distance=..0.1,limit=1] ArmorItems[3].tag.damage set from entity @s ArmorItems[3].tag.damage
execute if entity @s[scores={ActionTime=10}] positioned ^ ^0.2 ^ as @e[distance=..0.1,tag=bowling_ball,limit=1] unless entity @s[scores={GhostNr=1..}] positioned ^ ^-0.2 ^ run scoreboard players operation @s Owner = @e[nbt={ArmorItems:[{tag:{namespace:"luigis_mansion",id:"bowling_ghost"}}]},distance=..0.1,limit=1] GhostNr
