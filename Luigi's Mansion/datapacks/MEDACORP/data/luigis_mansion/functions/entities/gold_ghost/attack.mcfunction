...
execute if entity @s[scores={ActionTime=41}] run playsound luigis_mansion:entity.gold_ghost.attack hostile @a[tag=same_room] ~ ~ ~ 1
...
execute at @s[scores={ActionTime=41}] run data modify storage luigis_mansion:data damage set value {method:{namespace:"luigis_mansion",id:"punch"},amount:0,animation:"knockback/large",attacker:-1,no_delete:1b}
