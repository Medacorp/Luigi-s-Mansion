...
execute if entity @s[scores={Dialog=1..1271}] as @a[tag=same_room,gamemode=!spectator,tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/stand_still
...
execute if entity @s[scores={Dialog=2228}] run data modify entity @s ArmorItems[3].tag.luigis_mansion.scan_message set value {sender:"me",message:'{"translate":"luigis_mansion:message.melody_pianissima.scan.2"}',plural_message:'{"translate":"luigis_mansion:message.melody_pianissima.scan.2.more"}'}
