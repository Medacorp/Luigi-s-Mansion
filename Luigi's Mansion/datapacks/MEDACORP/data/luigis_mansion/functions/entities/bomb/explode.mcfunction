...
execute if entity @a[distance=..3,scores={Invulnerable=0},gamemode=!spectator] run tag @s add hit
execute as @a[distance=..3,scores={Invulnerable=0},gamemode=!spectator] run function luigis_mansion:entities/player/damage
