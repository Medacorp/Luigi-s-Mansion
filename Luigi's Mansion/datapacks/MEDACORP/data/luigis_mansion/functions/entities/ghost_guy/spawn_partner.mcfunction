...
execute as @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"dancing_ghost_guy"}}}]},tag=ghost,tag=hidden,tag=can_spawn,tag=dancing,distance=0.1..] if score @s GhostGuyCouple = #temp GhostGuyCouple run tag @s add partner
