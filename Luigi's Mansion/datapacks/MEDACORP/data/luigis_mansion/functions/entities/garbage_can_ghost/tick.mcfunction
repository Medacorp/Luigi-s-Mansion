...
execute if entity @e[tag=same_room,tag=!spectator,tag=player,distance=..0.7,limit=1] if entity @s[tag=!vanish,tag=!appear] if entity @s[tag=!dying,tag=!dead,tag=!captured,tag=!removed_from_existence,tag=!hurt,tag=!fleeing,tag=!element_hurt] run function luigis_mansion:entities/garbage_can_ghost/collide
...
execute at @s[tag=fleeing,tag=!second_flee_state] run function luigis_mansion:old_animations/garbage_can_ghost/flee
execute at @s[tag=fleeing,tag=second_flee_state] run function luigis_mansion:entities/garbage_can_ghost/second_flee_state
execute at @s[tag=!fleeing,tag=hurt] run function luigis_mansion:old_animations/garbage_can_ghost/hurt
