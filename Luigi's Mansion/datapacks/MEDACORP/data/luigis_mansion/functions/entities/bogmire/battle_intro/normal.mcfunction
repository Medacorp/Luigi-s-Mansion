...
execute if entity @s[scores={Dialog=1..459}] as @a[tag=same_room,gamemode=!spectator,tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/stand_still
...
execute if entity @s[scores={Dialog=200}] as @a[tag=same_room,tag=!spectator,tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/stand_still
...
execute if entity @s[scores={Dialog=240}] as @a[tag=same_room] at @s facing entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"bogmire"}}}]},scores={Dialog=240},limit=1] feet rotated ~ 0 positioned ^ ^ ^1 run function luigis_mansion:entities/player/animation/set/scare/normal
execute if entity @s[scores={Dialog=240}] run tag @e[tag=bogmires_shadow] add vanish
execute if entity @s[scores={Dialog=241..460}] run scoreboard players set @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"black_bogmire"}}}]},limit=1] SpawnTime 60
