...
execute if entity @s[scores={ActionTime=40}] positioned ~ ~0.4 ~ as @e[distance=..0.1,tag=bone,limit=1] unless entity @s[scores={Owner=-2147483648..}] positioned ~ ~-0.4 ~ run scoreboard players operation @s Owner = @e[nbt={ArmorItems:[{tag:{luigis_mansion:{namespace:"luigis_mansion",id:"mr_bones"}}}]},distance=..0.1,limit=1] GhostNr
