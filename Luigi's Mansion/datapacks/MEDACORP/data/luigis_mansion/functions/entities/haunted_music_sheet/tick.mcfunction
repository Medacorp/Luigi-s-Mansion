...
execute if entity @e[nbt={ArmorItems:[{tag:{namespace:"luigis_mansion",id:"melody_pianissima"}}]},tag=!vanish,limit=1] unless entity @e[nbt={ArmorItems:[{tag:{namespace:"luigis_mansion",id:"melody_pianissima"}}]},scores={VulnerableTime=1..},limit=1] run scoreboard players add @s[scores={WaitTime=0..}] WaitTime 1
