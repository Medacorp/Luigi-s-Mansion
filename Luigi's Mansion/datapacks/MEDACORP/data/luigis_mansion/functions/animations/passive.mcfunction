data modify storage luigis_mansion:data passive set value {tags:[],animation:{namespace:"luigis_mansion",id:"idle"}}
...
$execute if entity @s[tag=!freeze_animation] as @e[type=minecraft:armor_stand,tag=$(name)_model,scores={PassiveNr=$(id)}] run function luigis_mansion:animations/passive/main with storage luigis_mansion:data macro.passive
