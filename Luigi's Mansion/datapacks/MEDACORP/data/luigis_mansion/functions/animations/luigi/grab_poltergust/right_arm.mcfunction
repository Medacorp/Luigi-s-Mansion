...
execute if entity @s[tag=held_item,tag=poltergust_grabbed] unless data entity @s ArmorItems[3].tag.luigis_mansion.is_nozzle unless score #mirrored Selected matches 1 run data modify entity @s ArmorItems[3] set from storage luigis_mansion:data luigi.poltergust
execute if entity @s[tag=held_item,tag=!poltergust_grabbed] if data entity @s ArmorItems[3].tag.luigis_mansion.is_nozzle unless score #mirrored Selected matches 1 run data modify entity @s ArmorItems[3] set value {id:"minecraft:stone_button",Count:1b}
