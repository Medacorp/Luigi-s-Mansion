...
execute at @s[scores={DeathTime=11..50}] if score #mirrored Selected matches 0 run teleport @s[tag=!flipped_gravity] ^-0.01625 ^0.01125 ^
execute at @s[scores={DeathTime=11..50}] if score #mirrored Selected matches 0 run teleport @s[tag=flipped_gravity] ^0.01625 ^-0.01125 ^
...
execute if entity @s[scores={DeathTime=11..50}] if score #mirrored Selected matches 0 store result entity @s Pose.Head[2] float 1 run scoreboard players remove #temp Time 2
