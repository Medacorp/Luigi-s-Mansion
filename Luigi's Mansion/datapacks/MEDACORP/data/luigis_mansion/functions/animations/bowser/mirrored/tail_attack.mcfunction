...
execute unless entity @s[scores={AnimationProgress=11..30}] run teleport @e[tag=this_model,tag=tail,limit=1] ^ ^-2.1 ^-1.8 ~ ~
execute if entity @s[scores={AnimationProgress=11..30}] run teleport @e[tag=this_model,tag=tail,limit=1] ^ ^-2.6 ^-1.8 ~ ~
...
execute if entity @s[scores={AnimationProgress=1..10}] store result entity @e[tag=this_model,tag=tail,limit=1] Pose.Head[1] float 1 run scoreboard players remove #temp Time 5
execute if entity @s[scores={AnimationProgress=11..20}] store result entity @e[tag=this_model,tag=tail,limit=1] Pose.Head[1] float 1 run scoreboard players add #temp Time 10
execute if entity @s[scores={AnimationProgress=21..30}] store result entity @e[tag=this_model,tag=tail,limit=1] Pose.Head[1] float 1 run scoreboard players remove #temp Time 10
execute if entity @s[scores={AnimationProgress=31..40}] store result entity @e[tag=this_model,tag=tail,limit=1] Pose.Head[1] float 1 run scoreboard players add #temp Time 5
