...
execute if entity @s[scores={Dialog=1..49}] as @a[tag=same_room,gamemode=!spectator,tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/stand_still
...
tag @s[scores={Dialog=48}] remove explain
...
execute if entity @s[scores={Dialog=50}] as @a[tag=same_room,tag=!spectator,tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/none
