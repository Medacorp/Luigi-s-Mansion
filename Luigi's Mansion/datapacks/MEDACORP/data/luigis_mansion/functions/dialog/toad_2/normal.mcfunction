...
execute if entity @s[scores={Dialog=1..375}] as @a[tag=same_room,gamemode=!spectator,tag=!looking_at_map] run function luigis_mansion:entities/player/animation/set/stand_still
...
scoreboard players set @s[scores={Dialog=88}] AnimationProgress 0
...
execute if entity @s[scores={Dialog=216}] run function luigis_mansion:room/normal/balcony_1/clear
tag @s[scores={Dialog=236}] add explain
scoreboard players set @s[scores={Dialog=236}] AnimationProgress 0
...
execute if entity @s[tag=!talk,scores={Dialog=..215},tag=happy] run scoreboard players set @s AnimationProgress 0
