...
execute if score #dialog Dialog matches 27..67 run scoreboard players add #dialog Dialog 1
...
execute if score #dialog Dialog matches 3 if entity @a[tag=same_room,tag=select_dialog_branch_no,limit=1] run scoreboard players set #dialog Dialog 48
...
execute if score #dialog Dialog matches 4..46 if entity @a[tag=same_room,tag=skip_dialog,limit=1] run scoreboard players set #dialog Dialog 47
execute if score #dialog Dialog matches 48.. if entity @a[tag=same_room,tag=skip_dialog,limit=1] run scoreboard players set #dialog Dialog 69
execute if score #dialog Dialog matches ..68 unless score #dialog Dialog matches 3..23 unless score #dialog Dialog matches 27..47 as @a[tag=same_room,tag=!spectator] unless entity @s[tag=dialog_menu,tag=!dialog_choice_menu] run function luigis_mansion:selection_menu/dialog/original_menu
execute if score #dialog Dialog matches 3 as @a[tag=same_room,tag=!spectator,tag=!dialog_choice_menu] run function luigis_mansion:selection_menu/dialog/choice/toad_3
...
execute if score #dialog Dialog matches 27 run function luigis_mansion:room/clear_room_mansion with storage luigis_mansion:data current_state.current_data.mansion_id
execute if score #dialog Dialog matches 47 run scoreboard players set #dialog Dialog -1

#Branch: No
execute if score #dialog Dialog matches 48 as @a[tag=same_room,tag=!spectator] run function luigis_mansion:entities/player/animation/set/nod
execute if score #dialog Dialog matches 48 run advancement grant @a[tag=same_room] only luigis_mansion:vanilla/go_help_yourself washroom_1
execute if score #dialog Dialog matches 48 run tellraw @a[tag=same_room] {"type":"translatable","translate":"chat.type.text","with":[{"type":"translatable","translate":"luigis_mansion:entity.toad","color":"green"},{"type":"translatable","translate":"luigis_mansion:dialog.toad_3.no.1"}]}
execute if score #dialog Dialog matches 68 as @a[tag=same_room,tag=!spectator] run function luigis_mansion:entities/player/animation/set/idle

execute if score #dialog Dialog matches 69 run scoreboard players set #temp Dialog -1
