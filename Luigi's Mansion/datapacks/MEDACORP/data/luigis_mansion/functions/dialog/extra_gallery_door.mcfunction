...
tellraw @s[scores={Dialog=1}] {"translate":"chat.type.text","with":[{"translate":"luigis_mansion:entity.mansion","color":"green"},{"translate":"luigis_mansion:dialog.extra_gallery_door.1"}]}
function #luigis_mansion:dialog/extra_gallery_door
tellraw @s[scores={Dialog=56}] {"translate":"luigis_mansion:dialog.extra_gallery_door.nowhere","color":"green","clickEvent":{"action":"run_command","value":"/trigger GalleryChoice set 0"}}
...
execute if entity @s[scores={Dialog=78}] run scoreboard players operation @e[tag=door,tag=frame,tag=extra_gallery,sort=nearest,limit=1] GalleryChoice = @s GalleryChoice
execute if entity @s[scores={Dialog=78}] run tag @e[tag=door,tag=frame,tag=extra_gallery,sort=nearest,limit=1] add open_door
execute if entity @s[scores={Dialog=78}] at @e[tag=door,tag=frame,tag=extra_gallery,sort=nearest,limit=1] positioned ^ ^ ^0.5 rotated ~-180 ~ align xz run teleport @s ~0.5 ~ ~0.5 ~ ~
execute if entity @s[scores={Dialog=78}] at @e[tag=door,tag=frame,tag=extra_gallery,sort=nearest,limit=1] if entity @e[distance=..0.1,tag=door,tag=frame,tag=!locked,tag=!unlock,tag=!pull] run function luigis_mansion:entities/player/animation/set/door/open/push
execute if entity @s[scores={Dialog=78}] at @e[tag=door,tag=frame,tag=extra_gallery,sort=nearest,limit=1] if entity @e[distance=..0.1,tag=door,tag=frame,tag=!locked,tag=!unlock,tag=pull] run function luigis_mansion:entities/player/animation/set/door/open/pull
