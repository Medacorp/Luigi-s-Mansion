...
execute if entity @s[scores={Dialog=469}] at @e[tag=parlor_key,limit=1] run summon minecraft:armor_stand ~-0.7 ~0.9 ~-0.7 {CustomName:'{"translate":"luigis_mansion:item.key"}',Pose:{Head:[0.0f,-90.0f,0.01f]},Invisible:1b,Silent:1b,ArmorItems:[{},{},{},{id:"minecraft:diamond_pickaxe",Count:1b,tag:{namespace:"luigis_mansion",id:"key",variant:"parlor",Unbreakable:1b,Damage:5,CustomModelData:15}}],Small:1b,NoGravity:1b,Tags:["item","key","eternal"],Rotation:[-90.0f,0.0f],DisabledSlots:2039583}
execute if entity @s[scores={Dialog=469}] run scoreboard players set @e[nbt={ArmorItems:[{tag:{namespace:"luigis_mansion",id:"key",variant:"parlor"}}]},limit=1] ActionTime 4
