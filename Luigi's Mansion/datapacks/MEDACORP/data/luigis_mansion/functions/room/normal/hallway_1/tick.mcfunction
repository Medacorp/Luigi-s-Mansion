...
execute as @e[x=735,y=10,z=-2,dx=7,dy=16,dz=21] unless entity @s[x=743,y=10,z=-2,dx=15,dy=16,dz=21] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 2
...
execute unless data storage luigis_mansion:data dialogs[{room:1}] unless data storage luigis_mansion:data current_state.current_data{obtained_keys:["parlor"]} unless entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{item:{namespace:"luigis_mansion",id:"key"},variant:"parlor"}}}]},limit=1] if entity @e[x=736,y=19,z=8,dx=0,dy=3,dz=1,tag=door,scores={AnimationProgress=39}] run playsound luigis_mansion:entity.ghost.mumble hostile @a 751.0 13 9.0 3
...
execute if entity @a[tag=same_room,tag=!spectator,limit=1] run function luigis_mansion:room/normal/hallway_1/ghosts
