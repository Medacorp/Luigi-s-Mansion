...
execute as @e[x=734,y=8,z=-63,dx=14,dy=9,dz=21] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 21
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=21},limit=1] run function luigis_mansion:room/normal/mirror_room/ghosts