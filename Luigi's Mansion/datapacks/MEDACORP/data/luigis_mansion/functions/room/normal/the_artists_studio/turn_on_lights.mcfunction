execute unless data storage luigis_mansion:data current_state.current_data{obtained_keys:["secret_altar"]} run data modify storage luigis_mansion:data entity set value {room:71,variant:4b,rotation:[0.0f,0.0f]}
execute unless data storage luigis_mansion:data current_state.current_data{obtained_keys:["secret_altar"]} positioned 701 30.3 -74 run function luigis_mansion:spawn_entities/item/key {door:"secret_altar"}
...
execute if data storage luigis_mansion:data current_state.current_data{blackout:0b,boos:[{}],technical_data:{released_boos_talk:1b}} run function luigis_mansion:room/normal/the_artists_studio/load_boos
