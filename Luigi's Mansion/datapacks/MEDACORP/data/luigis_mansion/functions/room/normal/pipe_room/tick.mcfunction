...
execute as @a[x=686,y=-1,z=-6,dx=14,dy=9,dz=18] unless entity @s[scores={Room=67}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=686,y=-1,z=-6,dx=14,dy=9,dz=18] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 67
...
execute if entity @a[tag=!pause_dialog,scores={Room=67},limit=1] run function luigis_mansion:room/normal/pipe_room/ghosts