...
execute as @e[x=644,y=99,z=-41,dx=10,dy=9,dz=78] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 30
...
execute if entity @a[tag=exact_same_room,limit=1] run function luigis_mansion:room/normal/courtyard/ghosts
