...
execute as @e[x=712,y=1,z=-63,dx=8,dy=16,dz=21] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 15
...
execute if entity @a[tag=same_room,tag=!spectator,limit=1] run function luigis_mansion:room/normal/hallway_7/ghosts
