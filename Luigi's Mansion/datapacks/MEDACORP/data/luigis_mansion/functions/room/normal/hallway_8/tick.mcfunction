...
execute as @a[scores={Room=33}] run function luigis_mansion:room/normal/hallway_8/tick_per_player

execute if entity @e[x=661.0,y=20,z=-43.5,distance=..0.7,tag=furniture,tag=door,tag=!burning,limit=1] run data modify storage luigis_mansion:data current_state.current_data.technical_data merge value {extinguished_hallway_8_fire:1b}
