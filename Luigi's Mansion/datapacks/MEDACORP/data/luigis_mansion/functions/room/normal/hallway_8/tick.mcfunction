...
execute as @e[x=659,y=10,z=-62,dx=8,dy=16,dz=26] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 33
...
execute if entity @a[tag=same_room,tag=!spectator,limit=1] run function luigis_mansion:room/normal/hallway_8/ghosts
