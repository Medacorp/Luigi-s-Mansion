...
execute as @a[x=734,y=8,z=-39,dx=14,dy=9,dz=20] unless entity @s[scores={Room=20}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=734,y=8,z=-39,dx=14,dy=9,dz=20] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 20
...
execute if entity @a[tag=!pause_dialog,scores={Room=20},limit=1] run function luigis_mansion:room/normal/fortune_tellers_room/ghosts