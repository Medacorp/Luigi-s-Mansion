...
execute as @e[x=660,y=17,z=34,dx=8,dy=9,dz=14] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 42
...
execute as @a[scores={Room=42}] run function luigis_mansion:room/normal/washroom_2/tick_per_player
...
execute if entity @a[tag=exact_same_room,limit=1] run function luigis_mansion:room/normal/washroom_2/ghosts
