...
execute as @e[x=734,y=8,z=34,dx=14,dy=9,dz=20] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 20
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=20},limit=1] run function luigis_mansion:room/hidden/fortune_tellers_room/ghosts