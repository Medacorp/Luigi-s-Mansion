...
execute as @a[gamemode=!spectator,x=648,y=17,z=-14,dx=12,dy=9,dz=14] unless entity @s[scores={Room=43}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=648,y=17,z=-14,dx=12,dy=9,dz=14] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 43
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=43},limit=1] run function luigis_mansion:room/hidden/bathroom_2/ghosts