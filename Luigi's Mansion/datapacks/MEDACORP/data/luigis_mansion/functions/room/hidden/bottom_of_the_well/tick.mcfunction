...
execute as @a[x=645,y=90,z=14,dx=6,dy=7,dz=10] unless entity @s[x=648.5,y=93,z=25.5,distance=..0.7] unless entity @s[scores={Room=31}] run scoreboard players operation @s LastRoom = @s Room
...
execute as @e[scores={Room=31}] unless entity @s[type=!minecraft:armor_stand,type=!minecraft:item_frame] run scoreboard players set @s MirrorZ 41
...
execute if entity @a[tag=!pause_dialog,scores={Room=31},limit=1] run function luigis_mansion:room/hidden/bottom_of_the_well/ghosts