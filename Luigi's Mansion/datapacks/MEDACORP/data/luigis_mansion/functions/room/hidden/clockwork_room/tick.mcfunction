execute unless score #clockwork_room Ticking matches 1 run function #luigis_mansion:room/hidden/clockwork_room/load
execute as @a[x=682,y=117,z=-10,dx=17,dy=8,dz=35] unless entity @s[scores={Room=62}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=682,y=117,z=-10,dx=17,dy=8,dz=35] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 62

execute as @a[scores={Room=62}] run function luigis_mansion:room/hidden/clockwork_room/tick_per_player

function #luigis_mansion:room/hidden/clockwork_room/interactions/room

execute if entity @a[tag=!pause_dialog,scores={Room=62},limit=1] run function luigis_mansion:room/hidden/clockwork_room/ghosts

execute unless score #clockwork_room_elevator Time matches 1.. if entity @a[x=685.0,y=118,z=7.0,dx=1,dy=2,dz=1,tag=!spectator] run scoreboard players set #clockwork_room_elevator Searched 1
execute if score #clockwork_room_elevator Time matches 100 if entity @a[x=687.0,y=118,z=7.0,dx=1,dy=2,dz=1,tag=!spectator] run scoreboard players set #clockwork_room_elevator Searched -1