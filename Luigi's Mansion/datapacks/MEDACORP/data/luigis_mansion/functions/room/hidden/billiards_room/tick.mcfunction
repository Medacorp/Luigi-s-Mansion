...
execute as @a[x=673,y=10,z=-76,dx=15,dy=7,dz=27] unless entity @s[scores={Room=47}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=673,y=10,z=-76,dx=15,dy=7,dz=27] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 47
...
execute if entity @a[tag=!pause_dialog,scores={Room=47},tag=!spectator,limit=1] run function luigis_mansion:room/hidden/billiards_room/ghosts