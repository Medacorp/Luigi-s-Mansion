...
execute as @a[x=673,y=8,z=-76,dx=15,dy=9,dz=27] unless entity @s[scores={Room=47}] run scoreboard players operation @s LastRoom = @s Room
...
execute if entity @a[tag=!pause_dialog,scores={Room=47},limit=1] run function luigis_mansion:room/hidden/billiards_room/ghosts