...
execute as @a[gamemode=!spectator,x=667,y=8,z=3,dx=11,dy=9,dz=8] unless entity @s[scores={Room=14}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=667,y=8,z=3,dx=11,dy=9,dz=8] unless entity @s[tag=ghost,tag=appear] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 14
