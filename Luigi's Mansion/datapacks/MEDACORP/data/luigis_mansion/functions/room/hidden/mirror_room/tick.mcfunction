...
execute as @a[x=734,y=8,z=57,dx=14,dy=9,dz=21] unless entity @s[scores={Room=21}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=734,y=8,z=57,dx=14,dy=9,dz=21] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 21
...
execute if entity @a[tag=!pause_dialog,scores={Room=21},limit=1] run function luigis_mansion:room/hidden/mirror_room/ghosts