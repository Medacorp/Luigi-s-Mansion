...
execute as @a[x=465,y=23,z=-40,dx=49,dy=61,dz=85] run scoreboard players set @s LastRoom 76
...
execute if score #king_boo_battle ReturnTimer matches 100.. as @a[scores={Room=76}] run function luigis_mansion:room/hidden/king_boo_battle/clear_secret_altar
...
execute if entity @a[tag=!pause_dialog,scores={Room=76},limit=1] run function luigis_mansion:room/hidden/king_boo_battle/ghosts