...
execute as @a[x=722,y=19,z=-65,dx=8,dy=7,dz=38] unless entity @s[scores={Room=7}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=722,y=19,z=-65,dx=8,dy=7,dz=38] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 7
...
execute if entity @a[tag=same_room,tag=!spectator,limit=1] run function luigis_mansion:room/hidden/hallway_2/ghosts
