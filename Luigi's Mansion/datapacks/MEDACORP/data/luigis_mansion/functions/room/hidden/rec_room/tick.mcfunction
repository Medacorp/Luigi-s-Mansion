...
execute as @a[x=637,y=8,z=44,dx=12,dy=9,dz=33] unless entity @s[scores={Room=32}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=637,y=8,z=44,dx=12,dy=9,dz=33] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 32
...
execute if entity @a[tag=!pause_dialog,scores={Room=32},limit=1] run function luigis_mansion:room/hidden/rec_room/ghosts