...
execute as @a[x=744,y=26,z=-64,dx=22,dy=9,dz=25] unless entity @s[scores={Room=64}] run scoreboard players operation @s LastRoom = @s Room
...
execute if entity @a[tag=!pause_dialog,scores={Room=64},limit=1] run function luigis_mansion:room/hidden/armory/ghosts