...
execute as @a[x=660,y=8,z=-33,dx=8,dy=9,dz=14] unless entity @s[scores={Room=19}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=660,y=8,z=-33,dx=8,dy=9,dz=14] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 19
...
execute if entity @a[tag=!pause_dialog,scores={Room=19},limit=1] run function luigis_mansion:room/hidden/washroom_1/ghosts