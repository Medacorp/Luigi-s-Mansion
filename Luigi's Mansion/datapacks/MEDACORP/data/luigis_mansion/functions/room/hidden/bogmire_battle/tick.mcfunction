...
execute as @a[x=777,y=4,z=-14,dx=51,dy=38,dz=55] run scoreboard players set @s LastRoom 74
execute as @e[x=777,y=4,z=-14,dx=51,dy=38,dz=55] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 74
...
execute if score #bogmire_battle ReturnTimer matches 100.. as @a[scores={Room=74}] run function luigis_mansion:room/hidden/bogmire_battle/clear_graveyard
...
execute if entity @a[tag=!pause_dialog,scores={Room=74},tag=!spectator,limit=1] run function luigis_mansion:room/hidden/bogmire_battle/ghosts