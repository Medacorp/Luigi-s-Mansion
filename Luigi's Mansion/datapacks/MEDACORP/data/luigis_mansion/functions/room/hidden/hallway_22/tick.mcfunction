...
execute as @a[x=653,y=1,z=27,dx=33,dy=7,dz=12] unless entity @s[scores={Room=61}] run scoreboard players operation @s LastRoom = @s Room
execute as @e[x=653,y=1,z=27,dx=33,dy=7,dz=12] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 61
...
execute if entity @a[tag=same_room,tag=!spectator,limit=1] run function luigis_mansion:room/hidden/hallway_22/ghosts
