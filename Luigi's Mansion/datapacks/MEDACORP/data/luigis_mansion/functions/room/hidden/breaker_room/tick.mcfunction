...
execute as @a[x=686,y=-1,z=57,dx=14,dy=9,dz=21] unless entity @s[scores={Room=57}] run scoreboard players operation @s LastRoom = @s Room
...
execute if entity @a[tag=!pause_dialog,scores={Room=57},limit=1] run function luigis_mansion:room/hidden/breaker_room/ghosts