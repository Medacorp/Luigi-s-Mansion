...
execute as @a[x=686,y=-1,z=3,dx=14,dy=9,dz=18] unless entity @s[scores={Room=67}] run scoreboard players operation @s LastRoom = @s Room
...
execute if entity @a[tag=!pause_dialog,scores={Room=67},limit=1] run function luigis_mansion:room/hidden/pipe_room/ghosts