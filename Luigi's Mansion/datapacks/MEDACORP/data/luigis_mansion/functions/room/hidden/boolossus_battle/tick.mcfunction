...
execute as @a[gamemode=!spectator,x=562,y=13,z=-9,dx=48,dy=78,dz=87] run scoreboard players set @s LastRoom 75
execute as @e[x=562,y=13,z=-9,dx=48,dy=78,dz=87] unless entity @s[tag=ghost,tag=vanish] run scoreboard players set @s Room 75
...
execute if entity @a[gamemode=!spectator,tag=!pull_open_door,tag=!push_open_door,scores={Room=75},limit=1] run function luigis_mansion:room/hidden/boolossus_battle/ghosts