...
execute if data storage luigis_mansion:data furniture{mirror_reflection_edit:0b} store result score #temp Time run data get entity @e[tag=this_entity,limit=1] ArmorItems[3].tag.mirror.CustomModelData
execute if data storage luigis_mansion:data furniture{mirror_reflection_edit:0b} store result entity @e[tag=this_entity,limit=1] ArmorItems[3].tag.mirror.CustomModelData int 1 run scoreboard players remove #temp Time 1
