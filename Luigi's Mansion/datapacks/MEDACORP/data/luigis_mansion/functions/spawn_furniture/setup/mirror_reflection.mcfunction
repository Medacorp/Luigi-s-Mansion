...
execute store result score #temp Time run data get entity @e[tag=this_entity,limit=1] Pose.Head[0] 100
...
execute if data storage luigis_mansion:data furniture{mirror_reflection_edit:0b} as @e[tag=this_entity,limit=1] if data entity @s ArmorItems[3].tag.mirror store result score #temp Time run data get entity @s ArmorItems[3].tag.mirror.CustomModelData
execute if data storage luigis_mansion:data furniture{mirror_reflection_edit:0b} as @e[tag=this_entity,limit=1] if data entity @s ArmorItems[3].tag.mirror store result entity @s ArmorItems[3].tag.mirror.CustomModelData int 1 run scoreboard players remove #temp Time 1
