...
execute at @s[scores={ActionTime=1..60}] run function luigis_mansion:old_animations/mr_bones/attack
...
execute if entity @s[scores={ActionTime=40}] run summon minecraft:armor_stand ~ ~0.4 ~ {CustomName:'{"type":"translatable","translate":"luigis_mansion:entity.bone"}',NoGravity:1b,Tags:["bone"],Pose:{Head:[0.0f,0.0f,0.01f]},ArmorItems:[{},{},{},{id:"minecraft:diamond_pickaxe",Count:1b,tag:{Unbreakable:1b,Damage:4,CustomModelData:6,luigis_mansion:{entity:{namespace:"luigis_mansion",id:"bone"}}}}],Invisible:1b,DisabledSlots:2039583,Small:1b}
execute if entity @s[scores={ActionTime=40}] positioned ~ ~0.4 ~ run data modify entity @e[tag=bone,distance=..0.1,limit=1] ArmorItems[3].tag.luigis_mansion.damage set from entity @s ArmorItems[3].tag.luigis_mansion.damage
execute if entity @s[scores={ActionTime=40}] positioned ~ ~0.4 ~ as @e[distance=..0.1,tag=bone,limit=1] unless entity @s[scores={Owner=-2147483648..}] positioned ~ ~-0.4 ~ run scoreboard players operation @s Owner = @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"mr_bones"}}}}]},distance=..0.1,limit=1] GhostNr
...
execute at @s[scores={ActionTime=60..61}] run function luigis_mansion:old_animations/mr_bones/haunt
