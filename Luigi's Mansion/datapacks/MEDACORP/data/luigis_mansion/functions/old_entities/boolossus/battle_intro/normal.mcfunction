...
scoreboard players set @s[tag=flee] Dialog 440
execute if entity @s[scores={Dialog=1..99}] as @a[tag=same_room,gamemode=!spectator,tag=!looking_at_map] unless entity @s[scores={AnimationProgress=1..}] run function luigis_mansion:entities/player/animation/set/high_health_idle_no_sound
