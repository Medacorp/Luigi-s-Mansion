...
execute as @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"dancing_ghost_guy"}}}}]},tag=ghost,tag=hidden,tag=can_spawn,tag=dancing,distance=0.1..] if score @s GhostGuyCouple = #temp GhostGuyCouple run tag @s add partner
...
execute as @e[tag=partner,limit=1] at @s run function luigis_mansion:old_entities/ghost/hidden/selection
execute if entity @e[tag=partner,limit=1] if block ~ ~ ~ #minecraft:slabs[type=bottom] run teleport @s ~ ~-0.5 ~
execute if entity @e[tag=partner,limit=1] run function luigis_mansion:old_entities/ghost/hidden/selection
