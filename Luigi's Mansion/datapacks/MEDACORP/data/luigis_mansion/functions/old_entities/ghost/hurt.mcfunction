...
execute if entity @s[scores={MushroomTimer=0},tag=portrait_ghost,tag=!no_poison_mushroom_dropping] run function luigis_mansion:old_entities/ghost/drop_poison_mushroom
tag @s[scores={HurtTime=30},tag=!no_ai] add flee
execute at @s[scores={HurtTime=1..21},tag=!ground_bound,tag=!ceiling_ghost] run function luigis_mansion:old_entities/ghost/hurt_move
...
tag @s[tag=!in_vacuum,tag=!no_ai] remove flee
