...
scoreboard players set @s[scores={ElementHurtTime=0},tag=!freeze] AnimationProgress 0
...
execute if entity @s[scores={ElementHurtTime=1}] run data modify entity @s ArmorItems[3].tag.luigis_mansion.attacked_by set value []
scoreboard players remove @s ElementHurtTime 1
execute if entity @s[scores={ElementHurtTime=10..}] run function luigis_mansion:old_entities/ghost/hurt_element_damage
