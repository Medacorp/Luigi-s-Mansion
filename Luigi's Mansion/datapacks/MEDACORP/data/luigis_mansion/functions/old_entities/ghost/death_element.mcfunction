...
execute if entity @s[scores={DeathTime=1}] as @a[tag=vacuuming_this_ghost] run function luigis_mansion:old_entities/ghost/find_shooters
...
execute at @s[scores={DeathTime=1,Room=1..}] run function luigis_mansion:old_entities/ghost/capture with entity @a[tag=killer,limit=1]
execute at @s[scores={DeathTime=1,Room=-2}] run scoreboard players add #training_room TrainingRoomScore 1
execute at @s[scores={DeathTime=1,Room=-2,TrainingRoomScore=1..}] run scoreboard players operation @a[tag=killer] TrainingRoomScore += @s TrainingRoomScore
