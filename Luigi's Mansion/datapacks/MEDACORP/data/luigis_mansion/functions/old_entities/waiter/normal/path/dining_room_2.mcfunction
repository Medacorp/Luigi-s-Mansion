...
execute if entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"mr_luggs"}}}}]},scores={Dialog=..100}] run tag @s[scores={PathStep=4}] add placing_food
execute if entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"mr_luggs"}}}}]},scores={Dialog=101..}] run tag @s[scores={PathStep=4}] add complain
scoreboard players set @s[scores={PathStep=4}] AnimationProgress 0
execute if entity @s[scores={PathStep=6}] run scoreboard players set @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"mr_luggs"}}}}]},scores={Dialog=2..99},limit=1] Dialog 1
...
execute unless entity @s[scores={PathStep=4..12}] at @s run function luigis_mansion:old_entities/ghost/move