...
execute if entity @s[tag=walk_on_ceiling] store result entity @s Rotation[0] float 1 run scoreboard players get @s HomeRotation
execute at @s[tag=walk_on_ceiling] run teleport @s ~ ~ ~ ~-180 ~
execute if score #temp Move matches 1.. positioned as @s[tag=!walk_on_ceiling] run function luigis_mansion:old_entities/purple_mouse/move_up