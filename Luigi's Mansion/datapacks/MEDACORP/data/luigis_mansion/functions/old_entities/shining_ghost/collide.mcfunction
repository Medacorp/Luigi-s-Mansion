...
advancement grant @a[tag=!spectator,scores={Invulnerable=0},distance=..0.7] only luigis_mansion:vanilla/waluigi_pinball/shining_ghost
execute if entity @a[tag=!spectator,scores={Invulnerable=0},distance=..0.7] run tag @s add disappear
data modify storage luigis_mansion:data damage set value {method:{namespace:"luigis_mansion",id:"shining_ghost"},amount:0,animation:{namespace:"luigis_mansion",id:"knockback/small"},attacker:-1,no_delete:1b}
