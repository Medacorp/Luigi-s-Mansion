...
execute if entity @s[scores={ActionTime=10}] positioned ^ ^0.2 ^ as @e[distance=..0.1,tag=bowling_ball,limit=1] unless entity @s[scores={GhostNr=1..}] positioned ^ ^-0.2 ^ run scoreboard players operation @s Owner = @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"bowling_ghost"}}}}]},distance=..0.1,limit=1] GhostNr
execute if entity @s[scores={ActionTime=11..20}] if score #mirrored Selected matches 0 run function luigis_mansion:old_entities/bowling_ghost/ball_position/normal
execute if entity @s[scores={ActionTime=11..20}] if score #mirrored Selected matches 1 run function luigis_mansion:old_entities/bowling_ghost/ball_position/mirrored
