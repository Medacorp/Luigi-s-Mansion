...
execute as @a[distance=..0.7,scores={Invulnerable=0},tag=!spectator,tag=!pull_open_door,tag=!push_open_door] run function luigis_mansion:old_entities/fake_door/burning

scoreboard players add @s ElementHurtTime 1
execute if entity @s[scores={ElementHurtTime=..50}] positioned ^ ^ ^0.3 run function luigis_mansion:old_entities/ghost/hurt_element_damage
