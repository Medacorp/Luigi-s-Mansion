...
execute if entity @e[tag=same_room,tag=!spectator,tag=player,distance=..0.7,limit=1] if entity @s[tag=visible] run function luigis_mansion:old_entities/spark/collide

execute if entity @s[tag=!visible,tag=!turned] run function luigis_mansion:old_entities/spark/initial_rotation
execute store result score #temp Room run scoreboard players get @s Room
execute as @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"spark"}}}}]},tag=visible] if score @s Room = #temp Room run scoreboard players add #temp ActionTime 1
execute unless score #temp ActionTime matches 2.. unless entity @e[distance=..0.7,tag=same_room,tag=!spectator,tag=player,limit=1] unless entity @e[nbt={ArmorItems:[{tag:{luigis_mansion:{entity:{namespace:"luigis_mansion",id:"spark"}}}}]},tag=visible,distance=..0.7] run tag @s add visible
...
execute at @s[scores={SpawnTime=1..}] run function luigis_mansion:old_entities/ghost/move_forward
execute at @s[tag=visible] if block ~ ~-0.01 ~ #luigis_mansion:ghosts_ignore run teleport @s ~ ~-0.01 ~
execute at @s[scores={Turn=0},tag=visible] run function luigis_mansion:old_entities/spark/turn
...
execute if entity @s[tag=explode,tag=visible] run data modify storage luigis_mansion:data damage set value {method:{namespace:"luigis_mansion",id:"explosion"},amount:0,animation:{namespace:"luigis_mansion",id:"knockback/burn"},attacker:-1,no_delete:1b}
