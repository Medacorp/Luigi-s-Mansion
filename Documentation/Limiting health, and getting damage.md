Damage is done by calling the `luigis_mansion:entities/luigi/damage` function as the Luigi to take damage from the position where the damage originates from. It'll look at the damage data to determine what to do. This data is stored in the storage `luigis_mansion:data damage` object.
...
damage: {
    ...
    animation:{ //The animation to use.
        ...
        macro:"{...}" //Optional, macro argument for the animation.
    },
...
Run `luigis_mansion:entities/luigi/damage` again after limiting max health to have it "stack" with the health limitation.
