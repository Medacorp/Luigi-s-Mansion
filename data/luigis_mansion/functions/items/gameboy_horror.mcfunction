...
execute unless entity @a[scores={GBHCall=1..,Dialog=1..}] unless entity @e[tag=e_gadd,scores={Room=-2}] if entity @s[tag=!scanning,nbt={Inventory:[{tag:{luigis_mansion:{id:"luigis_mansion:gameboy_horror"}}}]},nbt=!{Inventory:[{tag:{Damage:1,luigis_mansion:{id:"luigis_mansion:gameboy_horror"}}}]}] run function luigis_mansion:items/gameboy_horror/turn_screen_off
