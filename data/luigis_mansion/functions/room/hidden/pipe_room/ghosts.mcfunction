execute if score #pipe_room Wave matches 2 unless entity @e[tag=ghost,tag=!optional_ghost,scores={Room=67}] run function luigis_mansion:room/hidden/pipe_room/clear
execute if score #pipe_room Wave matches 1 unless entity @e[tag=ghost,tag=!optional_ghost,scores={Room=67}] run function luigis_mansion:room/hidden/pipe_room/wave_2
